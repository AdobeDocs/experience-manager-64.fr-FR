...
1. Ajoutez le chemin `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` au fichier conf à l’aide de la commande `cat SWitchEngineLibs.conf`.

1. Exécutez la commande `ldconfig` pour créer les liens et le cache nécessaires.

1. Dans le compte utilisé pour démarrer AEM, modifiez le fichier `.bash_profile`. Ajoutez `LD_LIBRARY_PATH` en ajoutant ce qui suit.
