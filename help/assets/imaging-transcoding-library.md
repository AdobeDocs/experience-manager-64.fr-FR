...
1. Ajoutez `/aem64/author/crx-quickstart/launchpad/felix/bundle<id>/data/binaries/` chemin d’accès au fichier conf à l’aide de la commande `cat SWitchEngineLibs.conf`.

1. Exécutez la commande `ldconfig` pour créer les liens et le cache nécessaires.

1. Dans le compte utilisé pour début de l&#39;AEM, modifiez le fichier `.bash_profile`. Ajoutez `LD_LIBRARY_PATH` en ajoutant ce qui suit.
...
### Configurer le processus de mise à jour des ressources de gestion des actifs numériques {#configure-dam-asset-update-workflow}
