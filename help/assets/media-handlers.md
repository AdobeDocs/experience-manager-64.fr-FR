...
1. Copiez le lot `myBundle-0.0.1-SNAPSHOT.jar` et enregistrez-le sous `/apps/myApp/install` (avec WebDAV, par exemple). Le nouveau gestionnaire de texte est désormais principal en Experience Manager.
...
1. Pour vérifier si l’outil fonctionne correctement, ajoutez une image PDF dans le répertoire de travail et exécutez la commande de `convert <image-name>.jpg -flip <image-name>-flipped.jpg` sur la ligne de commande.
