---
title: Meilleures pratiques d’intégration d’AEM et de Creative Cloud
description: Meilleures pratiques pour intégrer un déploiement AEM à Adobe Creative Cloud afin de rationaliser les processus de transfert de ressources et d’optimiser l’efficacité
contentOwner: AG
...
Adobe Experience Manager Assets est une solution de gestion des actifs numériques (DAM) qui peut s’intégrer à Adobe Creative Cloud pour aider les utilisateurs de DAM à travailler ensemble avec des équipes de création, en rationalisant la collaboration dans le processus de création de contenu.
...
Cet article se concentre principalement sur les deux premiers aspects des besoins de collaboration. La distribution et l’approvisionnement de ressources à l’échelle de l’entreprise sont brièvement mentionnés comme cas d’utilisation. Pour répondre à ces besoins, pensez à Adobe Brand Portal ou à Asset Share Commons. Alternate solutions such as [Brand Portal](https://helpx.adobe.com/experience-manager/brand-portal/user-guide.html), solutions that can be built based on [Asset Share Commons](https://adobe-marketing-cloud.github.io/asset-share-commons/) components, [Link Share](/help/assets/link-sharing.md), using [Experience Manager Assets](/help/assets/managing-assets-touch-ui.md) should be reviewed based on specific requirement.
...
|---|---|---|---|
| Partager un nombre inférieur (1) de ressources DAM avec un utilisateur créatif | ✔✔ | ✔ |  |
...
* (1) Le nombre d&#39;actifs est moindre : par exemple, un petit ensemble d’actifs lié à un projet ou une campagne
* 2) Un plus grand nombre d&#39;actifs : par exemple, tous les actifs approuvés de l’organisation
* (3) Utilisez la fonction de dossier de téléchargement d’application de bureau AEM
...
* [Portail](https://helpx.adobe.com/experience-manager/brand-portal/user-guide.html) de marque pour un module complémentaire SaaS configurable aux ressources AEM pour publier des fichiers.
...
* **Travail en cours ou travail créatif en cours :** phase du cycle de vie d’une ressource au cours de laquelle une ressource subit plusieurs modifications et n’est généralement pas encore prête à être partagée avec des équipes plus grandes.
* **Ressources prêtes pour la création :** ressources prêtes à être partagées avec une équipe plus grande ou sélectionnées/approuvées par l’équipe créative pour le partage avec les équipes marketing ou LOB.
* **Approbations de ressources :** processus d’approbation qui s’exécute pour les ressources déjà transférées vers DAM, qui comprend généralement les approbations de marque, les approbations légales, etc.
* **Fichier final :** ressource qui a fait l’objet de toutes les approbations/balises de métadonnées et qui est prête à être utilisée par la grande équipe. Ce fichier est stocké dans DAM et mis à la disposition de tous les utilisateurs (ou de tous les utilisateurs intéressés). Il peut être utilisé dans les canaux marketing ou par des équipes créatives pour créer des conceptions.
...
* **DAM :** gestion des actifs numériques. Dans ce document, celle-ci est synonyme des ressources d’AEM Experience Manager, sauf mention contraire spécifique.
...
* **Utilisateur DAM :** utilisateur type d’un système de gestion des ressources numériques. En fonction de l’organisation, un utilisateur DAM peut être un utilisateur marketing ou non marketing, par exemple un utilisateur du secteur d’activité, un bibliothécaire, un vendeur, etc.
...
Il s’agit d’un bref résumé des meilleures pratiques relatives à l’intégration d’Experience Manager et de Creative Cloud. Lisez la suite de ce document pour obtenir une description détaillée des points suivants.

* **Pour les utilisateurs créatifs travaillant dans Photoshop, InDesign ou Illustrator :** Adobe Asset Link offre la meilleure expérience utilisateur, notamment une gestion propre du travail en cours sur les ressources extraites d’AEM.
* **Pour simplifier l’accès aux ressources du bureau pour tout format de fichier ou application générique :** utilisez l’application de bureau AEM.
...
* **Tenez compte du volume des ressources partagées :** si votre cas d’utilisation est la distribution des ressources, la gouvernance et la sécurité peuvent être les aspects les plus importants. Pensez à utiliser des outils conçus pour le faire à grande échelle, comme Brand Portal.
* **Comprenez le cycle de vie des ressources :** sachez comment les ressources sont gérées dans votre entreprise par différentes équipes
* **Gérez avec soin les enregistrements fréquents sur les ressources :** Adobe Asset Link s’occupe de cela pour vous avec PS, AI, ID. Pour les autres applications, n’effectuez pas les tâches en cours dans le dossier mappé/partagé, sauf si vous avez besoin de toutes les modifications dans DAM.
...
En règle générale, les équipes de création internes, les agences et les professionnels de la création intégrés au réseau interne ont accès à l’instance DAM, y compris la connexion à AEM.

Dans ce cas, l’application AEM pour bureau facilite l’accès aux ressources finales/approuvées et vous permet d’enregistrer des ressources prêtes pour la création dans la gestion des actifs numériques.
...
* Pour donner accès à quelques ressources : Le partage de dossiers AEM avec Adobe Creative Cloud peut être utilisé en plus du portail de marques AEM Assets ou du partage de ressources. Veuillez noter qu&#39;il existe certaines limitations liées à cette intégration, décrites plus en détail dans cet article.
...
1. Utilisez (placez) des fichiers approuvés dans une nouvelle conception : L’utilisateur créatif crée un fichier à l’aide des ressources approuvées existantes dans les applications Creative Cloud.
...
* **** 1 : Partagez le fichier de DAM sur votre bureau ou ouvrez-le directement sur votre bureau dans l’application de votre choix (par exemple, Adobe Photoshop, etc.). L’extraction est recommandée pour verrouiller le fichier.
...
* **** 4 : Les actifs sont mis en production.
...
* Transfert des résultats de  photoshoots ou projets plus volumineux
...
Notez que cette description fait référence aux transferts de fichiers du point de vue des opérations (par exemple, chaque semaine ou à chaque   photo , etc), en tant que partie normale du flux de travail de l’utilisateur de bureau. Les migrations de ressources de grande taille ne sont pas abordées ici.
