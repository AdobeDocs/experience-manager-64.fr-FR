...
* PNG demande d’obtenir des images sans perte.
* Demandes JPEG à différents paramètres de qualité.
* JPEGXR demande une gamme de paramètres de qualité.
* Demandes JPEG2000 à différents paramètres de qualité.
* WEBP demande à différents paramètres de qualité.
...
L’imagerie dynamique fonctionne avec vos paramètres d’image prédéfinis existants et conserve pratiquement tous vos paramètres d’image, tels que la taille, la qualité, l’accentuation, etc. Ce qui va changer, c&#39;est le format d&#39;image et les paramètres de qualité selon la demande. Pour la conversion de format, nous conservons une fidélité visuelle complète, telle que définie par vos paramètres d’image prédéfinis, mais avec une taille de fichier plus petite.
...
* Si vous utilisez Contenu multimédia dynamique, suivez les instructions pour [configurer les paramètres](config-dynamic.md) d’image Contenu multimédia dynamique Définissez la valeur **[!UICONTROL Expiration]** sur 24 heures ou plus.
...
L’imagerie dynamique fonctionne avec les images diffusées sur HTTP ou HTTPS. En outre, il fonctionne également sur HTTP/2.
