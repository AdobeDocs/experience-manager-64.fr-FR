...
Cette section décrit comment créer, modifier et gérer généralement les paramètres d’image prédéfinis. Vous pouvez appliquer un paramètre d’image prédéfini à une image lorsque vous la prévisualisez. Voir [Application de paramètres d’image prédéfinis](image-presets.md).
...
>
>Dans *Dynamic Media - mode Scene7*, vous devez *not* doivent publier les paramètres d’image prédéfinis, car ils sont automatiquement publiés.
>
...
Si vous avez l’intention de prendre en charge l’ingestion de fichiers AI, EPS et PDF afin de pouvoir générer des rendus dynamiques de ces formats de fichiers, vous pouvez consulter les informations suivantes avant de créer des paramètres d’image prédéfinis.

Le format de fichier d’Adobe Illustrator est une variante du format PDF. Les principales différences, dans le contexte d’AEM Assets, sont les suivantes :
...
* Les documents PDF se composent d’une ou de plusieurs pages. Chaque page est extraite sous la forme d’une sous-ressource de PDF de page unique sous le document de PDF de plusieurs pages principal.
...
Vous pouvez afficher les sous-ressources ou les pages lorsque vous ouvrez la ressource, appuyez sur le menu Contenu et sélectionnez **[!UICONTROL Sous-ressources]** ou **[!UICONTROL Pages]**. Les sous-ressources sont de véritables ressources. En d’autres termes, les pages PDF sont extraites par le composant de workflow `Create Sub Asset`. Elles sont ensuite stockées sous les noms `page1.pdf`, `page2.pdf`, etc. sous la ressource principale. Une fois qu’elles sont stockées, la variable **[!UICONTROL Ressources de mise à jour de gestion des actifs numériques]** le workflow les traite.
...
1. Le `cqdam.preview.png` Le rendu est ensuite optimisé dans un fichier PTIFF par la fonction **[!UICONTROL Ressources d’image de processus Dynamic Media]** composant de processus dans le workflow.
...
Vous avez accès à **[!UICONTROL Pixelliser le rendu d’aperçu d’image PDF/AI]** options du composant de processus par le biais de **[!UICONTROL Ressources de mise à jour de gestion des actifs numériques]** workflow.
...
**[!UICONTROL Largeur max. et Hauteur max. limitent la résolution à laquelle la pixellisation doit être effectuée.]****** Par exemple, si les valeurs maximales restent inchangées et que la résolution est définie sur 300 ppp, un document de lettre américaine est pixellisé à 186 ppp. En d’autres termes, le document fait 1 581 x 2 046 pixels.

Le **[!UICONTROL Pixelliser le rendu d’aperçu d’image PDF/AI]** un maximum est défini pour le composant de processus afin de s’assurer qu’il ne crée pas d’images trop grandes en mémoire. De telles images volumineuses peuvent déborder la mémoire fournie à la JVM (Java Virtual Machine). Il faut veiller à fournir suffisamment de mémoire à la machine virtuelle Java pour gérer le nombre configuré de workflows parallèles, de sorte que chacun d’eux soit en mesure de créer une image à la taille maximale configurée.
...
Pour les fichiers InDesign, les sous-ressources ne sont extraites que si le serveur Adobe InDesign est intégré à AEM. Les ressources référencées sont liées en fonction de leurs métadonnées. L’InDesign Server n’est pas nécessaire pour la liaison. Toutefois, les ressources référencées doivent être présentes dans AEM avant le traitement des fichiers d’InDesign pour que les liens soient créés entre les fichiers d’InDesign et les ressources référencées.

Voir [Intégration d’AEM Assets à InDesign Server](indesign.md).

Le composant Processus d’extraction de médias dans **[!UICONTROL Ressources de mise à jour de gestion des actifs numériques]** le workflow exécute plusieurs préconfigurés **[!UICONTROL Étendre les scripts]** pour traiter les fichiers d’InDesign.
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>Non</td>
   <td>Génère une sous-ressource PDF pour chaque page. La sous-ressource du PDF est traitée comme décrit précédemment. Étant donné que le PDF ne contient qu’une seule page, aucune sous-ressource n’est générée.<br/> </td>
  </tr> 
 </tbody> 
</table>
...
Le dimensionnement des miniatures est défini au format suivant : **width:height:center**, par exemple *80:80:false*. La largeur et la hauteur déterminent la taille en pixels de la miniature ; la valeur du centre est false ou true et, si elle est définie sur true, elle indique que la taille de l’image miniature est exactement la même que celle donnée dans la configuration. Si l’image redimensionnée est plus petite, elle est centrée dans la miniature.
...
1. Appuyer **[!UICONTROL Enregistrer]** pour enregistrer les modifications apportées au workflow.
...
>
>Dans le mode hybride de Dynamic Media, vous devez publier manuellement les paramètres d’image prédéfinis et les ressources d’image.
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>Vous pouvez remplacer l’intention de rendu par défaut. Les intentions de rendu déterminent ce qui arrive aux couleurs qui ne peuvent pas être reproduites dans le profil colorimétrique cible (hors gamme). L’intention de rendu est ignorée si elle n’est pas compatible avec le profil ICC.
    <ul> 
     <li>Sélectionner <strong>Perception</strong> pour compresser la gamme totale d’un espace colorimétrique dans un autre lorsqu’une ou plusieurs couleurs de l’image d’origine se situent en dehors de la gamme de l’espace colorimétrique de destination.</li>
...
1. Une fois le paramètre d’image prédéfini publié, l’état passe de la publication non publiée à la publication.
