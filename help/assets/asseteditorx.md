---
title: Etendre l’éditeur de fichiers
...
* La bibliothèque cliente doit également être incluse dans le mode WCM désactivé (par exemple, chargée lors de la **publication**) pour pouvoir générer les prédicats, les actions et les objectifs.
...
Some of the AEM Assets components require JS functions defined in `component.js`. Copiez ce fichier dans votre répertoire de composants et liez-le.
...
Certains composants d’AEM Assets utilisent la bibliothèque de widgets AEM. Pour que le rendu soit effectué correctement dans le contexte du contenu, une feuille de style supplémentaire doit être chargée. Le composant d’action de balise nécessite une feuille de style supplémentaire.
...
1. Ajoutez `content.xml` le fragment de code suivant :
...
1. Ajoutez `samplemeta.jsp` le fragment de code suivant :
...
1. Pour rendre le composant disponible, vous devez pouvoir le modifier. To make a component editable, in CRXDE Lite, add a node `cq:editConfig` of primary type `cq:EditConfig`. Pour pouvoir supprimer des paragraphes, ajoutez une propriété `cq:actions` à plusieurs valeurs avec une seule valeur de `DELETE`.
...
1. En mode **Édition**, le nouveau composant (par exemple, **Échantillons de métadonnées**) est désormais disponible dans le panneau latéral (situé dans le groupe **Éditeur de ressources**). Insérez le composant. Pour pouvoir stocker les métadonnées, il doit être ajouté au formulaire de métadonnées.
