...
Tout projet SPA sur AEM doit tirer parti de l’archétype [Maven pour le kit](https://github.com/adobe/aem-spa-project-archetype)de démarrage SPA.
...
* **[Portabilité](/help/sites-developing/spa-architecture.md#portability)-**Comme pour tout composant, les composants doivent être conçus pour être aussi portables que possible. L’application d’une seule page doit être créée avec des composants portatifs et réutilisables, évitant ainsi les chemins statiques qui font référence à la structure de contenu.
...
* **[Rendu](/help/sites-developing/spa-architecture.md#dynamic-rendering)dynamique :**tout rendu doit être dynamique.
* **[Routage](#dynamic-routing)dynamique -**L&#39;application d&#39;une seule page est responsable du routage et AEM l&#39;écoute et récupère les données du composant en fonction. Tout routage doit également être dynamique.
...
Le développeur doit éviter d’utiliser des chemins statiques faisant référence à la structure du contenu, car les chemins peuvent être modifiés à tout moment par les auteurs du contenu. Cela limite également la réutilisation de la bibliothèque et empêche l’utilisation de l’éditeur de modèles AEM, car sa structure se trouve à un autre emplacement que le contenu.
