...
>
>HTL est désormais le langage de script recommandé pour AEM. See [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) and [Developing AEM Components](/help/sites-developing/developing-components.md) to compare methods.
...
* `<cq:defineObjects />`
   ...
   * `pageManager` - Gestionnaire de pages pour accéder aux pages de contenu AEM ( `resourceResolver.adaptTo(PageManager.class);`).
   * `component` – Objet du composant AEM en cours.
   * `designer` - Objet designer pour récupérer les informations de conception ( `resourceResolver.adaptTo(Designer.class);`).
...
* Via l’objet properties introduit dans `global.jsp`:
...
* Par l&#39; `currentPage` objet introduit dans `global.jsp`:
   ...
   L’ `currentPage` objet est une instance d’une page (voir API [](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/Page.mhtml)AEM). La classe de page fournit plusieurs méthodes d’accès au contenu.

   Exemple: `String pageTitle = currentPage.getTitle();`

* Via `currentNode` un objet introduit dans `global.jsp`:
...
1. Ouvrez le nouveau dossier de composants et effectuez les modifications voulues. Supprimez également toute information superflue du dossier.
   ...
   * Ajouter un nouveau champ dans la boîte de dialogue
      ...
      * `cq:dialog` - boîte de dialogue de l’interface utilisateur tactile
...
1. Modifiez les métadonnées du composant :
   ...
   * Groupe, où le composant est répertorié dans le sidekick (laisser tel quel)
      ...
      * Laisser `componentGroup` définie sur `General`
