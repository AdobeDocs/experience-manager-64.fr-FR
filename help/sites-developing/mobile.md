...
* Par l’intermédiaire de son `head.jsp`, il récupère le groupe d’appareils mobiles actuel de la requête et, si un groupe d’appareils est trouvé, utilise la méthode `drawHead()` du groupe pour inclure le composant init de l’émulateur associé du groupe d’appareils (uniquement en mode création) et le CSS de rendu du groupe d’appareils.
...
* [com.day.cq.wcm.mobile.api.device.ability](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/api/device/capability/package-summary.html)  - définit DeviceCapability.
...
* [com.day.cq.wcm.mobile.core](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/core/package-summary.html)  - définit MobileUtil, qui fournit diverses méthodes d’utilitaire relatives à WCM Mobile.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>Liste mobile</td> 
   <td>Mobile</td> 
   <td>- en fonction du composant de base de liste<br /> - listitem_teaser.jsp effectue le rendu d’une image si l’appareil est capable<br /> </td> 
  </tr> 
  <tr> 
   <td>mobilelogo</td> 
   <td>hidden</td> 
...
>[!NOTE]
>
>Dans un fichier jsp, `slingRequest` est disponible par le biais de la balise `<sling:defineObjects>` et `currentPage` par le biais de la balise `<cq:defineObjects>`.
...
* Un groupe d’appareils est composé d’un ou de plusieurs émulateurs : la page de configuration du groupe d’appareils, par exemple /etc/mobile/groups/touch, contient la propriété `emulators` sous le noeud `jcr:content` .

   Remarque : bien que le même émulateur puisse être affecté à plusieurs groupes d’appareils, ce n’est pas très logique.

* Par le biais de la boîte de dialogue de configuration du groupe d’appareils, la propriété `emulators` est définie avec le chemin du ou des émulateurs souhaités. Par exemple : `/libs/wcm/mobile/components/emulators/iPhone4`.

* Les composants de l’émulateur (par exemple, `/libs/wcm/mobile/components/emulators/iPhone4`) étendez le composant d’émulateur mobile de base ( `/libs/wcm/mobile/components/emulators/base`).
...
* **** Smartphones, pour les appareils comme le Blackberry avec prise en charge du code HTML de base et des images, mais pas de JavaScript.
...
1. Créez une page sous **Outils** > **Mobile** > **Groupes d’appareils**. Dans la boîte de dialogue **Créer une page** : 
   ...
   * Sous **Titre**, saisissez `Special Phones`.
   * Sous **Nom**, saisissez `special`.
   * Sélectionnez un **Modèle de groupe de périphériques mobiles**.
   * Cliquez sur **Créer**.

1. Dans CRXDE, ajoutez un fichier **static.css** contenant les styles pour le groupe d’appareils sous le noeud `/etc/mobile/groups/special` .
...
AEM utilise une version tronquée de la base de données [WURFL](http://wurfl.sourceforge.net/index.php)™ pour interroger les fonctionnalités de l’appareil, telles que la résolution d’écran ou la prise en charge JavaScript, en fonction de l’agent utilisateur de l’appareil.

Le code XML de la base de données WURFL™ est représenté sous la forme de noeuds sous `/var/mobile/devicespecs` en analysant le fichier `wurfl.xml`à `/libs/wcm/mobile/devicespecs/wurfl.xml.` L’extension aux noeuds a lieu la première fois que le lot `cq-mobile-core` est démarré.
...
1. Renommez le fichier `wurfl.xml`.
...
1. Ajoutez les propriétés suivantes au nœud  :
   ...
   * **regexp** : expression régulière définissant user-agents, par exemple : .&amp;ast;Mozilla.&amp;ast;iPad.&amp;ast;AppleWebKit.&amp;ast;Safari.&amp;ast;
...
AEM prend en charge la détection côté client des appareils en fonction de `BrowserMap`. `BrowserMap` est fourni dans AEM en tant que bibliothèque cliente sous  `/etc/clientlibs/browsermap`.
...
* `cq:childNodesMapTo` - détermine à quel attribut de l’élément de lien les noeuds enfants seront mappés ; il est recommandé d’organiser le contenu de votre site web de telle sorte que les enfants du noeud racine représentent la racine d’une variante linguistique de votre site web global (par exemple,  `/content/mysite/en`,  `/content/mysite/de`), auquel cas la valeur de  `cq:childNodesMapTo` doit être  `hreflang`;
...
>[!NOTE]
>
>Pour plus de détails, veuillez lire la section [BrowserMap personnalisé](#using-browsermap-on-your-pages).
...
Par exemple, un périphérique qui navigue `www.example.com/index.html` identifié comme `smartphone` par BrowserMap est transféré à `www.example.com/index.smartphone.html.`
...
Outre l’ajout de la bibliothèque cliente `BrowserMap` à vos fichiers `JSP`, vous devez également ajouter une propriété String `cq:deviceIdentificationMode` définie sur `client-side` au noeud `jcr:content` sous la racine de votre site web.
...
Le script `/libs/wcm/core/browsermap/browsermap.jsp` est ainsi créé pour ajouter une balise meta à la page, ce qui permet à `BrowserMap` de ne pas effectuer de détection :
...
1. Un iPad envoie une requête à l’instance de publication AEM, par exemple `http://localhost:4503/content/geometrixx_mobile/en/products.html`
...
1. AEM mappe les fonctionnalités de l’appareil au groupe d’appareils et définit `touch` en tant que sélecteur de groupe d’appareils.

1. AEM redirige la demande vers `http://localhost:4503/content/geometrixx_mobile/en/products.touch.html.`
