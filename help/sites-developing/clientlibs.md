...
>[!CAUTION]
>
>`<cq:includeClientLib>`, qui auparavant était généralement utilisé pour inclure des bibliothèques clientes, est obsolète depuis AEM 5.6. [ doit `<ui:includeClientLib>`](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib) être utilisé à la place comme décrit ci-dessus.
...
>[!NOTE]
>
>Les ressources statiques sous le dossier de bibliothèque client doivent se trouver dans un dossier appelé *ressources*. Si vous ne disposez pas des ressources statiques, telles que les images, sous les *ressources* de dossier, elles ne peuvent pas être référencées sur une instance de publication. Voici un exemple : http://localhost:4503/etc.clientlibs/geometrixx/components/clinetlibs/resources/example.gif
