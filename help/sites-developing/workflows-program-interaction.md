...
| Informations relatives à un noeud qui se trouve dans le processus (ou non) | [`WorkflowStatus`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/status/WorkflowStatus.html) |
...
In this page it is assumed that AEM runs on localhost at port `4502` and that the installation context is &quot; `/`&quot; (root). Si ce n’est pas le cas de votre installation, les URI auxquelles les requêtes HTTP s’appliquent doivent être adaptées en conséquence.

Le rendu pris en charge pour les requêtes `GET` est de type JSON. The URLs for `GET` should have the `.json` extension, for example:
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><code>GET</code></td> 
   <td>Obtient la <code>HEAD</code> version du modèle (définition et métadonnées).</td> 
  </tr> 
  <tr> 
   <td><code>PUT</code></td> 
   <td>Met à jour la <code>HEAD</code> version du modèle (crée une nouvelle version).<br /> La définition complète du modèle pour la nouvelle version du modèle doit être ajoutée en tant que paramètre appelé <code>model</code>. Additionally a <code>type</code> parameter is needed as when creating new models and needs to have the value <code>JSON</code>.<br /> </td> 
  </tr> 
  <tr> 
   <td><code>POST</code></td> 
  ...
  <tr> 
   ...
   <td>Supprime le modèle. Afin de résoudre les problèmes de pare-feu/proxy, un <code>POST</code> qui contient une entrée <code>X-HTTP-Method-Override</code> d'en-tête avec une valeur <code>DELETE</code> sera également accepté comme <code>DELETE</code> demande.</td> 
  </tr> 
 </tbody> 
</table>
...
Dans un script JSP (ou un code Java pour une classe servlet), utilisez l’objet de requête HTTP pour obtenir un objet `SlingHttpServletRequest` qui permet d’accéder à un objet `ResourceResolver`. Adaptez l’ `ResourceResolver` objet à `WorkflowSession`.
...
* L’éditeur de modèle de workflow exige que les modèles utilisent une structure de nœud spécifique sous `/var/workflow/models`. The parent node of the model must be of the type `cq:Page` having a `jcr:content` node with the following property values:
...
You [can update the OSGi configuration](/help/sites-deploying/configuring-osgi.md) **Adobe Granite Workflow PayloadMapCache** that specifies the workflow `Models` to be considered as system workflows. Les modèles de workflow (exécution) par défaut sont les suivants :
