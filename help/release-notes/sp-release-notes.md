...
AEM 6.4.8.0 est une mise à jour importante qui comprend de nouvelles fonctionnalités, les principales améliorations et performances demandées par les clients, la stabilité et les améliorations de sécurité, publiées depuis la mise à disposition générale d’AEM 6.4 en **avril 2018.**

Il est également cumulatif, ce qui signifie que la version 6.4.8.0 inclut tous les Service Packs d’AEM 6.4 publiés avant elle.
...
* Ajout de la prise en charge de l&#39;utilisation de la méthode BUNSETSU pour rompre les phrases et les lignes en japonais aux emplacements appropriés.
...
* AEM Assets est maintenant configuré avec Brand Portal par le biais des E/S Adobe.

* Mise à jour de la version jQuery fournie dans ContextHub à 3.4.1.

## Liste des modifications        {#list-of-changes}

### Sites {#sites}

* Lorsqu’une URL de pages de sites AEM contient un signe deux-points ou un symbole de pourcentage, le navigateur sous-jacent cesse de répondre et les cycles de processeur montrent un pic (NPR-32368, NPR-31917).
...
* Ajout de la prise en charge de la division des phrases en japonais à l&#39;aide de la méthode BUNSETSU et de la séparation des lignes à la position appropriée (NPR-31836).
...
* La page des détails des ressources des fichiers PDF n’affiche pas les boutons d’action dans Experience Manager s’exécutant en mode Contenu multimédia dynamique Scene7 (NPR-32316).
...
* Le bouton Enregistrer n’importe pas la visionneuse à distance lorsque l’utilisateur n’a apporté aucune modification à l’éditeur de visionneuses dans le client de média dynamique (NPR-32178).
...
* Le téléchargement des codages vidéo de Contenu multimédia dynamique échoue lorsque la configuration de Scene7 Cloud est placée dans un dossier privé `/conf` au lieu de `/conf/global` (NPR-31779).
...
* Les menus déroulants **Société** et suite de **Rapports** sont masqués une fois que la source **du** Rapports est sélectionnée lors de la configuration d’Adobe Analytics dans les services cloud Experience Manager (NPR-31729).
...
* L’annulation de la publication du schéma de métadonnées dans AEM Assets génère un message d’erreur bien que le schéma soit supprimé sur le serveur principal (CQ-4286871).

### Interface utilisateur de la fondation {#foundations-ui-6480}
...
Il est également cumulatif, ce qui signifie que la version 6.4.7.0 inclut tous les Service Packs d’AEM 6.4 avant sa publication.
...
* Une nouvelle colonne pour la date créée, qui peut être triée, a été ajoutée dans la vue de liste **** DAM et sur les résultats de la recherche de ressources dans la vue de **Liste** (NPR-31311).
* Le tri des ressources basé sur la colonne **Nom** a été autorisé dans la vue de **Liste** .
* La taille du lot et le délai d’expiration du flux de travail pour le retraitement et le transfert par lot sont désormais configurables à partir de l’interface utilisateur dans Contenu multimédia dynamique.
* La valeur false `pdfBrochure` a été définie dans la configuration cloud de Scene7 pour enregistrer de la mémoire dans IPS.

##### Ressources {#assets-6470}

**Améliorations des produits**

* La version d’exportation du package d’API `package com.day.cq.dam.handler.standard.msoffice` prise en charge par `dam-handler` le lot est mise à niveau vers la version 6.0.0 (CQ-4279059).
...
* Les noms des fichiers sont changés en minuscules lorsque vous déplacez des fichiers d’un dossier à un autre dans AEM s’exécutant sur le mode d’exécution Contenu multimédia dynamique Scene7 (NPR-31631).
...
* Le flux de travaux vidéo de codage de média dynamique ne parvient pas à générer la miniature de la vidéo qui est migrée de Scene7 vers le mode d’exécution Contenu multimédia dynamique - Scene 7 (NPR-31523).

* Une erreur de serveur interne est observée lors de l’utilisation du filtre pour rechercher des visionneuses, dans AEM s’exécutant sur Contenu multimédia dynamique - Mode d’exécution Scene7 (NPR-31388).
...
* Après avoir créé une page contenant un composant Contenu multimédia dynamique lors de la publication de la même page, de l’instance d’auteur s’exécutant sur le mode d’exécution Contenu multimédia dynamique - Scene7, l’utilisateur est invité à publier la configuration de Dmscene7 (NPR-30880).

* La valeur du paramètre &quot;asset&quot; dans le code incorporé de la visionneuse reste inchangée après avoir modifié les valeurs des champs Titre après déplacement et Nom après déplacement dans Contenu multimédia dynamique - Scene7 (NPR-30745).
...
* Le changement de nom des ressources dans AEM ne parvient pas à se synchroniser avec la scène 7, lorsqu’un fichier portant un nom de duplicata existe déjà dans Scene7 (CQ-4276763).
...
##### Interface utilisateur de la fondation {#foundation-6470}
...
* L’intégration de Livefyre avec la mise à niveau d’AEM 6.4 donne une exception Null Point, lorsque l’intégration est effectuée à l’aide du module externe DITA pour les ressources synthétiques. L’intégration fonctionne toutefois lorsque des composants sont ajoutés manuellement (FYR-11066).
...
* L’administrateur d’un groupe restreint n’est pas en mesure de vue de la carte de groupe et ne peut pas effectuer d’opérations de lien rapide (groupes Modifier/Publier/Supprimer) dans l’instance d’auteur AEM (NPR-30810).
* Les informations des groupes de membres et des groupes ne sont pas visibles lors de la création d’un nouveau site dans l’instance d’auteur AEM (NPR-28840).
...
* Les cases à cocher et les listes déroulantes dans Forms ne fonctionnent pas lors des vérifications d’accessibilité (NPR-31282).
...
* Les fichiers temporaires créés par Assembler Service ne sont pas automatiquement supprimés et nécessitent un redémarrage d’AEM (NPR-30846).
...
Il est également cumulatif, ce qui signifie que la version 6.4.6.0 inclut tous les Service Packs d’AEM 6.4 avant sa publication.
...
* Ajout de la prise en charge du suivi des états de l’interface utilisateur dynamique dans le événement de suivi dans l’API de base.
...
* Le flux de travaux DAM se bloque lors de la création de ressources MP4 pour AEM. NPR-30031 : correctif pour CQ-4271352
...
* Avertissement ResourceResolver non fermé dans ExperienceFragmentsReplicationListener provoquant des problèmes de stabilité au fil du temps, forçant ainsi à redémarrer les instances AEM. NPR-30416 : correctif pour CQ-4257521
...
* Ajout de la prise en charge du suivi des états de l’interface utilisateur dynamique dans le événement de suivi dans l’API de base. NPR-30742, GRANITE-26322 : Correctif pour GRANITE-26036
...
* Les formulaires qui sont modifiés après la publication ne sont pas republiés lors de la publication du site. NPR-30411 : correctif pour CQ-4236566
...
* L’ajout ou la modification d’une connexion à un service Web en appelant des services Web à partir des outils AEM forms renvoie une erreur : ClassNotFoundException org.apache.axis.message.SOAPBodyElement. NPR-30116 : correctif pour CQ-4273217
...
Il est également cumulatif, ce qui signifie que la version 6.4.5.0 inclut tous les Service Packs d’AEM 6.4 avant sa publication.
...
* Ajout du délai d’expiration du socket et du délai de connexion dans les agents de réplication du portail de marque.
...
* Après avoir ajouté le titre de la ressource dans la page de propriétés, lorsque l’utilisateur tente de fermer la page, AEM rouvre la page de propriétés. NPR-29628 : correctif pour CQ-4264929
...
* La désactivation des autorisations de suppression de contenu côté AEM empêche l’utilisateur de publier sur le portail de marques. Correctif pour CQ-4270426
* Problèmes liés à la limite de Requête avec les rapports sur les ressources après la mise à niveau vers AEM 6.4.3. NPR-28588 : Correctif pour CQ-4262022, CQ-4260697
* La fonctionnalité de téléchargement exploite AEM Assets via la servlet de téléchargement de ressources, ce qui permet aux utilisateurs anonymes de télécharger tous les fichiers. NPR-27315, Correctif pour CQ-4254732
...
* Plusieurs scripts intersites (XSS) stockés et réfléchis dans CRXDE Lite. NPR-29634 : correctif pour GRANITE-26049
...
* La case héritée de la case à cocher apparaissant au niveau racine dans les Configurations de service Cloud doit être supprimée. NPR-27856 : correctif pour CQ-4259676
...
* Erreur de démarrage d’AEM provenant de GemFire : java.lang.IllegalStateException : Une seule connexion AdminDistributedSystem peut être établie en même temps. NPR-29524 : correctif pour CQ-4266295
...
* La conversion PDF/A à partir d’un fichier PDF produit avec Output Service ne passe pas la validation avec Acrobat DC. NPR-29647 : correctif pour CQ-4270448
* Le lot Apache POI échoue avec une exception. NPR-27861, NPR-28048 : correctif pour CQ-4245898, CQ-4244778

**Forms - Designer**

* Ajout de la prise en charge de PDF/UA dans les formulaires XFA (XML Forms Architecture) générés à l’aide de Designer et de Output Service. NPR-23022
...
Il est également cumulatif, ce qui signifie que la version 6.4.4.0 inclut tous les Service Packs d’AEM 6.4 avant sa publication.
...
* Ajout de la prise en charge de la mise en cache de la version du service afin d’éviter les demandes HTTP fréquentes de version de service.
...
* Migration d’ACP et d’intégration de stock vers AEM 6.4.4.0 NPR-27632
...
* Le gestionnaire de PDF prêtes à l’emploi AEM se bloque lors du traitement du PDF avec des caractères étrangers. NPR-28778 : correctif pour CQ-4254234
* Lorsqu’un fichier CSV comporte une valeur séparée par des virgules dans une seule colonne, l’éditeur CSV AEM n’échappe pas la virgule et la traite comme une colonne distincte. NPR-28801 : correctif pour CQ-4261694
...
* Les métadonnées ne sont pas enregistrées dans le fichier téléchargé lors de la première tentative au cas où la fonctionnalité Contenu multimédia dynamique serait activée sur l’instance. NPR-28233 : correctif pour CQ-4260759
...
* Déplacer le dossier du site administrateur se termine en mémoire insuffisante et rend AEM indisponible. NPR-28346 : correctif pour CQ-4261398
...
* Erreur de &quot;valeur de sélecteur de récursion non valide&quot; lors de la migration d’un composant d’AEM 6.0 vers AEM 6.2. NPR-28609 : Correctif pour CQ-4241258
...
* Ajouter rep:cache dans les noeuds ignorables du processus d’écoute de synchronisation des utilisateurs des communautés AEM sur les instances de publication. NPR-27842 : correctif pour CQ-4247234
...
* Ajout d’une logique permettant au événement clavier d’ouvrir la vidéo. NPR-27726 : correctif pour CQ-4254026
...
* Ajout d’un libellé aria pour le bouton de liste et de vue de carte. NPR-27727 : correctif pour CQ-4254027
...
* Les propriétés Enregistrer et Fermer sur la page ne reviennent pas à la page appropriée dans AEM WAR avec Tomcat. NPR-27566 : correctif pour GRANITE-23671
...
* (Configurations du service Cloud) La case &quot;hérité de&quot; qui s’affiche au niveau racine doit être supprimée. NPR-27856 : correctif pour CQ-4259676
...
* Ajout de la prise en charge de l’enregistrement des API de sécurité des documents pour la signature et la certification en tant que transactions.
...
* Envoyer une action à l’aide de problèmes de création classique de processus de formulaires dans la version 6.4. Correctif pour CQ-4252740
...
**Formulaires - Rapports de transaction**
...
Ajout de la prise en charge du remplacement de l’utilisation de la bibliothèque cliente des barres de poignées par un trait de soulignement dans l’assistant de révision des débuts de Forms Manager et l’assistant de déplacement des ressources. NPR-27643 : correctif pour CQ-4246536.
Un lot reste à l’état installé après l’installation du package Forms Management sur la branche release/640. Le correctif logiciel pour CQ-4265410Les formulaires envoyés avec des pièces jointes ne s’affichent pas dans le processus avec l’action d’envoi &quot;Appeler le flux de travail AEM Forms&quot; et activer l’envoi de portail coché. Correctif pour CQ-4263110
...
* Les données du client sont imprimées dans les journaux d’informations à l’aide du flux de travail Invoke FDM Service Forms. Correctif pour CQ-4260385
...
* Fonction activée dans la configuration OSGI **AEM Workflow Remove Feature Flag**. L&#39;action de retrait doit arrêter l&#39;instance de workflow après avoir défini l&#39;indicateur. NPR-26451 : correctif pour CQ-4259090
...
AEM 6.4.3.0 est une mise à jour importante qui comprend des améliorations et correctifs concernant les performances, la stabilité, la sécurité et les clients clés depuis la mise à disposition générale d’AEM 6.4 en avril 2018.

Il est également cumulatif, ce qui signifie que la version 6.4.3.0 inclut tous les Service Packs d’AEM 6.4 avant sa publication.
...
* Ajout de la prise en charge de l’authentification Adobe IMS et de la console d’administration pour les services gérés.
...
* Lorsque l’intégration de Contenu multimédia dynamique est activée, certaines propriétés de métadonnées des Fichiers sont modifiées. Les attributs de largeur, de hauteur et d’emplacement ne s’affichent pas. NPR-27203; Correctif pour CQ-4256258
...
* Les utilisateurs ne peuvent pas modifier les autorisations de groupe d’utilisateurs fermés dans les propriétés de la page. NPR-27138 : Correctif pour CQ-4256089Impossible de modifier les balises par balisage. NPR-26957 : correctif pour CQ-4254858
...
* Lorsqu’un dossier de troisième niveau est publié d’AEM vers le portail de marque, sans publier les dossiers parents, le nom du dossier est modifié. Correctif pour CQ-4255423
* Le prédicat du navigateur de chemins d’accès est publié d’AEM Assets vers Brand Portal comme prévu. Cependant, le chemin publié à BP reste /content/dam, qui doit être mis à jour. Correctif pour CQ-4256240
...
* La recherche par mots-clés dans l’adresse ne fonctionne pas dans la page des composants du calendrier lorsque la communauté AEM est définie pour fonctionner avec DSRP. NPR-26737 : correctif pour CQ-4258493
...
* Impossible d&#39;appeler Adobe Enregistrement : Erreur 414 - URI Long GET observé lorsque les utilisateurs recherchent /content/community-components/en/search.html et sélectionnent le champ d’auteur comme l’un des filtres de ce terme de recherche. NPR-26643 : correctif pour CQ-4251303
...
* Impossible de déplacer un panneau qui a été ajouté à l&#39;aide de l&#39;option Ajouter panneau enfant dans l&#39;arborescence de contenu dans le canal Web de la communication interactive et dans les formulaires adaptatifs. Correctif pour CQ-4253915
...
* Conformité Ajouter FIPS pour le flux de travaux RE dans Digital Signature, Reader Extensions, CryptoProvider et TrustStore. NPR-27495 : correctif pour CQ-4257572
...
* Problèmes de mémoire lors de la mise à niveau d’AEM Forms de 6.1 SP2 CFP5 vers CFP13. NPR-26285 : correctif pour CQ-4244379
...
Il est également cumulatif, ce qui signifie que la version 6.4.2.0 inclut tous les Service Packs d’AEM 6.4 avant sa publication.
...
* Ajout de la prise en charge des fonctionnalités HTML Template Language (HTL) Specification 1.4
...
* Ajout d’une configuration dans chaque composant pour activer/désactiver la fonctionnalité de création/mise à jour de miniatures de dossier.
...
* migrateAllAssets - Ajouter prise en charge du traitement par lots et amélioration de la méthode AEM qui ajoute un UUID aux anciens actifs. NPR-24861 : correctif pour CQ-4242863 et CQ-4247874
...
* (Contenu multimédia dynamique) Mise à jour des visionneuses DM vers la version 5.10.1, qui permet de vérifier les noms de duplicata sur la page Paramètres d’image prédéfinis. Reportez-vous à la section Mise à jour des visionneuses de médias dynamiques (5.10.1). NPR-24403 : correctif pour CQ-4247554
...
* Spécification du langage de modèle HTML version 1.4 - Fonctionnalités renvoyées à AEM 6.4.2.0 NPR-24585
...
* Pour les composants d’analyse imbriqués, la première conception satisfaisante (avec le chemin le moins imbriqué) est toujours appliquée à partir de plusieurs composants disponibles. Pour plus d’informations, voir Résolution [des chemins de](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/page-templates-static.html)conception. NPR-24973 : correctif pour CQ-4246276
...
* (Composant de page) L’ajout d’un composant à Parsys est rogné de la droite et sort de la largeur d’image du périphérique. NPR-25536 : correctif pour CQ-4238224
...
* Erreur de &quot;valeur de sélecteur de récursion non valide&quot; lors de la migration d’un composant d’AEM 6.2.1.13 vers AEM 6.4. NPR-24146
...
* Lors de la création d’un groupe de communauté, un groupe de communauté de duplicata est créé sur un cluster mongoMK actif/actif. NPR-26032 : correctif pour CQ-4245884
...
* Correction du fait que l&#39;importation d&#39;un projet traduit faisait deux requêtes POST simultanées, ce qui provoquait des erreurs. NPR-24889 : correctif pour CQ-4247638
...
* Intégration de fragments d’expérience/Cible fusionnés à AEM 6.4.2.0. Correctif pour CQ-4248653
...
* (6.4.1.0) L’ajout de plusieurs vidéos dans plusieurs paramètres interrompt le lecteur vidéo. Correctif pour CQ-4248517
...
* La génération de rendus de média dynamique a échoué pour le fichier EPS. Correctif pour CQ-4243688
...
* Le redémarrage d’Adobe Granite Workflow Core réexécute les étapes du processus à partir du milieu, créant des workflows inutiles. NPR-25057 : correctif pour Granite-22218
...
* Ajout d’une nouvelle propriété permettant aux files d’attente d’être mises à jour sans actualiser le navigateur.
...
* Ajout de la prise en charge de l’utilisation de la fonctionnalité [des rapports de](https://helpx.adobe.com/experience-manager/6-4/forms/using/transaction-reports-overview.html) transactions dans les déploiements sur site.
...
**Formulaires - Communication interactive et gestion de correspondance**
...
* Impossible d’utiliser le composant AEM Form prête à l’emploi en raison d’une requête inefficace. NPR-25065 : correctif pour CQ-4247256
...
* Action d’envoi à l’aide des problèmes de création classiques de processus des formulaires. Correctif pour CQ-4236981
...
* Optimisez les fichiers PDF en désincorporant sélectivement les polices par le biais d’un appel d’API unique. NPR-23287

**Services de documents**

* Output Service ne fournit pas les balises correctes pour l’accessibilité de Reader. NPR-24438, NPR-24439, NPR-24440, NPR-24441 : Correctif pour CQ-4243849, CQ-4243845, CQ-4243852, CQ-4243853
...
* AEM Forms JEE a été affecté par les débuts d’Apache et l’alerte de sécurité de liaison de données Jackson. NPR-25628 : correctif pour CQ-4242891
...
* Ajout de la prise en charge des métaschémiques conditionnelles dynamiques. For more information, see [Cascading Metadata](/help/assets/cascading-metadata.md). NPR-25189 : correctif pour CQ-4237413
...
* Ajout de la capacité d’annoter des fragments de contenu. For more information, see [Variations-Authoring Fragments](https://helpx.adobe.com/experience-manager/6-4/assets/using/content-fragments-variations.html#AnnotatingaContentFragment). NPR-25188 : correctif pour CQ-4235336
...
* Améliorations de l’éditeur d’images réaffectées à AEM 6.4.2.0. Pour plus d’informations, voir Editeur [d’](https://helpx.adobe.com/fr/experience-manager/6-4/sites/developing/using/image-editor.html)images. NPR-24467
...
AEM 6.4.1.0 est une mise à jour importante qui comprend des améliorations et correctifs concernant les performances, la stabilité, la sécurité et les clients clés depuis la mise à disposition générale d’AEM 6.4 en avril 2018.

AEM 6.4.1.0 peut être installé sur la version GA d’AEM 6.4. Voici les principales caractéristiques du Service Pack 1 :
...
* Ajout de profils de couleur aux paramètres du client sur les serveurs de diffusion.
* Migration des paramètres d’Image Server à partir de /etc —&amp;gt; /conf.
* Ajout du fragment de contenu source pour traduction.
* Ajout de la prise en charge ARIA à Print and PrintDialog.
* Ajout de la prise en charge de la validation ARIA par courrier électronique.
...
* L’extraction des métadonnées XMP par Scene7ListInfoProvider nécessite beaucoup de ressources. NPR-23754
...
* (Coral3 Datepicker) Ajouter prise en charge de typeHint pour enregistrer les valeurs sous la forme &quot;String&quot;. NPR-23398 : correctif pour Granite-21194
...
* (Identité graphique) La boîte de dialogue de souscription mentionne Adobe Marketing Cloud au lieu d’Adobe Experience Cloud. NPR-23210 : correctif pour CQ-4237799
* L’option de filtre rend AEM plus lente après la mise à niveau de 6.3 à 6.4. NPR-23260 : Correctif pour CQ-4239847 (à vérifier)
...
* (hybride DM) Copier l’URL pour les fichiers vidéo ou les visionneuses de supports variés publiés obtient &quot;[objet[]objet&quot; dans la boîte de dialogue URL. Correctif pour CQ-4245236, CQ-4245451
...
* Ajouter les profils de couleur aux paramètres du client sur les serveurs de diffusion. Correctif pour CQ-4241818, CQ-4241819
...
* (Contenu multimédia dynamique) Le téléchargement régulier de fichiers en dehors du dossier smartculture échoue. Correctif pour CQ-4237670
...
* L’étape de flux de travaux de Scene7 lit uniquement Scene7 dans le nom et la description, mais ne précise pas l’étape de flux de travaux dans le flux de travaux de mise à jour de gestion des actifs numériques. Correctif pour CQ-4237865

**DAM - Balises dynamiques**

* Ajout de balises actives améliorées. NPR-21951
...
* Sous `ACTION TAKEN` la section de l&#39;onglet Type de litige, &quot;documents supplémentaires à l&#39;appui de ma réclamation&quot; a un BAT de champ supplémentaire Type enregistré &quot;dedans&quot;. Correctif pour CQ-4239873
...
* Les formulaires envoyés sur un serveur antérieur à la mise à niveau ne sont pas présents sur le serveur mis à niveau. Correctif pour CQ-4241490
...
* La délégation de tâche d’affectation ne fonctionne pas dans l’intégration de formulaires dans les Workflows OSGI. NPR-23709 : correctif pour CQ-4243700
...
* Ajout de la configuration Colspan dans tous les composants du canal Web. Correctif pour CQ-4244946
...
* Ajout de la fonctionnalité Balises dynamiques améliorées. Pour plus d’informations, voir Balises [dynamiques](https://helpx.adobe.com/experience-manager/6-4/assets/using/enhanced-smart-tags.html)améliorées. NPR-21951 : correctif pour CQ-4234883
...
| Intégration des Ressources et d’Adobe Creative Cloud | [Le partage](https://helpx.adobe.com/experience-manager/6-4/sites/administering/using/creative-cloud.html) de dossiers AEM vers Creative Cloud a été introduit dans AEM 6.2 pour permettre aux utilisateurs créatifs d’accéder aux ressources d’AEM. Adobe Asset Link, nouvelle fonctionnalité proposée dans l’application Creative Cloud, offre une expérience utilisateur améliorée et un accès plus puissant aux ressources d’AEM directement à partir de Photoshop, InDesign et Illustrator. Adobe n’apportera pas d’améliorations supplémentaires à la fonctionnalité de partage de dossiers. Bien que la fonction soit incluse dans AEM, les clients sont (il est vivement conseillé d’utiliser le remplacement). | Adobe Asset Link ou application de bureau. Pour plus d’informations, reportez-vous à l’article [Intégration d’AEM Creative Cloud](/help/assets/aem-cc-integration-best-practices.md). | AEM 6.4.4.0 |
...
* Impossible de modifier les composants d’image et les propriétés de page après l’application de la version 6.4.3.0 en plus de la version 6.4.2.0. CQ-4260316 et CQ-4260441Pour résoudre ce problème :
