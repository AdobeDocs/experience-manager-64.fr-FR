...
En même temps que l’outil d’exportation JSON mis à jour, les fragments de contenu structuré peuvent également être utilisés pour livrer du contenu AEM via Content Services à des canaux autres que des pages AEM.
...
1. Vous pouvez :
   ...
   * Open the **[!UICONTROL Assets]** browser and filter for **[!UICONTROL Content Fragments]** (the default is Images). Faites ensuite glisser le fragment en question sur l’instance du composant.
   * Sélectionner le composant de fragment de contenu, puis **[!UICONTROL Configurer]** dans la barre d’outils. Dans la boîte de dialogue, vous pouvez ouvrir la boîte de dialogue de sélection afin de rechercher et de sélectionner le **[!UICONTROL fragment de contenu]** requis.
   >[!NOTE]
   >
...
1. Initially the content from the **[!UICONTROL Main]** Element and **[!UICONTROL Master]** (variation) will be shown. Vous pouvez [sélectionner d’autres éléments et/ou variantes](#selecting-the-element-or-variation) en fonction de vos besoins.
...
* **[!UICONTROL Elément]**
   ...
   * Le type **[!UICONTROL Principal]** par défaut est toujours disponible.
...
* **[!UICONTROL Paragraphes]** : indiquez la plage de paragraphes à inclure :
...
Vous pouvez ouvrir la source du fragment à modifier (la ressource) à l’aide de l’icône **[!UICONTROL Edition]** située dans la barre d’outils du composant. Vous pourrez ainsi [modifier et gérer le fragment de contenu](/help/assets/content-fragments.md). 
...
Cela permet d’ajouter du contenu supplémentaire [à l’intérieur (c’est-à-dire au niveau intermédiaire)](/help/assets/content-fragments.md#in-between-content-when-page-authoring-with-content-fragments) du contenu du fragment (au niveau de tout point disponible), sans avoir à modifier le fragment racine.
...
>[!CAUTION]
>
>Le contenu intermédiaire est un contenu de la page. Il n’est pas stocké dans le fragment de contenu.
...
Si des [ressources (des images, par exemple) ont été insérées dans le fragment proprement dit](/help/assets/content-fragments-variations.md#inserting-assets-into-your-fragment), les options permettant de les modifier dans l’éditeur de page sont limitées.

Par exemple, pour une image, vous pouvez

* Recadrer, faire pivoter ou retourner l’image.
* Ajouter un titre ou un texte de remplacement.
...
Les fragments doivent être publiés pour pouvoir être utilisés dans des pages web publiées :
