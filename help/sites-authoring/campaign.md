...
Voir aussi [Création d’Adobe Campaign Forms dans AEM](/help/sites-authoring/adobe-campaign-forms.md).
...
1. Sélectionner **Créer** then **Créer une page**.
1. Sélectionnez l’un des modèles disponibles spécifiques à Adobe Campaign auquel vous vous connectez, puis cliquez sur **Suivant**. Trois modèles sont disponibles par défaut :
...
Vous pouvez sélectionner le service cloud à partir de l’option **Sites** ou dans l’email/la newsletter après sa création.

Sélection du service cloud à partir de la variable **Sites** est l’approche recommandée. La sélection du service cloud à partir du courrier électronique/de la newsletter nécessite une solution de contournement.
...
1. Vous pouvez visualiser le modèle appliqué à l&#39;email en cliquant sur l&#39;onglet ** Adobe Campaign**. Si vous souhaitez sélectionner un autre modèle, vous pouvez y accéder à partir de l’e-mail en cours de modification.

   Si vous souhaitez appliquer un modèle de diffusion par e-mail spécifique (à partir d’Adobe Campaign), autre que le modèle d’e-mail par défaut, dans **Propriétés**, sélectionnez l’onglet **Adobe Campaign**. Saisissez le nom interne du modèle de diffusion email dans l&#39;instance Adobe Campaign associée.
...
1. Ouvrez l&#39;email et, par défaut, saisissez le mode d&#39;édition.
...
Pour insérer de la personnalisation :
...
1. Insérez du texte en saisissant . Insérez des champs de contexte ou des blocs de personnalisation en cliquant sur les composants Adobe Campaign et en les sélectionnant. Lorsque vous avez terminé, cochez la case.
...
La manière dont vous liez ou synchronisez AEM avec Adobe Campaign dépend de l’utilisation d’Adobe Campaign Standard avec abonnement ou d’Adobe Campaign Classic on-premise.
...
### Liaison d’une page AEM à un email Adobe Campaign (Adobe Campaign Standard) {#linking-an-aem-page-to-an-adobe-campaign-email-adobe-campaign-standard}

Adobe Campaign Standard vous permet de récupérer et de lier le contenu créé dans AEM avec :

* Un email.
* Un modèle d’email.

Cela vous permet de diffuser le contenu. Vous voyez si une newsletter est liée à une seule diffusion par le code qui s’affiche sur la page.
...
1. Sélectionnez le contenu que vous souhaitez utiliser dans votre email.

   Cette liste indique :

   * Libellé du contenu dans AEM.
   * État d’approbation du contenu dans AEM. Si le contenu n&#39;est pas validé, vous pouvez le synchroniser, mais il devra être validé avant l&#39;envoi de la diffusion. Cependant, vous pouvez exécuter certaines opérations, comme l’envoi d’un BAT ou le test d’aperçu.
   ...
   * Tout contenu déjà lié à une diffusion.
   ...
   Lorsque l&#39;email est lié à un contenu AEM, le contenu ne peut pas être édité dans Adobe Campaign.

1. Spécifiez les autres paramètres de votre email depuis son tableau de bord (audiences, planning d&#39;exécution).
1. Exécutez la diffusion email. Lors de l&#39;analyse de la diffusion, la version la plus récente du contenu AEM est récupérée.

   >[!NOTE]
   Si le contenu est mis à jour dans AEM lorsqu&#39;il est lié à un email, il est automatiquement mis à jour dans Adobe Campaign lors de l&#39;analyse. La synchronisation peut également être exécutée manuellement en utilisant l’option **Actualiser le contenu Adobe Experience Manager** dans la barre d’actions de contenu.
   Vous pouvez annuler le lien entre un email et le contenu AEM à l’aide de **Supprimer le lien avec le contenu Adobe Experience Manager** dans la barre d’actions de contenu. Ce bouton n&#39;est disponible que si un contenu est déjà lié à la diffusion. Pour lier un contenu différent à une diffusion, vous devez supprimer le lien du contenu courant avant de pouvoir établir un nouveau lien.
   Lorsque le lien est supprimé, le contenu local est conservé et devient modifiable dans Adobe Campaign. Si vous liez à nouveau le contenu après l’avoir modifié, toutes les modifications seront perdues.

### Synchronisation du contenu créé dans AEM avec une diffusion à partir de Adobe Campaign Classic {#synchronizing-content-created-in-aem-with-a-delivery-from-adobe-campaign-classic}

Adobe Campaign vous permet de récupérer et de synchroniser le contenu créé dans AEM avec :

* Diffusion d’une campagne
* une activité de diffusion dans un workflow de campagne
* Une diffusion récurrente
* Une diffusion continue
* Diffusion Message Center
...
Pour synchroniser le contenu créé dans AEM avec une diffusion depuis Adobe Campaign :
