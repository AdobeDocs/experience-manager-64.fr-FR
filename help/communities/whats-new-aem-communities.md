...
description: AEM Communities offre un cadre de collaboration entre ses partenaires, ses clients et ses employés.
...
# Nouveautés de AEM 6.4 Communautés {#what-s-new-in-aem-communities}
...
Lisez la suite pour découvrir rapidement les nouvelles fonctionnalités et améliorations. Voir aussi l&#39;AEM 6.4 Communautés [notes de mise à jour](../release-notes/communities-release-notes.md). Pour obtenir la documentation sur AEM 6.4 Communautés, consultez le [AEM 6.4 Community User Guide](home.md).
...
Dans le cadre d’un site communautaire, les groupes multilingues peuvent être créés en une seule opération. **[!UICONTROL Le]** champ Langue(s) de groupe(s) communautaire(s) disponible(s) supplémentaire(s) dans le  **[!UICONTROL modèle de groupe]** communautaire, disponible lors de la création d&#39;un  [nouveau ](groups.md) groupe communautaire dans un site communautaire, le rend possible.

![groupe multilingue-1](assets/multilingualgroup-1.png)

Pour créer de tels groupes, les utilisateurs peuvent simplement accéder à la collection de groupes du site de communautés souhaité à partir de la console Sites. Créez un groupe et spécifiez les langues de votre choix dans le champ **[!UICONTROL Langue(s) du groupe de la communauté disponible]** de la page **[!UICONTROL Modèle du groupe de la communauté]**.

### Supprimer des groupes communautaires de la console de groupes {#delete-community-groups-from-groups-console}

AEM 6.4 Communautés fournit l&#39;icône Supprimer le groupe sur les groupes communautaires existants, dans la collection Groupes communautaires de la console Sites communautaires. Ceci active la [suppression de groupe](groups.md#deleting-the-group) en un seul clic, ainsi que la suppression de tous les éléments associés au groupe (tels que le contenu et les abonnements utilisateur).
...
Le contenu d’apprentissage peut désormais être créé, géré et publié pour un ensemble spécifique de membres ciblés de la communauté. En raison de la disponibilité des fonctions de catalogue et d&#39;affectation pour les groupes communautaires (et pas seulement pour l&#39;ensemble du site communautaire), les gestionnaires d&#39;activation peuvent [affecter des ressources d&#39;activation](resource.md) et un chemin d&#39;apprentissage à un petit groupe de personnes également.
...
[Les ](moderate-ugc.md#spam-detection) détecteurs de messages indésirables recherchent et signalent un élément donné de contenu généré par l’utilisateur avec une précision de 90 %. Toutefois, cette fonctionnalité n’est pas activée par défaut. Pour l&#39;activer, les administrateurs de la communauté doivent accéder à configMgr sur le système/la console et ajouter Spam Process.
...
AEM Communities permet de [mettre en signet les filtres de modération prédéfinis](moderation.md#filter-rail) sur la console de modération. Ces signets enregistrés peuvent être réexaminés ultérieurement et partagés avec d’autres utilisateurs.
...
AEM 6.4 Communautés apporte des améliorations à la gestion du site, ce qui permet aux administrateurs du site de créer, gérer et supprimer facilement de nombreux sites communautaires dans une langue différente.

### Créez des sites de communauté à plusieurs paramètres régionaux en une seule étape {#create-multi-locale-community-sites-in-one-step}

AEM Communities permet de créer en une seule opération [des sites communautaires multilingues](create-site.md). Cela est possible en raison de la disponibilité de plusieurs langues à sélectionner dans le champ **[!UICONTROL Langue de base du site communautaire]** de la page **[!UICONTROL Modèle du site]**, tout en créant un nouveau site communautaire à partir de la console sites.
...
### Supprimer les sites de la communauté de la console de sites {#delete-community-sites-from-sites-console}

AEM 6.4 Communautés fournit l&#39;icône Supprimer le site sur les sites communautaires existants, dans la console Sites communautaires. Cela permet la [suppression du site](create-site.md) et des éléments associés en un seul clic.

![siteactions](assets/siteactions.png)

## Gestion du CDU et des profils utilisateur {#managing-ugc-and-user-profiles}

En maintenant la protection des données utilisateur au coeur de l’expérience des communautés, AEM Communities expose [les API prêtes à l’emploi](user-ugc-management-service.md) et [l’échantillon de servlet](https://github.com/Adobe-Marketing-Cloud/aem-communities-ugc-migration/tree/main/bundles/communities-ugc-management-servlet). Ces API permettent de gérer en bloc (suppression en masse et exportation en bloc) le contenu généré par les utilisateurs et de supprimer les profils d’utilisateurs. Elles sont également essentielles pour gérer les demandes de conformité aux RGPD de l’UE.
...
* Le passage en mode vrac et la sortie du mode en vrac ont été supprimés de l’interface de modération en bloc. Pour passer en mode de sélection multiple, cliquez sur l’icône Sélectionner ( ![sélection](assets/selecticon.png)) sur une publication, qui s’affiche lorsque vous pointez dessus avec la souris (bureau) ou lorsque vous appuyez et maintenez un doigt sur la publication (mobile).
