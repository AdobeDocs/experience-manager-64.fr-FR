...
* Dans l’environnement de publication :
   ...
   * Permettre aux visiteurs du site de trier en fonction des critères &quot;le plus vu&quot;, &quot;le plus actif&quot;, &quot;le plus aimé&quot;
...
En vous connectant à [l’Adobe Marketing Cloud](https://marketing.adobe.com/resources/help/en_US/analytics/getting-started/analytics-navigation.html) avec le nom de [Société et le nom d’utilisateur](analytics.md#prerequisites), il est possible de configurer une suite de rapports nouvelle ou existante pour qu’elle dispose des éléments suivants :
...
Pour ajouter le service cloud Analytics lors de la [création d’un nouveau site](sites-console.md)communautaire :
...
* Chemin SRP vers UGC : /content/usergenerate/asi/.../forum/jmtz-topic3 ou *chemin du composant à suivre*: /content/sites/communautés/fr/jcr:content/content/primary/forum
...
Le mappage des eVars et événements Analytics avec les variables AEM est visible à partir de la configuration de la structure après l’activation d’Analytics pour un site communautaire.
...
Si les eVars et les événements des communautés remplacent les mappages qui existaient avant dans la suite de rapports, pour éviter la perte de données, affectez les variables AEM pour les fonctionnalités des communautés à d’autres eVars et/ou événements Analytics et restaurez les mappages d’origine.
...
Lorsque le déploiement choisi est une batterie [de](topologies.md#tarmk-publish-farm)publication, une instance de publication AEM doit être identifiée comme l’éditeur principal pour interroger Adobe pour que les données de rapport soient écrites à [SRP](working-with-srp.md).
...
Par conséquent, il est nécessaire de modifier la configuration sur toutes les instances de publication secondaires pour désactiver la case à cocher **Principal Publisher** .
...
Une fois que le service Analytics Cloud a été activé pour un site communautaire et que, si nécessaire, le [mappage de Analytics aux variables AEM a été ajusté](#mapped-analytics-to-aem-variables), il est nécessaire de répliquer la configuration sur l’environnement de publication en [(re)publiant le site](sites-console.md#publishing-the-site)communautaire.
...
* Accédez au composant configuré pour Analytics rapports
