...
Pour plus d’informations sur les éléments essentiels de développement liés à chaque composant, voir [Fonctionnalités et composants essentiels](essentials.md).
...
>[!NOTE]
>
>Sachez que si la valeur par défaut de la SRP est définie sur [JSRP](jsrp.md), alors le contenu généré par l’utilisateur entré sur l’instance de publication sera visible uniquement lors de la publication et ne sera *pas visible depuis la [modération](moderate-ugc.md) sur l’instance d’auteur.
...
Pour plus d’informations, voir [Clientlibs des composants Communities](clientlibs.md).
...
Pour tester rapidement les personnalisations, la méthode `scg:showIde`doit être ajoutée au noeud de contenu JCR de la page de composant et définie sur true.
