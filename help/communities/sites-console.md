...
Voir [Prise en main d’AEM Communities](getting-started.md) pour savoir à quelle vitesse un site communautaire peut être créé dans l’environnement de création, ainsi que comment créer des groupes communautaires à partir des environnements de création et de publication.

>[!NOTE]
>
>Les principaux menus Communautés pour la création de [sites de communauté](sites-console.md), [modèles de site de communauté](sites.md), [modèles de groupe de communauté](tools-groups.md) et [fonctions de communauté](functions.md) ne sont utilisables que dans l’environnement de création.
...
* Activer le [service tunnel](deploy-communities.md#tunnel-service-on-author) pour gérer les membres et les groupes de membres
* Identifiez l’[Principal éditeur](deploy-communities.md#primary-publisher)
* [Configurez la ](deploy-communities.md#replication-agents-on-author) réplication lorsque le port Principal de l’éditeur n’est pas le port par défaut (4503).
...
* Identifier les [administrateurs de la communauté](users.md#creating-community-members)
...
Par exemple, l’image suivante montre la console Sites des communautés principale affichant les dossiers de deux sites de communauté : [activer](getting-started-enablement.md) et [engager](getting-started.md) :

![chlimage_1-448](assets/chlimage_1-448.png)

## Création du site {#site-creation}

La console de création de site fournit une approche détaillée pour assembler les fonctionnalités du site en fonction d’un [modèle de site communautaire](sites.md) et de paramètres sélectionnés.
...
* **[!UICONTROL Nom du site de la communauté]** : Nom de la page racine du site qui apparaît dans l’URL
   ...
   * L’URL de base ( `https://*server:port/site root/site name*)`) s’affiche sous la balise `Community Site Name`
   * Pour une URL valide, ajoutez un code de langue de base + &quot;.html&quot;

      *Par exemple*, `http://localhost:4502/content/sites/mysight/en.html`

* **[!UICONTROL Modèle de site communautaire]** : Utilisez le menu déroulant pour choisir un modèle [ de site de ](tools.md)communauté disponible.
...
>
>La convention est que les [utilisateurs et les groupes d’utilisateurs ](users.md) (membres et groupes de membres) côté publication ne soient pas dupliqués dans l’environnement de création.
>
...
>
>Pour ce faire, activez la balise ` [AEM Communities Publish Tunnel Service](deploy-communities.md#tunnel-service-on-author)`pour l’environnement de création.
...
>[!NOTE]
>
>Il est recommandé que les [sites de communauté d’activation](overview.md#enablement-community) soient privés (contactez votre gestionnaire de compte pour plus d’informations).
>
...
>
>Bien que des exemples de configurations Facebook et Twitter puissent exister et être sélectionnables, pour un [environnement de production](../../help/sites-administering/production-ready.md), il est nécessaire de créer des applications Facebook et Twitter personnalisées. Voir [Connexion sociale avec Facebook et Twitter](social-login.md).
...
Les [rôles des membres de la communauté](users.md) sont attribués avec ces paramètres.
...
* **[!UICONTROL Membres privilégiés de la communauté]**
   ...
   Commencez à saisir le texte pour sélectionner un ou plusieurs membres de la communauté ou groupes de membres à qui il sera possible de créer du contenu lorsque `Allow Privileged Member` a été sélectionné pour une [fonction de la communauté](functions.md).
...
* **[!UICONTROL Le contenu est prémodéré]**
   ...
   Si cette case est cochée, le contenu de la communauté publié n’apparaîtra pas tant qu’il n’aura pas été approuvé par un modérateur. Cette option n’est pas cochée par défaut. Pour plus d’informations, voir [Modération du contenu de la communauté](moderate-ugc.md#premoderation).
...
* **[!UICONTROL Enable Analytics (Activer Adobe Analytics)]**
   ...
   Disponible uniquement lorsque Adobe Analytics a été [configuré](analytics.md) pour les fonctionnalités Communities.
...
* **[!UICONTROL Référence de la structure de configuration du cloud]**
   ...
   `Communities`est l’exemple de structure de la documentation Configuration d’ [Analytics pour les ](analytics.md#aem-analytics-framework-configuration) fonctionnalités de communautés .
...
* **[!UICONTROL Sélectionner le fournisseur de traduction]**
   ...
   Par défaut, le fournisseur est un service d’évaluation utilisant `microsoft`à des fins de démonstration uniquement. Si aucun fournisseur de services de traduction n’est sous licence, la case **Autoriser la traduction automatique** doit être décochée.
...
Les paramètres `ENABLEMENT`s’appliquent lorsque le modèle de site de communauté choisi inclut la fonction [Affectations](functions.md#assignments-function), disponible lorsque les fonctionnalités d’activation sont sous licence et [configurées](enablement.md). Le modèle de site de référence qui inclut la fonction Affectations est `Reference Structured Learning Site Template.`

* **[!UICONTROL Responsables d’activation]**

   (obligatoire) Seuls les membres du groupe `Community Enablementmanagers` peuvent être sélectionnés pour gérer cette communauté d’activation. Les gestionnaires d’activation sont chargés d’affecter des membres aux ressources. Voir aussi [Gestion des utilisateurs et des groupes d’utilisateurs](users.md).

* **[!UICONTROL ID d’entreprise Marketing Cloud]**

   (Facultatif) Identifiant d’une licence [Video Heartbeat Analytics](analytics.md#video-heartbeat-analytics).
...
* Glissez-déposez des [fonctions de communauté](functions.md) supplémentaires dans la structure du site.
* Sur une instance d’une fonction de communauté dans la structure du site :

   * **`gear icon`**

      modifier les paramètres, y compris le titre d’affichage et l’URL name&amp;ast;
...
Après la publication d’un site de communauté, il est nécessaire de publier individuellement chaque sous-communauté (groupe imbriqué) créée à l’aide de la [console Groupes](groups.md).
