...
Pour inclure la connexion sociale, il est *requis *pour créer des applications Facebook et Twitter personnalisées.
...
1. Dans la navigation globale, sélectionnez **[!UICONTROL Outils > Cloud Services > Configuration de la connexion Facebook Social]**.
...
1. Accédez à **[!UICONTROL Outils > Général > Navigateur de configuration]**. Sélectionnez votre contexte et modifiez les propriétés. Activez les Configurations de cloud si elles ne sont pas encore activées.
...
1. Sélectionnez l&#39;icône représentant un crayon à ouvrir pour modification.
   ...
   * **[!UICONTROL Mettre à jour]**
l&#39;utilisateurSi cette option est cochée, actualise les données utilisateur dans le référentiel à chaque connexion afin de refléter les modifications de profil ou les données supplémentaires demandées. La valeur par défaut est désélectionnée.
...
1. Pour *site Web*, saisissez https://&lt;serveur>/.
1. Pour *URL de rappel*, saisissez https://&lt;serveur>/.
...
1. Dans la navigation globale, sélectionnez **[!UICONTROL Outils > Cloud Services > Configuration de la connexion Twitter Social]**.
...
1. Créez/modifiez la configuration du service cloud Twitter.
   ...
   * **[!UICONTROL consumer key]**  (*Obligatoire*) Saisissez le  **clavier de** consommateur (API) pour l’application Twitter. Ceci identifie l&#39;instance [Adobe Granite OAuth Application et Provider](https://helpx.adobe.com/experience-manager/6-3/communities/using/social-login.html#AdobeGraniteOAuthApplicationandProvider) créée à partir de la boîte de dialogue.

   * **[!UICONTROL secret du client]**  (*Obligatoire*) Saisissez le  ***secrétariat*** Consumer(API) pour l’application Twitter.

   * **[!UICONTROL Créer des]** utilisateursSi cette case est cochée, la connexion à l’aide d’un compte Twitter créera une AEM entrée utilisateur et l’ajoutera en tant que membre au ou aux groupes d’utilisateurs sélectionnés. La valeur par défaut est cochée (fortement recommandé).

   * **[!UICONTROL Masquer les]** ID utilisateurNe pas sélectionner.
...
* Vérifier que le [gestionnaire d’authentification OAuth Granite Adobe](#adobe-granite-oauth-authentication-handler) a été activé sur toutes les instances de publication
* Vérifier que les services cloud ont été publiés
* Vérifier que le site communautaire a été publié
* Lancement du site publié dans un navigateur
...
* Sélectionner pour ouvrir la configuration à modifier
...
1. Localisez l’application et le fournisseur OAuth Granite Adobe.
   ...
   * Localisez l’instance où **[!UICONTROL ID client]** correspond à l’**[!UICONTROL ID d’application]**.
...
**&amp;ast;** Pour le  **** nom *authid-* 123 *, remplacez* 123 ***par l’ID de l’application Facebook ou l’ID de utilisateur Twitter (API) Keykey qui est la valeur de l’ID de AdobeClientIDdans l’application OAuth Granite de l’ et la configuration du fournisseur de .*************[](social-login.md#adobe-granite-oauth-application-and-provider)
