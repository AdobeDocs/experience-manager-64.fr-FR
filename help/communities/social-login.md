...
UserSi cette option est cochée, les données utilisateur sont actualisées dans le référentiel à chaque connexion afin de refléter les modifications de profil ou les données supplémentaires demandées. La valeur par défaut est désélectionnée.
...
1. Créez/modifiez la configuration du service cloud Twitter.
   ...
   * **[!UICONTROL Titre]**  (*Obligatoire*) Saisissez un titre d’affichage qui identifie l’application Twitter. Il est recommandé d’utiliser le même nom que celui saisi sous *Nom d’affichage* pour l’application Twitter.

   * **[!UICONTROL Clé du client]**  (*requise*) Saisissez la  **clé** du client (API) pour l’application Twitter. Cela identifie l’instance [Adobe de l’application OAuth Granite et de fournisseur](https://helpx.adobe.com/experience-manager/6-3/communities/using/social-login.html#AdobeGraniteOAuthApplicationandProvider) créée à partir de la boîte de dialogue.

   * **[!UICONTROL Secret du client]**  (*obligatoire*) Saisissez le  ***secret*** du client (API) pour l’application Twitter.

   * **[!UICONTROL Créer des]** utilisateurs Si cette option est cochée, la connexion à l’aide d’un compte Twitter crée une entrée d’utilisateur AEM et l’ajoute en tant que membre au ou aux groupes d’utilisateurs sélectionnés. La valeur par défaut est cochée (vivement recommandé).

   * **[!UICONTROL Masquer les]** identifiants d’utilisateurLaissez désélectionné.
...
**&amp;ast;** Pour le  **** nom *123*, remplacez  *123*  par l’ ***ID de l’application Facebook***  ou le  ***Clé du consommateur (API) Twitter***  qui est la valeur de l’ **ID du client** dans la  [configuration de l’application OAuth Granite Adobe et du ](social-login.md#adobe-granite-oauth-application-and-provider)fournisseur.
