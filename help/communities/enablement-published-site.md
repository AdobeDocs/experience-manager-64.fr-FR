...
Il y a une activité des deux apprenants, `Riley Taylor` et `Sidney Croft. By s`en choisissant le `Administration`lien pour accéder à la console de modération, Quinn peut utiliser la [console de modération en bloc](moderation.md) pour modérer leurs publications.
...
Accédez à la console **Communautés [Rapports](reports.md)** et générez des rapports selon les
...
* Accédez au menu principal **[!UICONTROL Communautés > Ressources]**.
