...
Il y a, c’est l’activité des deux apprenants `Riley Taylor` et la `Sidney Croft. By s`sélection du `Administration`lien pour accéder à la console de modération, Quinn peut utiliser la console [de modération](moderation.md) en masse pour modérer leurs publications.
