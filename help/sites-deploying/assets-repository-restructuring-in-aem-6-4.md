...
seo-title: Restructuration des référentiels d’Assets dans AEM 6.4
...
seo-description: Découvrez comment apporter les modifications nécessaires pour migrer vers la nouvelle structure de référentiel dans AEM 6.4 pour Assets.
...
Comme décrit sur la page parent [Restructuration du référentiel dans AEM 6.4](/help/sites-deploying/repository-restructuring.md), les clients qui effectuent la mise à niveau vers AEM 6.4 doivent utiliser cette page pour évaluer l&#39;effort de travail associé aux modifications du référentiel qui affectent la solution AEM Assets. Certaines modifications demandent du travail lors du processus de mise à niveau vers AEM 6.4, tandis que d’autres peuvent être différées jusqu’à une mise à niveau vers la version 6.5.
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><strong>Remarques</strong></td> 
   <td>Bien que <code>/conf/global/settings/dam/adhocassetshare</code> soit techniquement pris en charge pour la recherche (elle prévaut avant <code>/apps</code> par le biais de la recherche habituelle de données CAConfig Sling, mais après <code>/etc</code>), le modèle peut être placé dans <code>/conf/global/settings/dam/adhocassetshare</code>. Cependant, cela n’est pas recommandé car il n’y a pas d’IU d’exécution pour faciliter la modification du modèle d’e-mail.</td> 
  </tr> 
 </tbody> 
</table>
   ...
   <td><p>Pour s’aligner sur la nouvelle structure de référentiel :</p> 
    <ol> 
     <li>Copier tous les scripts personnalisés ou modifiés de <strong><code>/etc/dam/indesign/scripts</code></strong> vers <strong><code>/apps/settings/dam/indesign/scripts</code></strong><br /> 
      <ol> 
       <li>Seules les copies de scripts nouveaux ou modifiés en tant que scripts non modifiés fournis par AEM seront disponibles via <strong><code>/libs/settings</code></strong> dans AEM 6.4.</li> 
      </ol> </li> 
