...
* Utilisation de XPath (recommandé)
* Utilisation de SQL2
...
>[!NOTE]
>
>En adoptant l’approche de recherche Solr intégrée, il est possible de décharger l’indexation sur un serveur Solr. Si les fonctionnalités plus avancées du serveur Solr sont utilisées selon une approche de robot d’exploration, un travail de configuration supplémentaire est nécessaire. Headwire a créé une [connecteur open source](https://www.aemsolrsearch.com/#/) pour accélérer ces types d’implémentations.
...
* S’applique pour/si :
   ...
   * Toutes les versions Oak
   * Uniquement [index de propriété](https://jackrabbit.apache.org/oak/docs/query/property-index.html)

* Symptômes :

   * Noeuds existants avant la mise à jour de la définition de l’index de propriété manquants dans les résultats
...
* Mode de résolution :
   ...
   * Vous pouvez également toucher (effectuer une opération d’écriture bénigne) aux noeuds manquants.
      ...
      * Nécessite que le jeu de noeuds manquants soit connu
      * Nécessite de modifier toute propriété sur le noeud
...
* Symptômes :
   ...
   * Les résultats de requête ne reflètent pas le comportement attendu de la définition d’index
   * Le plan de requête ne signale pas la sortie attendue en fonction de la définition d’index
...
>[!NOTE]
>
>Dans AEM 6.4, la méthode [oak-run.jar constitue la SEULE méthode prise en charge](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) pour effectuer une réindexation sur des référentiels MongoMK ou RDBMK.

#### Réindexation des index de propriété {#re-indexing-property-indexes}

* Utilisation [oak-run.jar](/help/sites-deploying/oak-run-indexing-usecases.md#usecase3reindexing) pour réindexer l’index de propriété
...
* Utilisation [oak-run.jar à réindexer](/help/sites-deploying/oak-run-indexing-usecases.md#usecase3reindexing) Index de la propriété Lucene.
