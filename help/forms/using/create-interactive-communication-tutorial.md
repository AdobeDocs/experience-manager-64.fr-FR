...
1. Appuyer **Créer** et sélectionnez **Communication interactive**. Le **Créer une communication interactive** s’affiche.
1. Spécifiez **create_first_ic** dans les champs **Titre** et **Nom**. Sélectionner **FDM_Create_First_IC** comme modèle de données de formulaire et appuyez sur **Suivant**.
1. Dans le **Canaux** assistant :

   1. Spécifier **create_first_ic_print_template** comme modèle d’impression et appuyez sur **Sélectionner**. Assurez-vous que la variable **Utilisation de l’impression comme Principal pour le canal web** n’est pas sélectionnée.
   1. Spécifier **Create_First_IC_templates** folder > **Create_First_IC_Web_Template** comme modèle web et appuyez sur **Sélectionner**.
   1. Appuyez sur **Créer**.
...
1. Accédez au **Ressources** et appliquez le filtre pour afficher uniquement les fragments de document dans le volet de gauche.
...
1. Appuyez sur le composant Graphique et sélectionnez ![configure_icon](assets/configure_icon.png) (Configuration). Les propriétés du graphique s’affichent dans le volet gauche :
   ...
   1. Sélectionnez la **calltype** de la propriété **calls** type d’objet de modèle de données dans la propriété **Axe Y** . Appuyer ![done_icon](assets/done_icon.png).
   1. Appuyer ![done_icon](assets/done_icon.png) pour enregistrer les propriétés du graphique.

1. Accédez au **Ressources** et appliquez le filtre pour afficher uniquement les fragments de mise en page dans le volet de gauche. Faites glisser le fragment de mise en page **table_lf** dans la zone cible **Appels détaillés**.
1. Sélectionnez le champ de texte dans le **Date** colonne et appuyez sur ![configure_icon](assets/configure_icon.png) (Configuration).
1. Sélectionnez **Objet du modèle de données** dans la liste déroulante **Type de liaison** et sélectionnez **calls** > **calldate**. Appuyer ![done_icon](assets/done_icon.png) deux fois pour enregistrer les propriétés.
...
1. Appuyer **PayNow** zone cible, puis appuyez sur **+** pour ajouter une **Image** composant.
1. Appuyez sur le composant Image et sélectionnez ![configure_icon](assets/configure_icon.png) (Configuration). Les propriétés de l’image s’affichent dans le volet gauche :
   ...
   1. Appuyer ![done_icon](assets/done_icon.png) pour enregistrer les propriétés de l’image.

1. Répétez les étapes 13 et 14 pour ajouter **ValueAddedServices** de l’image **ValueAddedServices** zone cible.
...
1. Appuyez sur **Modifier** pour ouvrir la communication interactive dans le volet droit.
1. Appuyez sur le bouton **Canaux** dans le volet de gauche, puis appuyez sur **Web**.
1. Accédez au **Ressources** et appliquez le filtre pour afficher uniquement les fragments de document dans le volet de gauche.
1. Faites glisser les fragments de documents suivants vers leurs zones cibles dans la communication interactive :
...
1. Appuyer **Récapitulatif des frais** zone cible, puis appuyez sur **+** pour ajouter une **Graphique** composant.
1. Appuyez sur le composant Graphique et sélectionnez ![configure_icon](assets/configure_icon.png) (Configuration). Les propriétés du graphique s’affichent dans le volet gauche :
...
1. Sélectionnez l’onglet **Sources de données** dans le volet gauche et faites glisser l’objet de modèle de données **calls** vers la zone cible **Appels détaillés**. Toutes les propriétés dans la variable **calls** l’objet de modèle de données est affiché sous forme de colonnes de tableau dans la variable **Appels détaillés** zone cible dans le volet de droite.
...
1. Sélectionner **Mobilenum** en-tête de colonne du tableau et sélectionnez **Autres options** > **Supprimer la colonne**. De même, supprimez la colonne **calltype**.
1. Sélectionnez la **Calldate** en-tête de colonne du tableau et appuyez sur ![edit](assets/edit.png) (Modifier) pour renommer le texte en **Date de l’appel**. De même, renommez les autres en-têtes des colonnes du tableau.
1. Selon le cas d’utilisation, insérez un bouton **Payer maintenant** dans la communication interactive qui permet à l’utilisateur d’effectuer le paiement en cliquant sur le bouton. Effectuez les étapes suivantes pour insérer le bouton :
   ...
   1. Appuyer **Payer maintenant** zone cible, puis appuyez sur **+** pour ajouter une **Texte** composant.
   1. Appuyez sur le composant de texte, puis sur ![edit](assets/edit.png) (Modifier).
   ...
   1. Appuyer ![done_icon](assets/done_icon.png) pour enregistrer les propriétés de l’hyperlien.
...
1. Personnalisez le texte de l’hyperlien pour l’afficher en tant que bouton dans la communication interactive en suivant les étapes suivantes :
   ...
   1. Appuyez sur le composant de texte et sélectionnez ![edit](assets/edit.png) (Modifier).
...
1. Appuyer **Payer maintenant** zone cible, puis appuyez sur **+** pour ajouter une **Image** composant.
1. Appuyez sur le composant Image et sélectionnez ![configure_icon](assets/configure_icon.png) (Configuration). Les propriétés de l’image s’affichent dans le volet gauche :

   1. Spécifiez **PayNow** comme nom de l’image dans le champ **Nom**.
   1. Appuyer **Télécharger**, sélectionnez la variable **PayNowWeb** image enregistrée sur le système de fichiers local, puis appuyez sur **Ouvrir**.
   1. Appuyer ![done_icon](assets/done_icon.png) pour enregistrer les propriétés de l’image.

1. En fonction du cas d’utilisation, insérez une **S’abonner** dans la communication interactive qui permet à l’utilisateur de s’abonner aux services à valeur ajoutée en cliquant sur le bouton .

   Répétez les étapes 13 à 17 pour ajouter une **S’abonner** au bouton **Services à valeur ajoutée** et ajoutez la zone cible **ValueAddedServicesWeb** image.
...
1. Dans le **Canaux** assistant :
   ...
   1. Spécifier **create_first_ic_print_template** comme modèle d’impression et appuyez sur **Sélectionner**.
   1. Sélectionnez la **Utilisation de l’impression comme Principal pour le canal web** .
   1. Spécifier **Create_First_IC_templates** folder > **Create_First_IC_Web_Template** comme modèle web et appuyez sur **Sélectionner**.
   1. Appuyez sur **Créer**.
...
1. Appuyez sur **Modifier** pour ouvrir la communication interactive dans le volet droit.
1. Exécutez les étapes 6 à 15 de [Créer une communication interactive pour le canal d’impression](#create-interactive-communication-for-print-channel) .
...
1. Comme la variable **Utilisation de l’impression comme Principal pour le canal web** est cochée à l’étape 4, le contenu et les liaisons sont générés automatiquement pour le canal web à partir du canal d’impression.
   ...
   Passez la souris sur la zone cible appropriée du canal web et sélectionnez ![annulation inheritance](assets/cancelinheritance.png) (Annuler l’héritage), puis dans le **Annuler l’héritage** dialog, appuyez sur **Oui**.

   ![cancel_inheritance_web_channel](assets/cancel_inheritance_web_channel.png)

   Si vous avez annulé l’héritage d’un composant, vous pouvez le réactiver. Pour réactiver l’héritage, passez la souris sur la limite de la zone cible appropriée, qui inclut le composant, puis appuyez sur ![réactiver l’héritage](assets/reenableinheritance.png).
