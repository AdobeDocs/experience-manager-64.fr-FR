...
description: Les workflows d’AEM centrés sur Forms vous permettent d’automatiser des processus d’entreprise basés sur l’utilisation de Forms. Explorez plus en détail les données utilisateur et les entrepôts de données. Découvrez comment accéder aux données utilisateur et les supprimer.
...
>[!NOTE]
>
>L’application AEM Forms stocke également les données en mode hors ligne. Il est possible que les données d’une instance de workflow soient stockées localement sur des appareils individuels et envoyées au serveur Forms lorsque l’application se synchronise avec le serveur.
...
1. Identifiez et supprimez les données hors ligne de brouillon et d’envoi de la boîte d’envoi de l’application AEM Forms des participants au workflow afin d’éviter tout envoi au serveur.
