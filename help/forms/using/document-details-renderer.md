...
Dans l’espace de travail AEM Forms, la vue PDFTaskForm communique avec le `FormBridge`javascript, au moyen d’un code HTML intermédiaire présent à `/lc/libs/ws/libs/ws/pdf.html`. Le flux est :
...
La communication se fait en utilisant `swfObject.postMessage` / `window.flexMessageHandler`.
