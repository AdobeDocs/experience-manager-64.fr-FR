...
* Utilisation de l’étape d’affectation des tâches et des étapes des services de documents des processus AEM
* Utilisation d’un formulaire adaptatif dans un formulaire adaptatif

Les API de facturation ne tiennent pas compte du nombre de pages, de la longueur d’un document ou d’un formulaire, ni du format final du document rendu. Un rapport de transaction divise les transactions en trois catégories : Documents traités, documents rendus et formulaires envoyés.

* **** Formulaires envoyés : Lorsque des données sont envoyées à partir de n’importe quel type de formulaire créé avec AEM Forms et que les données sont envoyées à n’importe quel référentiel de stockage de données ou base de données, elles sont considérées comme des envois de formulaire. Par exemple, l’envoi d’un formulaire adaptatif, d’un formulaire HTML5, de formulaires PDF et d’un jeu de formulaires est comptabilisé comme des formulaires envoyés. Chaque formulaire d’un jeu de formulaires est considéré comme un envoi. Si, par exemple, un jeu de formulaires comporte 5 formulaires, le service de création de rapports de transactions comptabilise 5 envois lors de l’envoi du jeu de formulaires.
* **** Documents rendus : La génération d’un document en combinant un modèle et des données, la signature numérique ou la certification d’un document, l’utilisation d’une API Document Services facturable pour Document Services ou la conversion d’un document d’un format à un autre sont comptabilisées comme des documents rendus.

>[!NOTE]
>
>L’interface utilisateur des rapports de transaction affiche trois catégories : Formulaires envoyés, documents rendus et documents traités. Les documents rendus et les documents traités sont comptabilisés comme documents rendus.

## API Billable Document Services {#billable-document-services-apis}
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>permet de convertir des documents XDP et PDF aux formats de fichier PostScript (PS), PCL (Printer Command Language) et ZPL. </td> 
   <td>Documents traités</td> 
   <td> </td> 
  </tr>
  <tr>
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>permet de convertir un document PDF en liste de documents image. Les formats d’image pris en charge sont JPEG, JPEG2K, PNG et TIFF.</td> 
   <td>Documents traités</td> 
   <td> </td> 
  </tr>
  <tr>
   ...
   <td>Convertit un fichier PDF aplati au format PostScript à l’aide des options spécifiées dans la spécification d’option.</td> 
   <td>Documents traités</td> 
   <td> </td> 
  </tr>
 </tbody>
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>Exécute le document DDX spécifié et renvoie un objet <a href="https://helpx.adobe.com/experience-manager/6-3/forms/javadocs/com/adobe/fd/assembler/client/AssemblerResult.html">AssemblerResult</a> contenant les documents cible. </td> 
   <td>Documents traités</td> 
   <td>Les opérations suivantes ne sont pas comptabilisées comme des transactions :
    <ul> 
     <li>Création de packs ou de portefeuilles</li> 
     <li>Rognage de plusieurs fichiers XDP </li> 
    </ul> </td> 
  </tr>
  <tr>
  ...
  <tr>
   ...
   <td>Convertissez un document spécifié au format PDF/A à l’aide des options spécifiées.</td> 
   <td>Documents traités</td> 
   <td> </td> 
  </tr>
 </tbody>
...
>[!NOTE]
>
>* L’API d’appel du service Assembler peut, en interne, appeler une API facturable d’un autre service selon l’entrée. Ainsi, l’API d’appel peut être comptabilisée comme aucune, une ou plusieurs transactions. Le nombre de transactions comptabilisées dépend de l’entrée et des API internes invoquées.
...
Tous les événements d’envoi des formulaires adaptatifs, des formulaires HTML5 et des jeux de formulaires sont comptabilisés comme des transactions. Par défaut, l’envoi d’un formulaire PDF n’est pas comptabilisé comme une transaction. Utilisez l’API [des rapports de](https://www.bdnsw.gov.bn/PublishingImages/page-under-construction.jpg) transaction fournie pour enregistrer un envoi de formulaires PDF en tant que transaction.
...
<table> 
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ul> 
     <li>Les envois réussis sont comptabilisés pour une ou deux transactions. Le nombre de transactions comptabilisées dépend du type d’action d’envoi utilisée pour l’envoi. Par exemple, l’envoi d’un fichier PDF par courrier électronique génère des comptes d’action d’envoi pour deux comptes de transactions. Une transaction pour l’envoi du formulaire et une autre pour le PDF généré à l’aide du service de document d’enregistrement (DOR). </li> 
     <li>L’utilisation du formulaire adaptatif dans un formulaire adaptatif (jeu de formulaires adaptatifs) ne tient compte que d’une seule transaction. Un formulaire adaptatif peut comporter n’importe quel nombre de formulaires adaptatifs.</li> 
    </ul> </td> 
  </tr>
 </tbody>
</table>
 ...
 <tbody>
  ...
  <tr>
   ...
   <td>
    <ul> 
     <li>L’utilisation du formulaire adaptatif dans un formulaire adaptatif (jeu de formulaires adaptatifs) ne tient compte que d’une seule transaction. Un formulaire adaptatif peut comporter n’importe quel nombre de formulaires adaptatifs.</li> 
     <li>Chaque formulaire d’un jeu de formulaires HTML5 prend en compte une transaction distincte. </li> 
    </ul> </td> 
  </tr>
 </tbody>
</table>

## Communication interactive facturable et processus AEM orientés formulaires sur les API OSGi {#billable-interactive-communication-and-form-centric-aem-workflows-on-osgi-apis}

Affectez des étapes de tâche et de services de document aux processus AEM orientés formulaire sur OSGi et à tous les rendus de communication interactive et sont comptabilisés comme des transactions. La prévisualisation d’une communication interactive sur l’instance d’auteur et la prévisualisation sur l’instance de publication à l’aide de l’interface utilisateur de l’agent ne sont pas comptabilisées comme des transactions. Si une étape de flux de travail comptabilise une transaction et que celle-ci échoue, le nombre de transactions n’est pas annulé.
...
<table> 
 <tbody>
  ...
  <tr>
   <td><a href="https://helpx.adobe.com/experience-manager/6-4/forms/javadocs/com/adobe/fd/ccm/channels/print/api/model/PrintChannel.html" target="_blank">render</a> (conversion au format PDF)</td> 
