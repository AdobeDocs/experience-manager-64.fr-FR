...
description: User Management est un composant AEM Forms JEE qui permet de créer, gérer et autoriser des utilisateurs AEM Forms à accéder à AEM Forms. Explorez plus en détail les données utilisateur et les entrepôts de données. Découvrez comment accéder aux données utilisateur et les supprimer.
...
Les données User Management qui ont accédé au moins une fois aux applications Forms à l’adresse `https://[*server*]:[*host*]/lc` sont également stockées dans le référentiel AEM.
