...
Examinons un exemple de la manière dont vous pouvez configurer un serveur de proxy inverse Apache 2.4 sans répartiteur. Dans cet exemple, vous allez héberger le serveur AEM avec `/forms` chemin et mappage du contexte `/forms` pour le proxy inverse. Cela garantit que toute demande de `/forms` sur le serveur Apache sont dirigés vers l’instance AEM. Cette topologie permet de réduire le nombre de règles au niveau de la couche du Dispatcher, car toutes les requêtes affectées du préfixe `/forms` routage vers le serveur AEM.
...
1. Configurez des règles de proxy en ajoutant les lignes de code suivantes dans la variable `httpd-proxy.conf` fichier de configuration.
   ...
   Remplacer `[AEM_Instance]` avec l’URL de publication du serveur AEM dans les règles.
