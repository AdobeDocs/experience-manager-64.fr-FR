...
1. **Utilisation des propriétés de configuration** : le composant Recherche et énumérateur possède plusieurs configurations que vous pouvez utiliser dans l’interface utilisateur. Par exemple, pour afficher du texte d’info-bulle HTML enregistré dans la boîte de dialogue de modification, utilisez l’attribut **${config-htmlLinkText}.** De même, pour le texte d’info-bulle PDF, utilisez l’**attribut ${config-pdfLinkText}** .
