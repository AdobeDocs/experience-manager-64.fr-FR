...
* Pour les actions d’envoi autres que **Action d’envoi du portail Forms**, la variable **[!UICONTROL Stockage des données dans Forms Portal]** est activée dans la variable **Envoi** propriétés du conteneur de formulaires adaptatifs.
...
Toutes les données de brouillons et de formulaires envoyés dans des instances AEM (auteur, publication ou distante) pour les utilisateurs connectés et anonymes sont stockées dans la variable `/content/forms/fp/` du référentiel d’AEM applicable. Chaque fois qu’un utilisateur connecté ou anonyme enregistre un formulaire en tant que brouillon ou l’envoie, un `draft ID` ou un `submission ID`, un `user data ID`, et un `ID` aléatoire pour chaque pièce jointe (le cas échéant) sont générés et associés au brouillon ou à l’envoi correspondant.

#### Accès aux données utilisateur {#access-user-data}

Lorsqu’un utilisateur connecté enregistre un formulaire en tant que brouillon ou l’envoie, un nœud enfant est créé avec son ID utilisateur. Par exemple, les données de brouillons et d’envois pour Sarah Rose dont l’ID utilisateur est `srose` sont stockées dans `/content/forms/fp/srose/` dans le référentiel AEM. Dans le nœud de l’ID utilisateur, les données sont organisées dans une structure hiérarchique.

Le tableau suivant explique comment les données de tous les brouillons par `srose` est stocké dans AEM référentiel.

>[!NOTE]
>
>Une structure exacte telle que `drafts` est répliqué pour les formulaires envoyés pour `srose` sous le `/content/forms/fp/srose/submit/` noeud .
>
>Tous les brouillons et envois de `anonymous` Les utilisateurs sont stockés sous le `/content/forms/fp/anonymous/` , qui organise les brouillons et les envois pour tous les utilisateurs anonymes sous le `draft` et `submit` noeuds.
...
Les données de brouillons et d’envoi pour tous les utilisateurs anonymes sont stockées dans le `drafts` et `submit` noeuds sous `/content/forms/fp/anonymous`. Vous ne pouvez pas rechercher des données pour un utilisateur anonyme particulier sauf si certaines informations d’identification sont connues. Dans ce cas, vous pouvez rechercher ces informations qui permettent d’identifier l’utilisateur anonyme dans le référentiel AEM et supprimer manuellement le nœud le contenant de toutes les instances AEM applicables pour supprimer les données du système AEM. Toutefois, pour supprimer des données pour tous les utilisateurs anonymes, vous pouvez supprimer la variable `anonymous` pour supprimer les données de brouillons et d’envois pour tous les utilisateurs anonymes.
