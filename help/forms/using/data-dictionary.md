...
Correspondence Management uses the back end, customer, `` ``or recipient-specific data stored according to the data dictionary’s structure to create correspondence meant for different customers. Par exemple, un document peut être créé avec des noms conviviaux, tels que « Cher/chère {Prénom} » ou « M. {Nom} ».
...
1. (Facultatif) Après la sélection d’un élément, vous pouvez ajouter des propriétés dans l’onglet Avancé (attributs). You can also tap **Add Field** and extend the properties of a DD element.
...
1. Appuyez sur Coller. La copie du dictionnaire de données est créée. Vous pouvez désormais apporter les modifications requises dans votre nouveau dictionnaire de données.
...
1. Tap **OK** in the alert message. Un fichier XML se télécharge.
1. Ouvrez le fichier XML avec le Bloc-notes ou un éditeur XML différent. Le fichier XML a la même structure que les chaînes de dictionnaire de données et d’espace réservé dans les éléments. Remplacer les chaînes d’espace réservé par celles avec lesquelles vous souhaitez tester une lettre.
   ...
   >[!NOTE]
   >
   >Dans cet exemple, XML crée de l’espace pour trois valeurs à un élément de collection, mais le nombre de valeurs peut être augmenté/réduit selon les besoins.
...
Une fois que vous avez créé un dictionnaire de données, vous pouvez le télécharger en tant qu’exemple de fichier de données XML pour y saisir du texte.
...
1. Tap **OK** in the alert message.
   ...
   Correspondence Management crée un fichier XML basé sur la structure du dictionnaire de données sélectionné et le télécharge vers votre ordinateur avec le nom &lt;nom-du-dictionnaire-de-données>-SampleData. Now you can edit this file in an XML or text editor to make data entries while [creating a letter](/help/forms/using/create-letter.md).
...
1. Enregistrez les différents fichiers de propriétés mis à jour sous les noms suivants :
