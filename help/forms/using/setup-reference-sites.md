...
* Communications interactives pour créer des communications client interactives, personnalisées et adaptées qui s’adaptent au paramètre et à la mise en page du périphérique.
...
* **Essentiels AEM**
   ...
   AEM Quickstart, module complémentaire AEM Forms et packages de site de référence. See [AEM Forms releases](https://helpx.adobe.com/aem-forms/kb/aem-forms-releases.html) for add-on and reference sites packages details.
...
* **Compte de développeur Adobe Sign et application API Adobe Sign**
   ...
   Pour utiliser les fonctionnalités de signature numérique, un compte Développeur Adobe Sign est requis. Voir [Adobe Sign](https://acrobat.adobe.com/us/en/why-adobe/developer-form.html).
...
* Une instance en cours d’exécution d’AEM 6.4 avec module complémentaire Forms. Pour plus d’informations, reportez-vous à la section [Installation et configuration d’AEM Forms](https://helpx.adobe.com/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html).
...
Les paramètres du service AEM DS sont requis sur l’instance de publication pour les communications par courrier électronique dans les cas d’utilisation du site de référence. Pour obtenir des instructions détaillées sur la configuration du service AEM DS sur l’instance de publication, voir [Configuration des paramètres](/help/forms/using/configuring-the-processing-server-url-.md)AEM DS.

Pour les sites de référence d’AEM Forms, dans le service des paramètres d’AEM DS, spécifiez l’URL du serveur de publication au lieu de l’URL du serveur de traitement.
...
Installez les packages de sites de référence suivants à l’aide du partage de package.
...
Configurez le service cloud OAuth dans AEM Forms pour activer la communication entre AEM Forms et Microsoft Dynamics. Effectuez les étapes suivantes pour configurer le service OAuth Cloud sur les instances d’auteur et de publication AEM :

1. On AEM author instance, go to **[!UICONTROL Tools > Cloud Services > Data Sources > global]**. Appuyez sur l’icône Intégration **[!UICONTROL de la dynamique]** de site de référence et appuyez sur **[!UICONTROL Propriétés]**.
...
1. Go to **[!UICONTROL Forms > Data Integrations > We.Finance]**. Sélectionnez Assurance automatique (Dynamique) et cliquez sur Modifier. Les entités Microsoft Dynamics sont répertoriées sous l’onglet Sources de données. Attendez que toutes les entités soient récupérées de Microsoft Dynamics et répertoriées sous l’onglet Sources de données.
1. Select the **[!UICONTROL AutoInsuranceRenewal entity]** and click **[!UICONTROL Test Model Object]**. In the input request section, specify the value for customer ID as “900001” and click **[!UICONTROL Test]**. La section Output affiche les enregistrements récupérés de Microsoft Dynamics pour l’ID de client 900001.
1. In the input request section, specify the value for customer ID as “900001” and click **[!UICONTROL Test]**. La section Output affiche les enregistrements récupérés de Microsoft Dynamics pour l’ID de client 900001.
...
1. Configuration de l’expression **[!UICONTROL du planificateur de mise à jour]** d’état comme **0 0/2 &amp;ast; &amp;ast; &amp;ast; ?**.
...
1. Go to **[!UICONTROL Tools > Cloud Services > Adobe Sign > global]**. Sélectionnez **[!UICONTROL AEM Forms Reference Site Sign]** et appuyez sur **[!UICONTROL Propriétés]**.

   >[!CAUTION]
   >
   >Assurez-vous que l’URL https://[host]:[ssl_port]/mnt/overlay/adobesign/cloudservices/adobesign/properties.html est ajoutée à la liste des URL de redirection de la configuration OAuth de l’application API Adobe Sign.
...
1. (Instance **Auteur uniquement**) Accédez à **[!UICONTROL Outils** > Services ****Cloud > Sources **de**données > **global.]** Sélectionnez **[!UICONTROL roi-repos]** et appuyez sur **[!UICONTROL Propriétés]**. Appuyez sur Paramètres **[!UICONTROL d’]** authentification et définissez le Type **[!UICONTROL d’]** authentification sur Authentification **** de base. Spécifiez/ `admin``admin`comme nom d’utilisateur/mot de passe pour accéder au service. Tap **[!UICONTROL Save &amp; Close]**.
...
Pour générer un rapport, les données de base sont regroupées avec les sites de référence. Avant d’utiliser les données de base, procédez comme suit :
...
1. Accédez à https://&lt;*hôte*>:&lt;*port*>/system/console/configMgr et procédez comme suit :
   ...
   * Dans le champ **[!UICONTROL Cadre]** SiteCatalyst de la boîte de dialogue de configuration d’AEM Forms Analytics, sélectionnez we-finance(we-finance) ou we-gov(we-gov).
...
1. Accédez au gestionnaire de formulaires à l’adresse https://&lt;hôte>:&lt;port>/aem/forms et procédez comme suit :
   ...
   * Dans la barre d’outils des actions, cliquez sur Rapport d’analyse. Une fois que vous avez activé les analyses pour le formulaire, cliquez sur Rapport d’analyse. Vous constatez qu’un rapport vide a été généré. Après la génération d’un rapport vierge, vous devez fournir les données de base envoyées avec le module de site de référence pour générer un rapport d’analyse à des fins de démonstration.
   Les sites de référence fournissent des rapports d’analyse avec des données de base pour les cas d’utilisation de cartes de crédit, de prêts hypothécaires et de pensions alimentaires pour enfants. Pour la configuration des données de base, consultez la présentation [du site de référence](/help/forms/using/finance-reference-site-walkthrough.md) We.Finance et la présentation [du site de référence](/help/forms/using/gov-reference-site-walkthrough.md)We.Gov.
...
1. Dans le champ **[!UICONTROL Target URLs]**, indiquez l’URL vers AEM Forms. For example: https://&lt;*hostname*>:&lt;*port*>.

1. Cliquez sur **[!UICONTROL Enregistrer]**.

Les cas d’utilisation de demandes de carte de crédit et de demandes de prêt immobilier montrent comment effectuer des tests A/B et présenter un rapport à des fins de démonstration. Pour consulter les procédures pas à pas, consultez la présentation [du site de référence](/help/forms/using/finance-reference-site-walkthrough.md)Web.Finance.
