...
1. Dans le **[!UICONTROL Général]** de l’onglet **[!UICONTROL Création d’une configuration Adobe Sign]** , spécifiez une **Nom** pour la configuration et appuyez sur **Suivant**. Vous avez la possibilité d’indiquer un titre et de rechercher et de sélectionner une vignette pour la configuration.
...
1. Dans le **Autoriser** champ, **select** Tous les utilisateurs : tous les utilisateurs, anonymes ou connectés, peuvent prévisualiser les pièces jointes, vérifier et signer des formulaires, puis cliquer sur **Enregistrez.** L’instance d’auteur est configurée pour utiliser Adobe Sign.
1. Utilisation [réplication](/help/sites-deploying/replication.md) pour créer une configuration identique sur les instances de publication correspondantes.
