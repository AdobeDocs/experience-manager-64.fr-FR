...
**Pages surdimensionnées :** Crée des documents PDF adaptés à l’affichage et à l’impression fiables de dessins techniques de plus de 200 x 200 pouces. Les documents PDF créés peuvent être ouverts dans Adobe Acrobat Professional, Acrobat Standard 7 ou versions ultérieures et Adobe Reader 7 ou versions ultérieures.
...
**PDF/X-1a 2001 :** Vérifie les tâches entrantes pour la conformité PDF/X-1a et crée des fichiers de PDF uniquement en cas de conformité. PDF/X-1a est une norme ISO relative à l’échange de contenu graphique. PDF/X-1a requiert l’incorporation de toutes les polices, la définition de toutes les zones PDF appropriées et l’affichage en couleurs CMJN ou couleurs d’accompagnement. Les fichiers PDF répondant aux normes PDF/X-1a sont soumis à des conditions d’impression spécifiques (par exemple, l’impression rotative offset selon les Specifications Web Offset Publications). Pour plus d’informations sur PDF/X, voir Adobe et les standards du marché.

**PDF/X-3 2002 :** Vérifie les tâches entrantes pour la conformité PDF/X-3 et crée des fichiers PDF uniquement si elles sont conformes. Comme PDF/X-1a, PDF/X-3 est une norme ISO relative à l’échange de contenu graphique. La principale différence réside dans le fait que la norme PDF/X-3 prend en charge les couleurs de référence.

**Qualité de la presse :** Crée des fichiers PDF pour une production d’impression de haute qualité (par exemple, sur une imageuse d’images ou une imageuse de plaques). Dans ce cas, la taille de fichier n’est pas prise en compte. L’objectif est de gérer toutes les informations dans un fichier PDF, dont un imprimeur commercial ou un prestataire de services de pré-presse a besoin pour imprimer correctement le document. Cet ensemble d’options :
...
**Compression au niveau de l’objet :** Consolidation des petits objets (qui ne sont pas compressibles eux-mêmes) en flux qui peuvent ensuite être compressés efficacement.

**Désactivé :** Ne compresse aucune information structurelle dans le document du PDF. Sélectionnez cette option si vous souhaitez que les utilisateurs affichent, naviguent et interagissent avec des signets et d’autres informations structurelles à l’aide d’Acrobat 5 et versions ultérieures.

**Balises uniquement :** Compresse les informations structurelles dans le document du PDF. Cette option génère un fichier PDF pouvant être ouvert et imprimé avec Acrobat 5. Les utilisateurs ne peuvent pas afficher les informations d’accessibilité, de structure ou de balisage du PDF dans Acrobat 5 ou Acrobat Reader 5.0 mais l’opération est possible dans Acrobat 6 et Adobe Reader 6.0.
...
**Sous-échantillonnage moyen à :** Moyenne des pixels dans une zone d’échantillon et remplace la zone entière par la couleur moyenne en pixels à la résolution spécifiée.
...
**Sous-échantillonnage à :** Sélectionne un pixel au centre de la zone d’échantillon et remplace la zone entière par ce pixel à la résolution spécifiée. Comparée à l’échantillonnage classique, cette méthode permet de réduire significativement la durée de conversion. Cependant, les images qui en résultent sont moins lisses et continues.
...
**Anti-alias To Gray :** Lit les bords dentés des images monochromes. Sélectionnez 2 bits, 4 bits ou 8 bits pour définir 4, 16 ou 256 niveaux de gris (le lissage des gris peut donner un aspect flou aux corps intermédiaires ou aux lignes fines).
...
**Jeux partiels de polices incorporées lorsque le pourcentage de caractères utilisés est inférieur à :** Si vous sélectionnez cette option, indiquez un pourcentage seuil pour incorporer uniquement un sous-ensemble des polices. Par exemple, si le seuil est de 35 et que moins de 35 % des caractères sont utilisés, PDF Generator incorpore uniquement ces caractères. Seules les polices avec des bits de droits d’accès appropriés sont incorporées.

**En cas d’échec de l’incorporation :** Indique la manière dont PDF Generator répond s’il ne parvient pas à trouver une police à incorporer lors du traitement d’un fichier. Vous pouvez demander à PDF Generator d’ignorer la demande et de remplacer la police, de vous avertir et de remplacer la police ou d’annuler le traitement du travail en cours.

**Source de la police :** L’emplacement des polices utilisées par PDF Generator.
...
**Laissez la couleur inchangée :** Permet de ne pas changer les couleurs dépendantes de l’appareil et conserve les couleurs indépendantes de l’appareil comme l’équivalent le plus proche possible en PDF. Cette option s’avère utile pour les ateliers d’impression ayant étalonné l’ensemble de leurs périphériques, ayant utilisé ces informations pour définir les couleurs du fichier et qui impriment uniquement sur ces périphériques.

**Référencer les couleurs :** Incorpore un profil ICC lors de la conversion de fichiers et calibre la couleur des images, ce qui rend les couleurs des fichiers PDF générés indépendantes de l’appareil si vous avez sélectionné la compatibilité Acrobat 4 (PDF 1.3) ou ultérieure. Cependant, les espaces colorimétriques dépendants du périphérique des fichiers (RVB, Niveaux de gris et CMJN) sont convertis en espaces colorimétriques de référence (CalRVB, CalGray et LAB).
...
**Convertir toutes les couleurs en sRVB ou Convertir toutes les couleurs en CMJN :** permet d’étalonner la couleur du fichier, en l’rendant indépendante de l’appareil, comme pour l’option Baliser tout pour la gestion des couleurs. Si vous avez sélectionné la compatibilité Acrobat 4 (PDF 1.3) ou versions ultérieures et Convertir toutes les couleurs en sRVB, en CMJN et RVB, les images RVB sont converties en sRVB.
...
**Conserver :** Signifie que l’intention est spécifiée dans le périphérique de sortie plutôt que dans le fichier du PDF. Dans de nombreux périphériques de sortie, Colorimétrie relative est le mode par défaut.

**Perception :** Maintient les valeurs colorimétriques relatives parmi les pixels d’origine lors de leur mappage à la gamme de destinations. Cette méthode donne la possibilité de conserver la relation visuelle entre les couleurs, bien que les valeurs des couleurs elles-mêmes puissent changer.
...
**Autoriser le remplacement des paramètres Adobe PDF par un fichier PostScript :** Utilise les paramètres stockés dans un fichier PostScript au lieu du fichier de paramètres Adobe PDF actuel. Avant de traiter un fichier PostScript, vous pouvez placer des paramètres dans le fichier pour contrôler les aspects suivants :
...
**Autoriser les XObjects au format PostScript :** Les XObjects au format PostScript stockent des informations qui apparaissent sur de nombreuses pages du même fichier, telles qu’une image d’arrière-plan ou des informations d’en-tête et de pied de page. Leur utilisation peut contribuer à accélérer l’impression, mais nécessite une plus grande mémoire d’imprimante. Pour empêcher la création de XObjects au format PostScript, désélectionnez cette option si vous créez des fichiers PDF avec une compatibilité Acrobat 5 (PDF 1.4) ou versions ultérieures.

**Convertir les dégradés en ombres Lissers :** Convertit les mélanges en ombres lissées pour Acrobat 4 et versions ultérieures, ce qui réduit les fichiers PDF et améliore potentiellement la qualité de la sortie finale. PDF Generator convertit les dégradés issus d’Adobe Illustrator, Adobe InDesign, Adobe Freehand MX, CorelDraw, Quark XPress et Microsoft PowerPoint.

**Convertir les lignes Lissers en courbes :** Réduit la quantité de points de contrôle utilisés pour créer des courbes dans des dessins CAO, ce qui se traduit par des PDF plus petits et un rendu plus rapide à l’écran.

**Conserver la sémantique de page de copie de niveau 2 :** Utilise l’opérateur copypage défini dans LanguageLevel 2 PostScript au lieu de dans LanguageLevel 3 PostScript. Si vous disposez d’un fichier PostScript et que cette option est sélectionnée, un opérateur copypage copie la page. Si cette option n’est pas sélectionnée, l’équivalent d’une opération showpage est exécuté, mais l’état de l’image n’est pas réinitialisé.
...
**La Surimpression Par Défaut Est La Surimpression Non Nulle :** Empêche les objets surimprimés dont les valeurs CMJN sont nulles de supprimer les objets CMJN situés en-dessous. Pour ce faire, il suffit d’insérer le paramètre d’état d’image OPM 1 dans le fichier PDF partout où Setoverprint est présent.
...
**Si Possible, Enregistrez Les Images Du JPEG D’Origine Dans PDF :** Traite toutes les images de JPEG compressées (images déjà compressées à l’aide de l’encodage DCT) sans les compresser à nouveau. Si cette option est sélectionnée, PDF Generator décompresse les images JPEG afin de vérifier qu’elles ne sont pas corrompues. Toutefois, elle ne recompresse pas les images valides. L’image d’origine est traitée sans aucune modification. Les performances sont alors accrues en raison d’une décompression non suivie d’une recompression et les données d’image ainsi que les métadonnées sont conservées.

**Enregistrer le ticket de tâche portable dans le fichier du PDF :** Permet de conserver un ticket de tâche PostScript dans un fichier de PDF. Le dossier de correspondance contient des informations relatives au fichier PostScript, telles que le format de page, la résolution et des informations de recouvrement, plutôt que les informations sur le contenu. Il est possible d’utiliser ces informations ultérieurement dans un flux de travail ou pour l’impression du PDF.
...
**Traiter les commentaires DSC :** Conserve les informations du DSC d’un fichier PostScript. Les sous-options disponibles sont les suivantes :

**Enregistrer les avertissements DSC :** Affiche des messages d’avertissement à propos des commentaires DSC problématiques pendant le traitement et les ajoute à un fichier journal.

**Conserver les informations EPS de DSC :** Conserve les informations, telles que l’application d’origine et la date de création d’un fichier EPS. Si cette option est désélectionnée, la page est dimensionnée et centrée en fonction du coin supérieur gauche de l’objet supérieur gauche et du coin inférieur droit de l’objet inférieur droit de la page.

**Conserver les commentaires OPI :** Conserve les informations requises pour remplacer une image ou un commentaire For Placement Only (FPO) par l’image haute résolution située sur les serveurs qui prennent en charge les versions 1.3 et 2.0 d’Open Prepress Interface (OPI).

**Conserver les informations sur le document de DSC :** Conserve des informations telles que le titre, la date de création et l’heure. Lorsque vous ouvrez un fichier PDF dans Acrobat, ces informations s’affichent dans le panneau de description des propriétés du document.

**Redimensionner la page et centrer l’illustration pour les fichiers EPS :** Centre une image EPS et redimensionne la page de sorte qu’elle s’ajuste de manière à s’adapter à l’image. Cette option concerne uniquement les travaux composés d’un seul fichier EPS.
...
**Lorsque non conforme :** Indique si le fichier de PDF doit être créé si le fichier PostScript ne respecte pas les exigences de PDF/X. Cette option est disponible si une valeur autre qu’Aucune est affectée à l’option Norme de conformité de la page Rapport et conformité aux normes.

**Continuer :** Crée un fichier de PDF.

**Annuler la tâche :** Crée un fichier de PDF uniquement si le fichier PostScript répond aux exigences du PDF/X des options de rapport sélectionnées et s’il est sinon valide. Si les deux options de rapport PDF/X sont sélectionnées et que le fichier PostScript ne répond qu’à un seul jeu de critères PDF/X (par exemple, PDF/X-3), PDF Generator crée le fichier conforme.

**Si vous ne spécifiez ni zone de rognage ni zone graphique :** Cette option est disponible lorsque la norme de conformité de la page Rapport et conformité aux normes est définie sur une option autre que Aucune.

**Signaler comme erreur :** Marque le fichier PostScript comme non conforme si l’une des options de rapport est sélectionnée et qu’une zone de rognage ou une zone graphique est manquante dans une page.

**Définissez La Zone De Rognage Sur La Zone De Média Avec Les Décalages :** Calcule les valeurs en points de la zone de rognage en fonction des décalages de la zone de support des pages respectives si aucune zone de rognage ni zone graphique n’est spécifiée. La zone de rognage est toujours aussi petite ou plus petite que la zone de support qui l’entoure.

**Si BleedBox N’Est Pas Spécifié :** Cette option est disponible lorsque la norme de conformité de la page Rapport et conformité aux normes est définie sur une option autre que Aucune.

**Définir la zone de fond perdu sur la zone multimédia :** Utilise les valeurs de la zone de fond perdu si la zone de fond perdu n’est pas spécifiée.

**Définir la zone de fond perdu pour la zone de rognage avec les décalages :** Calcule les valeurs en points de la zone de fond perdu en fonction des décalages de la zone de rognage des pages respectives si la zone de fond perdu n’est pas spécifiée. La zone de fond perdu est toujours aussi grande ou plus grande que la zone de rognage insérée.

**Valeurs par défaut si elles ne sont pas spécifiées dans le document :** Cette option est disponible lorsque l’option Norme de conformité de la page Rapport et conformité aux normes est définie sur Aucune.

**Nom du profil d’intention de sortie :** Indique la condition d’impression caractérisée pour laquelle le document est préparé. Si un document ne spécifie aucun nom de profil du mode de sortie, PDF Generator utilise la valeur sélectionnée à partir de ce menu. Vous pouvez sélectionner un des noms fournis ou saisir un nom dans l’espace prévu à cet effet. Si votre flux de travail nécessite la définition du mode de sortie, sélectionnez Aucun. Tout document ne répondant pas aux exigences ne passe pas le contrôle de conformité.

**Identifiant de condition de sortie :** Indique le nom de référence spécifié par le registre du nom du profil d’intention de sortie.
...
**Piège :** Indique l’état du recouvrement dans le document. La conformité à la norme PDF/X requiert la valeur True ou False. Si le document ne spécifie pas l’état de recouvrement, la valeur fournie ici est utilisée. Si votre flux de travail nécessite la définition de cet état, sélectionnez Ne pas définir. Tout document ne répondant pas aux exigences ne passe pas le contrôle de conformité.
...
**Lorsque non conforme :** Indique s’il faut créer le fichier du PDF si le fichier PostScript ne respecte pas les exigences du PDF/A.

**Continuer :** Crée un fichier de PDF même si le fichier PostScript ne répond pas aux exigences de la norme.

**Annuler la tâche :** Crée un fichier de PDF uniquement si le fichier PostScript répond aux exigences du PDF/A et est valide dans le cas contraire.

**Nom du profil d’intention de sortie :** Indique la condition d’impression caractérisée pour laquelle le document a été préparé et est requise pour la conformité PDF/A. Si votre flux de travail nécessite de connaître le mode de sortie, sélectionnez Aucun. Le document ne pourra pas vérifier la conformité si cette information n’est pas fournie.
...
**Mise en page :** Détermine si le document est affiché en mode Une seule page, Page en vis-à-vis, Page continue ou Page en mode En face continue.
...
**Ouvrir à la page :** Définit la page à laquelle le document s’ouvre, qui correspond généralement à la page 1.
...
**Redimensionner la fenêtre en page initiale :** Ajuste la fenêtre du document pour l’adapter facilement à la page d’ouverture, en fonction des options sélectionnées sous Options du document.

**Centrer la fenêtre sur l’écran :** Positionne la fenêtre au centre de la zone de l’écran.

**Ouvrir en mode Plein écran :** Maximise la fenêtre du document et affiche le document sans les commandes de la barre de menus, de la barre d’outils ou de la fenêtre.
...
**Masquer les commandes de fenêtre :** Si cette option est sélectionnée, les commandes de fenêtre sont masquées.
