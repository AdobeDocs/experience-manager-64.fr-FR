...
**PDF/X-3 2002 :** vérifie la conformité des tâches entrantes à la norme PDF/X-3 et crée des fichiers PDF uniquement en cas de conformité. Comme PDF/X-1a, PDF/X-3 est une norme ISO relative à l’échange de contenu graphique. La principale différence réside dans le fait que la norme PDF/X-3 prend en charge les couleurs de référence.

**Qualité d’impression :** crée des fichiers PDF pour une production d’impression de haute qualité (par exemple, sur une imageuse d’images ou une devant de plaques). Dans ce cas, la taille de fichier n’est pas prise en compte. L’objectif est de gérer toutes les informations dans un fichier PDF, dont un imprimeur commercial ou un prestataire de services de pré-presse a besoin pour imprimer correctement le document. Cet ensemble d’options :
...
**Compression au niveau de l’objet :** consolide les petits objets (qui ne sont pas compressibles eux-mêmes) en flux qui peuvent ensuite être compressés efficacement.

**Off :** ne compresse pas les informations structurelles du document PDF. Sélectionnez cette option si vous souhaitez que les utilisateurs affichent, naviguent et interagissent avec des signets et d’autres informations structurelles à l’aide d’Acrobat 5 et versions ultérieures.
...
**Sous-échantillonnage moyen à :** permet de moyennes des pixels d’une zone d’échantillonnage et de remplacer l’ensemble de la zone par la couleur moyenne des pixels à la résolution spécifiée.

**Sous-échantillonnage bicubique à :** utilise une moyenne pondérée pour déterminer la couleur des pixels et donne généralement de meilleurs résultats que la méthode de sous-échantillonnage moyenne simple. La méthode bicubique est la plus lente, mais également la plus précise et est celle qui offre les dégradés de tons les plus lisses.
...
**Anti-alias To Gray :** permet de lisser les bords dentelés dans les images monochromes. Sélectionnez 2 bits, 4 bits ou 8 bits pour définir 4, 16 ou 256 niveaux de gris (le lissage des gris peut donner un aspect flou aux corps intermédiaires ou aux lignes fines).
...
**Jeux partiels de polices incorporées lorsque le pourcentage de caractères utilisés est inférieur à :** si vous sélectionnez cette option, indiquez un pourcentage seuil pour incorporer uniquement un sous-ensemble de polices. Par exemple, si le seuil est de 35 et que moins de 35 % des caractères sont utilisés, PDF Generator incorpore uniquement ces caractères. Seules les polices avec des bits de droits d’accès appropriés sont incorporées.

**En cas d’échec de l’incorporation :** indique comment PDF Generator répond s’il ne parvient pas à trouver une police à incorporer lors du traitement d’un fichier. Vous pouvez demander à PDF Generator d’ignorer la demande et de remplacer la police, de vous avertir et de remplacer la police ou d’annuler le traitement du travail en cours.
...
**Laissez les couleurs inchangées :** laisse inchangées les couleurs dépendantes du périphérique et conserve les couleurs indépendantes du périphérique comme équivalent le plus proche possible dans PDF. Cette option s’avère utile pour les ateliers d’impression ayant étalonné l’ensemble de leurs périphériques, ayant utilisé ces informations pour définir les couleurs du fichier et qui impriment uniquement sur ces périphériques.

**Baliser tout pour la gestion des couleurs :** incorpore un profil de consortium de couleurs international lors de la conversion de fichiers et de l’calibrage de la couleur dans les images, ce qui rend les couleurs des fichiers PDF obtenus indépendantes du périphérique si vous avez sélectionné la compatibilité Acrobat 4 (PDF 1.3) ou ultérieure. Cependant, les espaces colorimétriques dépendants du périphérique des fichiers (RVB, Niveaux de gris et CMJN) sont convertis en espaces colorimétriques de référence (CalRVB, CalGray et LAB).
...
**Perception :** conserve les valeurs colorimétriques relatives parmi les pixels d’origine lors de leur mappage sur la gamme de destinations. Cette méthode donne la possibilité de conserver la relation visuelle entre les couleurs, bien que les valeurs des couleurs elles-mêmes puissent changer.
...
**Lorsque des fonctions de transfert sont détectées :** détermine ce qu’il faut faire lorsque des fonctions de transfert sont détectées :

**Conserver :** conserve les fonctions de transfert généralement utilisées pour compenser le gain de point ou la perte de point qui peut se produire lorsqu’une image est transférée vers le film. L’élargissement du point survient lorsque les points d’encre dont une page imprimée est composée sont plus grands que sur la trame de similigravure (par exemple, en raison de l’étalement sur le papier). Par contre, le rétrécissement du point survient lorsque les points s’impriment en plus petit. Grâce à cette option, les fonctions de transfert sont conservées dans le fichier et appliquées à ce dernier lors de son impression.
...
**Autoriser le fichier PostScript à remplacer les paramètres Adobe PDF :** permet d’utiliser les paramètres stockés dans un fichier PostScript au lieu du fichier de paramètres Adobe PDF actuel. Avant de traiter un fichier PostScript, vous pouvez placer des paramètres dans le fichier pour contrôler les aspects suivants :
...
**Convertir les dégradés en ombres Lisser :** permet de convertir les dégradés en ombres lissées pour Acrobat 4 et versions ultérieures, ce qui réduit les fichiers PDF et améliore potentiellement la qualité de la sortie finale. PDF Generator convertit les dégradés issus d’Adobe Illustrator, Adobe InDesign, Adobe Freehand MX, CorelDraw, Quark XPress et Microsoft PowerPoint.

**Convertir les lignes Lissers en courbes :** permet de réduire le nombre de points de contrôle utilisés pour créer des courbes dans des dessins CAO, ce qui se traduit par des PDF plus petits et un rendu plus rapide à l’écran.

**Conserver la sémantique de page de copie de niveau 2 :** utilise l’opérateur copypage défini dans LanguageLevel 2 PostScript plutôt que dans LanguageLevel 3 PostScript. Si vous disposez d’un fichier PostScript et que cette option est sélectionnée, un opérateur copypage copie la page. Si cette option n’est pas sélectionnée, l’équivalent d’une opération showpage est exécuté, mais l’état de l’image n’est pas réinitialisé.

**Conserver les paramètres de surimpression :**  conserve les paramètres de surimpression des fichiers convertis en PDF. Les couleurs surimprimées consistent en deux encres ou plus imprimées l’une sur l’autre. Par exemple, si une encre cyan s’imprime sur une encre jaune, la couleur de surimpression obtenue est verte. Sans surimpression, le jaune sous-jacent ne serait pas imprimé et donnerait du cyan.

**Surimpression par défaut La surimpression non nulle :** empêche les objets surimprimés avec zéro valeur CMJN d’éliminer les objets CMJN situés en-dessous. Pour ce faire, il suffit d’insérer le paramètre d’état d’image OPM 1 dans le fichier PDF partout où Setoverprint est présent.
...
**Conserver les informations EPS du DSC :** conserve les informations, telles que l’application d’origine et la date de création d’un fichier EPS. Si cette option est désélectionnée, la page est dimensionnée et centrée en fonction du coin supérieur gauche de l’objet supérieur gauche et du coin inférieur droit de l’objet inférieur droit de la page.

**Conserver les commentaires OPI :** conserve les informations nécessaires au remplacement d’une image ou d’un commentaire For Placement Only (FPO) par l’image haute résolution située sur les serveurs qui prennent en charge les versions 1.3 et 2.0 de l’interface Open Prepress (OPI).

**Conserver les informations du document de DSC :** conserve des informations telles que le titre, la date de création et l’heure. Lorsque vous ouvrez un fichier PDF dans Acrobat, ces informations s’affichent dans le panneau de description des propriétés du document.

**Redimensionner la page et centrer l’illustration pour les fichiers EPS :** recense une image EPS et redimensionne la page pour l’adapter à l’image. Cette option concerne uniquement les travaux composés d’un seul fichier EPS.
...
**When Not Compliant :**  indique s’il faut créer le fichier PDF si le fichier PostScript ne respecte pas les exigences de la norme PDF/X. Cette option est disponible si une valeur autre qu’Aucune est affectée à l’option Norme de conformité de la page Rapport et conformité aux normes.

**Continuer :** crée un fichier PDF.

**Annuler la tâche :** crée un fichier PDF uniquement si le fichier PostScript répond aux exigences PDF/X des options de rapport sélectionnées et s’il est valide. Si les deux options de rapport PDF/X sont sélectionnées et que le fichier PostScript ne répond qu’à un seul jeu de critères PDF/X (par exemple, PDF/X-3), PDF Generator crée le fichier conforme.

**Si vous ne spécifiez ni zone de rognage ni zone graphique :**  cette option est disponible lorsque la norme de conformité de la page de création de rapports et de conformité aux normes est définie sur une option autre que Aucune.
...
**Définir la zone de rognage sur la zone de média avec des décalages :** calcule les valeurs en points de la zone de rognage en fonction des décalages de la zone de média des pages respectives si ni la zone de rognage ni la zone d’illustration ne sont spécifiées. La zone de rognage est toujours aussi petite ou plus petite que la zone de support qui l’entoure.
...
**Définir la zone de fond perdu sur la zone de contenu multimédia :**  permet d’utiliser les valeurs de la zone de fond perdu si la zone de fond perdu n’est pas spécifiée.
...
**Valeurs par défaut si elles ne sont pas spécifiées dans le document :**  cette option est disponible lorsque la norme de conformité de la page de création de rapports et de conformité aux normes est définie sur une option autre que Aucune.
...
**Identifiant de condition de sortie :** indique le nom de référence spécifié par le registre du nom de profil du mode de sortie.
...
**When Not Compliant :**  indique s’il faut créer le fichier PDF si le fichier PostScript ne respecte pas les exigences de la norme PDF/A.

**Continuer :** crée un fichier PDF même si le fichier PostScript ne répond pas aux exigences de la norme.

**Annuler la tâche :** crée un fichier PDF uniquement si le fichier PostScript répond aux exigences PDF/A et est valide dans le cas contraire.
...
**Afficher :** détermine les panneaux et onglets qui s’affichent par défaut dans la fenêtre de l’application. Panneau et page Signets permet d’ouvrir le panneau du document et l’onglet Signets.

**Mise en page :** détermine si le document est affiché en mode Une seule page, Page en vis-à-vis, Page en continu ou Page en mode En face continue.

**Loupe :** définit le niveau de zoom utilisé pour afficher le document lorsqu’il est ouvert. La valeur par défaut est celle configurée par l’utilisateur dans les préférences Acrobat ou Adobe Reader.

**Ouvrir à la page :** définit la page à laquelle le document s’ouvre, c’est-à-dire généralement la page 1.
...
**Redimensionner la fenêtre sur la page initiale :**  permet de régler la fenêtre du document de manière à l’adapter facilement à la page d’ouverture, en fonction des options sélectionnées sous Options du document.

**Centrer la fenêtre sur l’écran :** positionne la fenêtre au centre de la zone d’écran.

**Ouvrir en mode Plein écran :** optimise la fenêtre du document et affiche le document sans les commandes de la barre de menus, de la barre d’outils ou de la fenêtre.

**Afficher :** le nom de fichier affiche le nom de fichier dans la barre de titre de la fenêtre. Titre du document indique le titre du document dans la barre de titre de la fenêtre.
...
**Masquer la barre de menus :** cette option masque la barre de menus.

**Masquer les barres d’outils :** si cette option est sélectionnée, les barres d’outils sont masquées.

**Masquer les commandes de fenêtre :** cette option masque les commandes de fenêtre.
