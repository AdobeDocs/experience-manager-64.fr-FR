...
**PDF/X-3 2002 :** Vérifie la conformité des travaux entrants à la norme PDF/X-3 et crée des fichiers PDF uniquement si ces travaux sont conformes. Comme PDF/X-1a, PDF/X-3 est une norme ISO relative à l’échange de contenu graphique. La principale différence réside dans le fait que la norme PDF/X-3 prend en charge les couleurs de référence.

**Qualité de la presse :** Crée des fichiers PDF pour une impression de haute qualité (par exemple, sur une imageuse d’images ou une imageuse de plaques). Dans ce cas, la taille de fichier n’est pas prise en compte. L’objectif est de gérer toutes les informations dans un fichier PDF, dont un imprimeur commercial ou un prestataire de services de pré-presse a besoin pour imprimer correctement le document. Cet ensemble d’options :
...
**Compression de niveau objet :** Consolidation des petits objets (qui ne sont pas compressibles eux-mêmes) en flux qui peuvent ensuite être compressés efficacement.
...
**Sous-échantillonnage moyen à :** Applique la moyenne des pixels à une zone d’échantillonnage et remplace la zone entière par la couleur moyenne des pixels à la résolution spécifiée.

**Sous-échantillonnage bicubique à :** Utilise une moyenne pondérée pour déterminer la couleur des pixels et produit généralement de meilleurs résultats que la méthode de sous-échantillonnage de moyenne simple. La méthode bicubique est la plus lente, mais également la plus précise et est celle qui offre les dégradés de tons les plus lisses.
...
**Lissage des gris :** permet de lisser les bords dentés dans les images monochromes. Sélectionnez 2 bits, 4 bits ou 8 bits pour définir 4, 16 ou 256 niveaux de gris (le lissage des gris peut donner un aspect flou aux corps intermédiaires ou aux lignes fines).
...
**Perception :** Conserve les valeurs de couleur relatives entre les pixels d’origine lors de leur mappage à la gamme de couleurs de destination. Cette méthode donne la possibilité de conserver la relation visuelle entre les couleurs, bien que les valeurs des couleurs elles-mêmes puissent changer.
...
**La Surimpression Par Défaut Est La Surimpression Non Nulle :** Empêche les objets surimprimés dont les valeurs CMJN sont nulles d’éliminer les objets CMJN situés en dessous. Pour ce faire, il suffit d’insérer le paramètre d’état d’image OPM 1 dans le fichier PDF partout où Setoverprint est présent.
...
**Conserver les informations EPS de DSC :** Conserve les informations, telles que la demande d’origine et la date de création d’un fichier EPS. Si cette option est désélectionnée, la page est dimensionnée et centrée en fonction du coin supérieur gauche de l’objet supérieur gauche et du coin inférieur droit de l’objet inférieur droit de la page.
...
**Conserver les informations de Document de DSC :** Conserve des informations telles que le titre, la date et l’heure de création. Lorsque vous ouvrez un fichier PDF dans Acrobat, ces informations s’affichent dans le panneau de description des propriétés du document.
...
**Lorsque non conforme :** Indique si le fichier PDF doit être créé si le fichier PostScript ne respecte pas les exigences de la norme PDF/X. Cette option est disponible si une valeur autre qu’Aucune est affectée à l’option Norme de conformité de la page Rapport et conformité aux normes.
...
**Si la zone de fond perdu n’est pas spécifiée :** Disponible lorsque l’option Norme de conformité de la page Rapports et conformité des normes est définie sur une autre option que Aucune.
...
**Définir la zone de fond perdu comme zone de rognage avec décalages :** Calcule les valeurs en points de la zone de fond perdu en fonction des décalages de la zone de rognage des pages respectives si la zone de fond perdu n’est pas spécifiée. La zone de fond perdu est toujours aussi grande ou plus grande que la zone de rognage insérée.

**Valeurs par défaut si elles ne sont pas spécifiées dans le Document :** Cette option est disponible lorsque l’option Norme de conformité de la page Rapports et conformité des normes est définie sur une autre option que Aucune.
...
**Identificateur de condition de sortie :** Indique le nom de référence spécifié par le registre du nom du profil du mode de sortie.
...
**Lorsque non conforme :** Indique si le fichier PDF doit être créé si le fichier PostScript ne respecte pas les exigences PDF/A.
...
**Mise en page :** Détermine si le document est affiché en mode Page unique, Page en vis-à-vis, Page continue ou Page en vis-à-vis continue.
...
**Redimensionner la fenêtre en page initiale :** Ajuste la fenêtre du document pour l’ajuster en douceur autour de la page d’ouverture, en fonction des options sélectionnées sous Options de Document.

**Centrer la fenêtre sur l&#39;écran :** Positionne la fenêtre au centre de la zone d’écran.

**Ouvrir en mode Plein écran :** Agrandit la fenêtre de document et affiche le document sans les commandes de la barre de menus, de la barre d’outils ou de la fenêtre.
