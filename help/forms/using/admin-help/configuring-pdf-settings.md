...
**** Qualité de la presse : Crée des fichiers PDF pour une impression de haute qualité (par exemple, sur une imageuse ou une plaquette). Dans ce cas, la taille de fichier n’est pas prise en compte. L’objectif est de gérer toutes les informations dans un fichier PDF, dont un imprimeur commercial ou un prestataire de services de pré-presse a besoin pour imprimer correctement le document. Cet ensemble d’options :
...
**** Compression au niveau de l’objet : Regroupe les petits objets (qui ne sont pas compressibles eux-mêmes) dans des flux qui peuvent être compressés efficacement.
...
***Remarque ** : si Traiter les commentaires DSC est sélectionnée dans les paramètres Avancées et si des %%commentaires relatifs à l’orientation de l’affichage sont inclus, ces derniers sont prioritaires dans le choix de l’orientation de la page.*
...
***Remarque ** : l’augmentation de la valeur du paramètre de résolution entraîne l’accroissement de la taille du fichier et peut légèrement faire accroître la durée nécessaire au traitement de certains fichiers.*
...
***Remarque ** : la compression du texte et des dessins au trait est toujours activée.*
...
**** Balisez tout pour la gestion des couleurs : Incorpore un profil International Color Consortium lors de la conversion de fichiers et étalonne la couleur des images, ce qui rend les couleurs des fichiers PDF obtenus indépendantes du périphérique si vous avez sélectionné la compatibilité Acrobat 4 (PDF 1.3) ou versions ultérieures. Cependant, les espaces colorimétriques dépendants du périphérique des fichiers (RVB, Niveaux de gris et CMJN) sont convertis en espaces colorimétriques de référence (CalRVB, CalGray et LAB).
...
***Remarque ** : dans tous les cas, il est possible d’ignorer ou de remplacer les modes par des opérations de gestion des couleurs qui surviennent après la création du fichier PDF.*
...
***Remarque ** : le fait de sélectionner Aucun pour les trois espaces de travail revient à sélectionner Reproduire les couleurs.*
...
**** Conserver les informations sur le document de DSC : Conserve des informations telles que le titre, la date et l’heure de création. Lorsque vous ouvrez un fichier PDF dans Acrobat, ces informations s’affichent dans le panneau de description des propriétés du document.
...
**** Si Aucune Zone De Rognage Ou Zone D’Illustration N’Est Spécifiée : Disponible lorsque l’option Norme de conformité de la page Rapports et conformité aux normes est définie sur une option autre qu’Aucune.

**** Signaler comme erreur : Marque le fichier PostScript comme non conforme si l’une des options de création de rapports est sélectionnée et qu’une zone de rognage ou une zone graphique est absente d’une page.
...
**** Si BleedBox N’Est Pas Spécifié : Disponible lorsque l’option Norme de conformité de la page Rapports et conformité aux normes est définie sur une option autre qu’Aucune.
...
**** Valeurs Par Défaut Si Elles Ne Sont Pas Spécifiées Dans Le Document : Cette option est disponible lorsque la norme de conformité de la page Rapports et conformité aux normes est définie sur une option autre qu’Aucune.
...
**** Annuler la tâche : Crée un fichier PDF uniquement si le fichier PostScript est conforme à la norme PDF/A et s’il est valide.

**** Nom du profil du mode de sortie : Indique la condition d’impression caractérisée pour laquelle le document a été préparé et est requise pour la conformité à la norme PDF/A. Si votre flux de travail nécessite de connaître le mode de sortie, sélectionnez Aucun. Le document ne pourra pas vérifier la conformité si cette information n’est pas fournie.
...
**** Ouvrir à la page : Définit la page à laquelle le document s’ouvre, qui est généralement la page 1.

***Remarque ** : les valeurs par défaut des options relatives au zoom et à la mise en page sont celles des options des utilisateurs définies dans les préférences d’affichage des pages dans Acrobat ou Adobe Reader.*
...
**** Redimensionner la fenêtre sur la page initiale : Ajuste la fenêtre du document en fonction des options que vous avez sélectionnées sous Options du document pour l’ajuster en douceur autour de la page d’ouverture.
...
***Remarque ** : si vous masquez la barre de menus et la barre d’outils, les utilisateurs sont dans l’impossibilité d’appliquer des commandes et de sélectionner des outils à moins d’en connaître le raccourci clavier lorsqu’ils ouvrent le fichier dans Acrobat.*
