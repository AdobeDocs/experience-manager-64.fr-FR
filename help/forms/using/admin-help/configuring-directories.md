...
**Liaison** (obligatoire) indique le mode d’accès à l’annuaire.
...
**Nom :** Nom pouvant être utilisé pour se connecter à la base de données LDAP lorsque l’accès anonyme n’est pas activé. Pour Principale Directory 2003, spécifiez `[domain name]\[userid]`. Pour Sun™ One, eDirectory ou IBM Tivoli Directory Server, spécifiez le nom qualifié complet de l’utilisateur, comme uid=lcuser,ou=it,o=company.com.
...
**Renseigner la page avec :** Lorsque cette option est sélectionnée, les attributs des pages de paramètres Utilisateur et Groupe sont renseignés avec les valeurs LDAP par défaut correspondantes.

**Récupérer les DN de base :** Récupère les ND de base et les affiche dans la liste déroulante. Ce paramètre est utile lorsque vous avez plusieurs DN de base et que vous devez sélectionner une valeur.

**Activer la référence :** Ce paramètre s’applique lorsque votre organisation utilise plusieurs domaines Principal Directory organisés dans une structure hiérarchique et que vous avez spécifié des paramètres d’annuaire pour le domaine parent uniquement. Dans ce cas, lorsque vous sélectionnez cette option, User Management peut accéder aux détails des utilisateurs et des groupes à partir des domaines enfants.
...
**Modifier l’horodatage :** Pour activer la synchronisation d’annuaires delta, définissez cette valeur pour modifier TimeStamp. Voir Activation de la synchronisation d’annuaires delta.

**Organisation :** attribut de schéma pour le nom de l’organisation à laquelle appartient l’utilisateur.

**Principal Email :** attribut de schéma pour l’adresse email Principale de l’utilisateur.

**Email Secondaire :** attribut de schéma pour l’adresse électronique secondaire de l’utilisateur.
...
**Paramètres régionaux :** attribut de schéma contenant les informations des paramètres régionaux ISO. La valeur de cet attribut est un code de langue à deux lettres ou un code de langue et de pays.
...
>[!NOTE]
>
>pour activer le contrôle VLV, configurez Sun One. Voir [Configuration de User Management pour l’utilisation du mode Liste virtuelle (VLV)](configuring-directories.md#configure-user-management-to-use-virtual-list-view-vlv).

**Champ de tri :** Si vous avez sélectionné Activer le contrôle VLV (Virtual List View), indiquez le nom d’attribut utilisé pour trier l’index. Il s’agit du nom de l’attribut (uid, par exemple) spécifié lors de la création d’un index pour VLV sur le serveur d’annuaire.

### Paramètres du groupe {#group-settings}

**Identifiant unique :** (obligatoire) attribut unique et constant utilisé pour identifier les groupes. Utilisez un attribut non DN comme identificateur unique. Ce paramètre dépend du serveur d’annuaire. La valeur est objectGUID pour Active Directory 2003, nsuniqueID pour Sun One et guid pour eDirectory.

>[!NOTE]
>
>vérifiez que vous spécifiez un attribut unique au sein de votre entreprise. La saisie d’une valeur incorrecte peut en effet entraîner de graves dysfonctionnements au niveau du système.

**ND de base :** (obligatoire) nom unique de base de l’annuaire.
...
**Identifiant unique de membre :** Identifiant unique d’un utilisateur ou d’un groupe membre du groupe sélectionné. La valeur de ce paramètre dépend du serveur d’annuaire. La valeur est objectSID pour Active Directory 2003, nsuniqueID pour Sun One et guid pour eDirectory.
...
**Principal Email :** attribut de schéma pour l’adresse email Principale du groupe.

**Email Secondaire :** attribut de schéma pour l’adresse électronique secondaire du groupe.
...
**Nom du champ de tri :** Si vous avez sélectionné Activer le contrôle VLV (Virtual List View), indiquez le nom d’attribut utilisé pour trier l’index. Il s’agit du nom de l’attribut spécifié lors de la création d’un index pour VLV sur le serveur d’annuaire.
...
1. A l’aide de l’outil vlvindex, générez l’index en saisissant :
   ...
   *instance de serveur de répertoire* `\vlvindex.bat -n userRoot -T lcuser`
   ...
   L’outil vlvindex figure dans le répertoire de l’instance du serveur d’annuaire. Si le serveur Sun ONE possède deux instances exécutant server1 et server2, alors l’outil vlvindex se situe dans le *répertoire annuaire serveur Sun ONE*\server1. La valeur du paramètre `-T` est la valeur de la variable `cn` de l’entrée vlvindex créée précédemment dans l’exemple LDIF. En l’occurrence, il s’agit de `lcuser`.
...
1. Si le contrôle VLV est activé pour les groupes, créez l’index correspondant pour les groupes. Vérifiez que les index ont été créés en exécutant la commande suivante :
   ...
   *annuaire de serveurs sun* `\shared\bin>ldapsearch -h`*hostname* `-p`*port no* `-s base -b "" objectclass=*`
