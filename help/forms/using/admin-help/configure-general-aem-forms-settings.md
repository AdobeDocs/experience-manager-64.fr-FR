...
**Délai d’expiration par défaut de l’élimination du document (secondes)** Durée maximale (en secondes) pendant laquelle un document transmis entre les différents composants d’AEM forms est considéré comme principal. Lorsque ce délai est écoulé, les fichiers utilisés pour stocker ce document peuvent être supprimés. Utilisez ce paramètre pour gérer l’espace disque disponible.

ce paramètre est obligatoire. La valeur par défaut est de 600 secondes.

**Intervalle de balayage du document (secondes)** Durée, en secondes, entre les tentatives de suppression de fichiers devenues inutiles et utilisés pour transmettre des données de document entre les services.
