...
**** Compatibilité : Définit le type de chiffrement à utiliser pour ouvrir un document protégé par mot de passe. L’option Acrobat 3.0 et versions ultérieures utilise un niveau de chiffrement faible, alors que les autres options utilisent un niveau élevé de chiffrement :
...
**** Restriction du document : Les restrictions de document disponibles dépendent de l’option Compatibilité sélectionnée.
...
***Remarque **: si vous oubliez votre mot de passe, il est impossible de le récupérer à partir du document. Il est conseillé de stocker les mots de passe à un autre emplacement en cas d’oubli. De plus, conservez une copie de sauvegarde du document non protégé par mot de passe.*

**** Exiger Un Mot De Passe Pour Ouvrir Le Document : Active les options de mot de passe.
...
**** Confirmer le mot de passe d&#39;ouverture du document : Vérifie que le mot de passe d’ouverture du document est correct.
...
**** Utilisez Un Mot De Passe Pour Restreindre L’Impression Et La Modification Du Document Et De Ses Paramètres De Sécurité : Active les restrictions d’autorisations.
...
**** Basse résolution (150 ppp) : Permet aux utilisateurs d’imprimer le document avec une résolution maximale de 150 ppp. L’impression peut être plus lente car chaque page est imprimée sous forme d’image bitmap. Cette option n’est disponible que si un niveau de chiffrement élevé (Acrobat 5.0, 6.0, 7.0 ou 9.0) est sélectionné.
...
**** Aucun : Empêche les utilisateurs de modifier le document, notamment de remplir les champs de signature et de formulaire.
