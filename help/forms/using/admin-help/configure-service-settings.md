...
1. Si le service dispose d’un onglet Configuration , utilisez-le pour modifier les paramètres du service. Pour plus d’informations, voir la liste des liens ci-dessous.
   ...
   >[!NOTE]
   >
   >Tous les services répertoriés dans la page Gestion des services ne disposent pas d’un onglet Configuration . Pour les processus que vous avez créés, l’onglet Configuration ne s’affiche que si vous avez ajouté un paramètre de configuration au processus dans Workbench. (Voir &quot;Paramètres de configuration&quot; dans le * [Aide de Workbench](https://www.adobe.com/go/learn_aemforms_workbench_63_fr) .) *
...
De nouveaux paramètres peuvent être créés à l’aide de l’interface utilisateur de PDF Generator.
...
Reprise de la conversion du PDF si le temps consommé pour la première tentative de conversion est inférieur au délai d’expiration spécifié. Par exemple, si le délai d’expiration est de 270 s et que la première tentative a duré 200 s, PDF Generator effectue une nouvelle tentative de conversion. Si la première tentative elle-même a duré 270 s, la conversion ne sera pas retentée.

## Paramètres du service Guides ES4 Utilities {#guides-es4-utilities-service-settings}

Lorsque vous créez un guide, certaines ressources, telles que la définition du guide, sont intégrées au guide. Les ressources peuvent également exister en tant que références aux ressources d’application stockées localement ou sur le serveur AEM forms. Le Guide ne contient pas de données, et les valeurs de l’emplacement et des entrées d’envoi ne sont pas adaptées à tous les environnements externes.
...
**macRefreshIntervalInMinutes :** durée d’activité d’une clé. Lorsqu’une clé a été principale pour cet intervalle, une nouvelle clé est générée. La nouvelle clé devient la principale clé. La clé précédemment principale est conservée pendant 10 % de l’intervalle d’actualisation. Ce comportement permet aux URL générées à l’aide de l’ancienne clé de continuer à fonctionner sur l’ensemble du commutateur de clés. La valeur par défaut est 144000.
...
1. Dans la liste Demander aux appelants de s’authentifier , sélectionnez Oui ou Non pour indiquer si le service peut être appelé avec ou sans informations d’identification.
   ...
   Si vous sélectionnez Oui, l’appelant du service doit être authentifié et l’entité de sécurité de l’utilisateur de cet appelant doit être autorisée à appeler le service ; sinon, la tentative d’appel sera refusée.
