...
description: Découvrez comment configurer des points de fin de courrier électronique.
...
**Modèle de fichier :** indique les modèles de pièce jointe de fichier entrant acceptés par le fournisseur. Les fichiers portant des extensions particulières (*.dat, *.xml), des noms spécifiques (data) et contenant des expressions composites dans leur nom et leur extension (.[dD][aA][Tt]). La valeur par défaut est &amp;ast;.&amp;ast;.
...
**Modèle de fichier :** indique les modèles de pièce jointe de fichier entrant qui sont acceptés par le fournisseur. Cela inclut les fichiers ayant des extensions spécifiques (&amp;ast;.dat, &amp;ast;.xml), des noms spécifiques (data) ou des expressions composites dans le nom et l’extension (&amp;ast;.[dD][aA][Tt]).
...
Pour désactiver ce paramètre, laissez le paramètre vide. Dans certains cas, vous souhaitez déclencher un processus et ne souhaitez pas recevoir de notification par email du résultat.
