...
1. Sélectionnez les paramètres par défaut souhaités. Dans la page Définir les paramètres par défaut, un ou plusieurs des paramètres suivants sont disponibles :
   ...
   **[!UICONTROL Paramètres de protection]**: La valeur par défaut d’origine est No Security (Acrobat 5).
...
**Extensions de nom de fichier :** Liste séparée par des virgules des extensions de nom de fichier qui peuvent être converties.

**Essayer le convertisseur de secours :** PDF Generator peut utiliser Java™ ou Acrobat pour convertir des fichiers image en PDF. Lorsque cette option est sélectionnée et qu’une conversion échoue ou atteint le délai d’expiration spécifié, PDF Generator tente d’effectuer la conversion en utilisant la méthode alternative. Si la méthode alternative échoue ou atteint le délai d’expiration spécifié, une exception est consignée dans le fichier journal.

>[!NOTE]
>
>les fichiers JPEG 2000 ne peuvent être convertis qu’avec Acrobat.

**Utiliser la reconnaissance optique des caractères :** Indique s’il faut appliquer la reconnaissance optique des caractères (OCR) au PDF. Un logiciel d’OCR vous permet de rechercher, corriger et copier du texte dans un fichier PDF.

***Remarque ** : la fonction OCR pour les fichiers PDF (PDF indexables) est uniquement prise en charge sous Microsoft Windows.*

**Langue OCR Principal :** Spécifie la langue à utiliser par le moteur OCR pour identifier les caractères.

**Style de sortie du PDF :** Détermine le type de PDF à produire. Tous les formats appliquent la reconnaissance optique de caractères, ainsi que la reconnaissance de polices et de pages, aux images textuelles et les convertissent en texte normal.

**Image indexable :** Vérifie que le texte peut faire l’objet d’une recherche et peut être sélectionné. Cette option conserve l’image d’origine, corrige son décalage au besoin et place un calque de texte invisible dessus. L’option Sous-échantillonner les images détermine si l’image est sous-échantillonnée et dans quelle mesure.

**Image indexable (exacte) :** Vérifie que le texte peut faire l’objet d’une recherche et peut être sélectionné. Cette option conserve l’image d’origine et place un calque de texte invisible dessus. Elle est recommandée dans les situations où la fidélité à l’image d’origine est une priorité.

**ClearScan :** synthétise une nouvelle police Type 3 qui se rapproche étroitement de l’original et conserve l’arrière-plan de la page en utilisant une copie à basse résolution.
...
**Essayer le convertisseur de secours :** PDF Generator peut utiliser Java™ ou Acrobat pour convertir des fichiers de HTML en PDF. Lorsque cette option est sélectionnée et qu’une conversion échoue ou atteint le délai d’expiration spécifié, PDF Generator tente d’effectuer la conversion en utilisant la méthode alternative. Si la méthode alternative échoue ou atteint le délai d’expiration spécifié, une exception est consignée dans le fichier journal.
...
**Forcer le codage sélectionné :** Ignore tout codage spécifié dans le fichier source du HTML et utilise la sélection affichée dans l’option Codage par défaut .
...
**Obtenir uniquement X niveaux :** Développe et convertit les pages jusqu’à une profondeur du niveau spécifié à partir de l’URL de la page de base. Une valeur égale à 1 permet de convertir uniquement l’URL indiquée.
...
**Rester sur le même serveur :** Les liens pointant vers des pages de différents serveurs ne sont pas convertis lors de l’indexation. Seuls les liens renvoyant au même serveur que l’URL définie sont convertis.
...
**Orientation :** Sélectionnez portrait ou paysage pour le document de PDF converti.

**Marges :** Spécifie les marges (Haut, Bas, Gauche et Droite) dans le document de PDF généré.

**Ajouter Des Signets À PDF :** Ajoute des signets au document du PDF.

**Activer le PDF balisé :** Incorpore des balises dans le document du PDF.

**Définissez les paramètres d’affichage initial :** Permet de configurer les options Document, Window et User Interface. Ces paramètres déterminent le mode d’affichage initial du contenu.
...
**Afficher :** Sélectionnez les volets à ouvrir dans Acrobat à l’ouverture du document du PDF.

**Mise en page :** Sélectionnez le type de mise en page du document du PDF.

**Zoom :** Sélectionnez un agrandissement prédéfini pour la vue initiale du document du PDF ou une valeur personnalisée. Le choix d’un paramètre par défaut implique l’utilisation du zoom par défaut d’Acrobat.
...
**Masquer la barre de menus :** Masque la barre de menus dans le document du PDF.

**Masquer les barres d’outils :** Masque les barres d’outils dans le document du PDF.

**Masquer les commandes de fenêtre :** Masque les commandes de fenêtre dans le document du PDF.
...
**Extensions de nom de fichier :** Liste de toutes les extensions de nom de fichier XPS pouvant être converties, séparées par des virgules. Il n’existe actuellement un seul format : .xps.
...
**Version du PDF Target :** Indique la version d’Acrobat avec laquelle le PDF est compatible.
...
1. Pour incorporer uniquement les jeux partiels utilisés des polices incorporées, sélectionnez **Créer des jeux partiels de toutes les polices incorporées**.
   ...
   >[!NOTE]
   >
   >*Si vous utilisez cette option conjointement avec **Désincorporer certaines polices**, polices dans le **Ajout de polices à désincorporer**Les listes sont toujours complètement désincorporées.*
...
**Texte et illustration au trait** Résolution à laquelle tous les objets, y compris les images, les illustrations vectorielles, le texte et les dégradés, sont pixellisés. Cette valeur peut être comprise entre 1 et 9600 pixels par pouce (ppp).
...
**Dégradé et filets** Résolution de pixellisation des dégradés et filets. Cette valeur doit être comprise entre 1 et 1200 ppp.
...
**Convertir tous les contours en contours** Convertit tous les contours en chemins remplis simples sur les pages contenant de la transparence. Cette option préserve la largeur des contours lors de l’aplatissement. Notez que l’activation de cette option a pour effet de donner aux contours fins un aspect légèrement plus épais et peut dégrader les performances de l’aplatissement.

**Ecrêter les régions complexes** Veille à ce que les limites entre l’illustration vectorielle et l’illustration pixellisée s’étendent le long des chemins d’accès à l’objet. Cette option réduit les artefacts de raccordement qui se produisent lorsqu’ils font partie d’un journal.
...
**Ajout De Signets À Adobe PDF**: permet de convertir les noms de feuilles de calcul Excel en signets. Cette option est sélectionnée par défaut.

**Ajuster une feuille de calcul à une seule page**: Réduit la taille du texte pour l’adapter à la feuille de calcul sur une seule page.

**Convertir l’intégralité du classeur**: permet de convertir toutes les feuilles de calcul du fichier Excel. Si cette option n’est pas sélectionnée seule la page en cours est convertie.

**Exécution automatique des macros**: exécute les macros du document Excel (par exemple une macro qui insère l’heure actuelle) avant de le convertir.

**Convertir les informations sur le document**: Ajoute des propriétés de document PDF en fonction des informations de document dans le fichier source. Cela inclut des informations telles que le titre, l’auteur, le sujet et les mots-clés du document.
...
**Joindre le fichier source à Adobe PDF**: Lorsque cette option est sélectionnée, la feuille de calcul Excel d’origine est insérée en pièce jointe dans le document de PDF généré.

**Activation De L’Accessibilité Et De Reflow Avec Tagged Adobe PDF**: Incorpore des balises dans le document du PDF afin d’activer l’accessibilité et la redistribution.

**Liste des compléments Excel à charger**: Par défaut (pour des raisons de sécurité), aucun complément Excel n’est exécuté lorsqu’un fichier Excel est converti en PDF. Pour permettre l’exécution de certains compléments Excel au moment de la conversion, fournissez une liste séparée par des virgules indiquant le nom de ces compléments.

**Liste des feuilles de calcul à convertir**: Si cette zone est vide, toutes les feuilles de la feuille de calcul Excel sont incluses dans le PDF généré. Pour convertir un sous-ensemble spécifique des feuilles, fournissez une liste séparée par des virgules indiquant leur nom.
...
1. **[!UICONTROL Extensions de nom de fichier :]** Spécifie les extensions de nom de fichier acceptées pour cette application, séparées par des virgules. La valeur par défaut est de `mpp`. N’insérez pas de point dans les extensions ou d’espace entre celles-ci.
...
1. **[!UICONTROL Exécution automatique des macros]**: exécute les macros du document Projet Microsoft (par exemple une macro qui insère l’heure actuelle) avant de le convertir.
...
**Codage Acrobat PDFMaker :** spécifie le type de codage des fichiers d’entrée de texte brut dans Acrobat PDFMaker. Par exemple, si vous utilisez un fichier codé UTF-8, sélectionnez UTF-8 pour obtenir les meilleurs résultats.
...
**Ajout De Liens À Adobe PDF**: Permet de conserver tous les liens. Cette option est sélectionnée par défaut.
...
**Créer un fichier compatible avec le PDF/A-1b**: Force l’utilisation du paramètre Adobe PDF PDF/A-1b:2005 (RGB).

**Convertir les commentaires en commentaires Adobe PDF**: permet de convertir les notes Visio en commentaires de PDF.
...
## Autres paramètres de l’application (Windows uniquement) {#other-applications-settings-windows-only}
