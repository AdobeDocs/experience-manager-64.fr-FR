...
* Un dossier de contrôle possède plusieurs propriétés, voir [Propriétés du dossier de contrôle](/help/forms/using/watched-folder-in-aem-forms.md#watchedfolderproperties).
...
1. Appuyer **Avancé**. L’onglet Avancé contient d’autres champs. La plupart de ces champs contiennent une valeur par défaut.
   ...
   * **Exclure le modèle de fichier** : indiquez une liste délimitée par des points-virgules (;) de modèles utilisés par un dossier de contrôle pour déterminer les fichiers et les dossiers à analyser et à sélectionner. Aucun fichier ou dossier avec le modèle spécifié n’est analysé en vue du traitement. Pour plus d’informations sur les modèles de fichiers, voir [A propos des modèles de fichier](/help/forms/using/admin-help/configuring-watched-folder-endpoints.md#about-file-patterns).
