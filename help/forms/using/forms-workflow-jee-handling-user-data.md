...
Un workflow peut être conçu de telle sorte que les données utilisateur soient capturées dans une variable qui est stockée en tant qu’objet Blob dans la base de données. Dans ce cas, vous pouvez interroger les données utilisateur uniquement si elles sont stockées dans l’une des variables de type primitif suivantes :
...
1. Vérifiez l’état de l’instance de flux de travail. Si l’état est différent de 2 (COMPLETE) ou 4 (TERMINÉ), arrêtez d’abord l’instance en appelant la méthode suivante :
...
1. Selon que votre stockage global de document pointe vers un système de fichiers ou une base de données, effectuez l’une des opérations suivantes :
   ...
   1. **Stockage global de document dans le système de fichiers** 
      ...
      1. Recherchez des fichiers avec les chaînes d’ID de session suivantes comme extensions :
      * `_wfattach<task_id>`
      * `_wftask<fd_id>`
      * `_wftaskformid<fd_id>`
