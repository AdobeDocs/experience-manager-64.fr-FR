...
Une fois les informations sur lʼutilisateur définies, vous pouvez lʼajouter à AEM Forms. Pour ajouter un utilisateur, appelez la méthode `createLocalUser` de lʼobjet `DirectoryManagerServiceClient`.
...
1. Spécifiez les informations relatives à l’utilisateur.
   ...
   * Définissez la valeur du nom de famille en attribuant une valeur de chaîne au champ `familyName` de lʼobjet `UserImpl`.
...
1. Vérifiez que l’utilisateur a été ajouté.
   ...
   * Créez un objet `PrincipalSearchFilter` en utilisant son constructeur.
   * Définissez la valeur de l’identifiant de l’utilisateur en attribuant une valeur de chaîne qui représente cette valeur au champ `userId` de lʼobjet `PrincipalSearchFilter`.
...
Pour supprimer un utilisateur, appelez la méthode `deleteLocalUser` de lʼobjet `DirectoryManagerServiceClient`.
...
1. Supprimez l’utilisateur d’AEM Forms.
   ...
   Appeler la méthode `deleteLocalUser` de lʼobjet `DirectoryManagerServiceClient` et transmettez la valeur du champ `oid` de lʼobjet `User`. Appeler la méthode `getOid` de lʼobjet `User`. Utilisez lʼobjet `User` récupéré dans l’instance `java.util.List`.
...
1. Supprimez l’utilisateur d’AEM Forms.
   ...
   Supprimez l’utilisateur en transmettant la valeur du champ `oid` de lʼobjet `User` à la méthode `deleteLocalUser` de lʼobjet `DirectoryManagerServiceClient`.
...
1. Créez le groupe.
   ...
   * Appelez la méthode `setDomainName` de l’objet `Group` et transmettez une valeur de chaîne qui indique le nom de domaine.
...
1. Créez un client DirectoryManagerService.
   ...
   Créez un objet `DirectoryManagerServiceService` à l’aide de votre constructeur de classe proxy.
...
1. Appelez l’opération d’authentification.
   ...
   * Un objet `java.lang.String` contenant le nom de l’utilisateur.
   * Un tableau d’octets (un objet `byte[]`) contenant le mot de passe de l’utilisateur. Vous pouvez obtenir l’objet `byte[]` en appelant la méthode `getBytes` de l’objet `java.lang.String`.
...
1. Récupérez le contexte d’authentification.
   ...
   Appelez ensuite la méthode `initPrincipal` de l’objet `Context` et transmettez le `AuthResult`.
...
1. Appelez l’opération d’authentification.
   ...
   * Un objet `string` contenant le nom de l’utilisateur
   * Un tableau d’octets (un objet `byte[]` ) contenant le mot de passe de l’utilisateur. Vous pouvez obtenir l’objet `byte[]` en convertissant un objet `string` contenant le mot de passe en un tableau `byte[]` en utilisant la logique affichée dans l’exemple ci-dessous.
   * La valeur renvoyée est un objet `AuthResult`, qui peut être utilisé pour récupérer des informations sur l’utilisateur. Dans l’exemple ci-dessous, les informations de l’utilisateur sont récupérées en obtenant d’abord le champ `authenticatedUser` de l’objet `AuthResult` et ensuite les champs `canonicalName` et `domainName` de l’objet résultant `User`.
...
Après avoir défini un ou plusieurs domaines d’entreprise, vous pouvez effectuer l’opération de synchronisation. Le temps nécessaire à l’exécution de cette opération dépend du nombre d’enregistrements d’utilisateurs qui se trouvent dans le référentiel d’utilisateurs.
