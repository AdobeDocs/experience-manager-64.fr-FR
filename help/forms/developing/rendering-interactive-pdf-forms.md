---
title: Rendu de formulaires PDF interactifs
seo-title: Rendu de formulaires PDF interactifs
description: 'null'
...
Un exemple de demande de prêt est présenté pour montrer comment le service Forms utilise les formulaires interactifs pour collecter des informations auprès des utilisateurs. Cette application permet à un utilisateur de remplir un formulaire avec les données requises pour sécuriser un prêt, puis d’envoyer les données au service Forms. Le diagramme suivant illustre le flux logique de la demande de prêt.
...
<table> 
 ...
 <tbody>
  <tr> 
   <td><p>1</p></td> 
   <td><p>Le servlet <code>GetLoanForm</code> Java est appelé à partir d’une page HTML. </p></td> 
  </tr> 
  <tr> 
   <td><p>2</p></td> 
  ...
  <tr> 
   <td><p>3</p></td> 
   <td><p>Une fois que l’utilisateur remplit le formulaire de prêt et clique sur le bouton d’envoi, les données sont envoyées au servlet <code>HandleData</code> Java. (Voir <i>"Formulaire de prêt"</i>.)</p></td> 
  </tr> 
  <tr> 
   <td><p>4</p></td> 
...
Ce formulaire interactif de prêt est généré par le servlet `GetLoanForm` Java de la demande de prêt d’exemple.

![ri_ri_loanform](assets/ri_ri_loanform.png)

**Formulaire de confirmation**

Ce formulaire est généré par le servlet `HandleData` Java de la demande de prêt.

![ri_ri_verify](assets/ri_ri_confirm.png)

Le servlet `HandleData` Java prérempli ce formulaire avec le prénom et le nom de l’utilisateur, ainsi que le montant. Une fois le formulaire prérempli, il est envoyé au navigateur Web client. (Voir [Préremplissage de formulaires avec des mises en page](/help/forms/developing/prepopulating-forms-flowable-layouts.md)à disposition souple)

**Servlets Java**

L’exemple de demande de prêt est un exemple d’application de service Forms qui existe en tant que servlet Java. Un servlet Java est un programme Java exécuté sur un serveur d’applications J2EE, tel que WebSphere, et contient le code de l’API du client du service Forms.

Le code suivant illustre la syntaxe d’une servlet Java appelée GetLoanForm :
...
Normalement, vous ne placez pas le code API du client du service Forms dans une `doGet` ou une `doPost` méthode Java Servlet. Il est plus pratique de programmer de placer ce code dans une classe distincte, d&#39;instancier la classe à partir de la `doPost` (ou `doGet` méthode), et d&#39;appeler les méthodes appropriées. Toutefois, pour la concision du code, les exemples de code de cette section sont réduits au minimum et les exemples de code sont placés dans la `doPost` méthode.
...
Avant de pouvoir exécuter par programmation une opération d’API du client de service Forms, vous devez créer un objet API du client Forms. Si vous utilisez l’API Java, créez un `FormsServiceClient` objet. Si vous utilisez l’API du service Web de Forms, créez un `FormsService` objet.

**Spécifier des valeurs URI**

Vous pouvez spécifier les valeurs URI requises par le service Forms pour générer un formulaire. Une conception de formulaire enregistrée dans le cadre d’une application Forms peut être référencée à l’aide de la valeur URI racine du contenu `repository:///`. Prenons l’exemple de la conception de formulaire suivante nommée *Loan.xdp* située dans une application Forms appelée *FormsApplication*:

![ri_ri_formrepository](assets/ri_ri_formrepository.png)

Pour accéder à cette conception de formulaire, spécifiez `Applications/FormsApplication/1.0/FormsFolder/Loan.xdp` le nom du formulaire (premier paramètre transmis à la `renderPDFForm` méthode) et `repository:///` la valeur URI racine du contenu.
...
Lorsque vous générez un formulaire interactif, vous pouvez définir des valeurs URI telles que l’URL cible vers laquelle les données du formulaire sont publiées. L’URL cible peut être définie de l’une des manières suivantes :
...
Si l’URL cible est définie dans la conception de formulaire, ne la remplacez pas par l’API Client du service Forms. En d’autres termes, la définition de l’URL cible à l’aide de l’API Forms réinitialise l’URL spécifiée dans la conception de formulaire à celle spécifiée à l’aide de l’API. Si vous souhaitez envoyer le formulaire PDF à l’URL cible spécifiée dans la conception de formulaire, définissez par programmation l’URL cible sur une chaîne vide.

Si vous disposez d’un formulaire contenant un bouton d’envoi et un bouton calculate (avec un script correspondant qui s’exécute sur le serveur), vous pouvez définir par programmation l’URL vers laquelle le formulaire est envoyé pour exécuter le script. Utilisez le bouton d’envoi sur la conception de formulaire pour spécifier l’URL vers laquelle les données du formulaire sont publiées. (Voir [Calcul des données](/help/forms/developing/calculating-form-data.md)de formulaire.)
...
L’objet `renderPDFForm` utilise un `URLSpec` objet. La racine de contenu du fichier XDP est transmise au service Forms à l’aide de la `URLSpec` `setContentRootURI` méthode de l’objet. Le nom de la conception de formulaire ( `formQuery`) est transmis en tant que valeur de paramètre distincte. Les deux valeurs sont concaténées pour obtenir la référence absolue à la conception de formulaire.
...
1. Joindre des fichiers au formulaire
   ...
   * Appelez la méthode `java.util.HashMap` `put` de l’objet pour chaque fichier à joindre au formulaire généré. Transmettez les valeurs suivantes à cette méthode :

      * Valeur de chaîne qui spécifie le nom du fichier joint, y compris l’extension du nom de fichier.
   * Objet `com.adobe.idp.Document` contenant le fichier joint.
   >[!NOTE]
   >
