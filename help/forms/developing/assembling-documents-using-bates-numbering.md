...
Aux fins de cette discussion, l’identifiant de page unique est placé dans l’en-tête d’un document. Supposons que le document DDX suivant soit utilisé.
...
Ce document DDX fusionne deux documents PDF nommés *map.pdf* et* directions.pdf* en un seul document PDF. Le document PDF résultant contient un en-tête composé d’un identifiant de page unique. Par exemple, le document de l’illustration ci-dessus montre 000016.

>[!NOTE]
>
>Avant de lire cette section, il est recommandé de vous familiariser avec l’assemblage de documents PDF à l’aide du service Assembler. Cette section ne traite pas des concepts, tels que la création d’un objet de collection contenant des documents d’entrée ou l’extraction des résultats de l’objet de collection renvoyé. (Voir Assemblage [programmatique de documents](/help/forms/developing/programmatically-assembling-pdf-documents.md)PDF.)
...
Pour assembler un document PDF contenant un identifiant de page unique (numérotation Bates), procédez comme suit :
...
Un document DDX doit être référencé pour assembler un document PDF. Prenons l’exemple du document DDX présenté dans cette section. Pour assembler un document PDF contenant des identifiants de page uniques, le document DDX doit contenir l’ `BatesNumber` élément.

**Documents PDF d’entrée de référence**

Les documents PDF d’entrée doivent être référencés pour assembler un document PDF. Par exemple, les documents map.pdf et directions.pdf doivent être référencés pour assembler ces documents PDF dans un document PDF unique.

**Définir la valeur du nombre Bates initial**
...
Après avoir créé le client du service Assembler, fait référence au document DDX qui contient des informations sur `BatesNumber` les éléments, fait référence à un document PDF d’entrée et défini des options d’exécution, vous pouvez appeler l’ `invokeDDX` opération qui entraîne l’assemblage d’un document PDF contenant des identifiants de page uniques par le service Assembler.

**Extraire les résultats**

Le service Assembler renvoie un objet de collection contenant les résultats de la tâche. Vous pouvez extraire le document PDF résultant et les exceptions qui sont générées. Dans ce cas, un document PDF chiffré se trouve dans l’objet Collection.

>[!NOTE]
>
>Un objet de collection est renvoyé si vous appelez l’ `invokeDDX` opération. Cette opération est utilisée lors de la transmission de plusieurs documents PDF d’entrée au service Assembler. Toutefois, si vous ne transmettez qu’un seul document PDF d’entrée au service Assembler, vous devez appeler l’ `invokeOneDocument` opération. Pour plus d’informations sur l’utilisation de cette opération, voir [Assemblage de documents](/help/forms/developing/assembling-encrypted-pdf-documents.md)PDF chiffrés.
...
Assemblez un document PDF qui utilise des identifiants de page uniques (numérotation Bates) à l’aide de l’API du service Assembler (Java) :
...
1. Extrayez les résultats.
   ...
   * Appelez la méthode `AssemblerResult` `getDocuments` de l’objet. Cette action renvoie un `java.util.Map` objet.
...
Assemblez un document PDF qui utilise des identifiants de page uniques (numérotation Bates) à l’aide de l’API du service Assembler (service Web) :
...
1. Définissez la valeur initiale du nombre Bates.
   ...
   * Créez un `AssemblerOptionSpec` objet qui stocke les options d’exécution à l’aide de son constructeur.
   * Définissez le nombre Bates initial en attribuant une valeur numérique au membre de `firstBatesNumber` données qui appartient à l’ `AssemblerOptionSpec` objet.
