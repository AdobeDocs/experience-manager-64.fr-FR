...
* Valider les signatures numériques situées dans un document PDF. (Voir [Vérifier les signatures numériques](#unresolvedlink-lc-si).)
* Valider toutes les signatures numériques situées dans un document PDF. (Voir [Vérifier plusieurs signatures numériques](#unresolvedlink-lc-si).)
...
1. Certifier le document PDF
   ...
   * Une valeur de chaîne représentant le nom du champ de signature qui contient la signature.
   * Objet `Credential` représentant les informations d’authentification utilisées pour certifier le document PDF. Créez un objet `Credential` en utilisant son constructeur, puis spécifiez l’alias en affectant une valeur à la propriété `Credential` de l’objet `alias`.
   ...
   * Valeur string représentant les informations de contact du signataire.
   * Membre de données statique de l’objet `MDPPermissions` spécifiant les actions pouvant être effectuées sur le document PDF qui invalide la signature.
...
Les signatures numériques peuvent être vérifiées pour vous assurer qu’un document PDF signé n’a pas été modifié et que la signature numérique est valide. Lors de la vérification d’une signature numérique, vous pouvez vérifier le statut et les propriétés de la signature, comme l’identité du signataire. Avant d’approuver une signature numérique, il est recommandé de la vérifier. Lors de la vérification d’une signature numérique, référencez un document PDF contenant une signature numérique.
...
De même, lorsque vous vérifiez par programmation une signature numérique, vous pouvez déterminer le statut de l’identité du signataire. Par exemple, si vous vérifiez la signature numérique dans le document illustré ci-dessus, il en ressort que l’identité du signataire est inconnue.
...
[Vérifier des signatures numériques à l’aide de l’API Java](#unresolvedlink-lc-si)

[Vérifier des signatures numériques à l’aide de l’API de service web](#unresolvedlink-lc-si)
...
1. [Créer une Signature client](#unresolvedlink-lc-si)
...
[Vérifier des signatures numériques](#unresolvedlink-lc-si)
...
1. Définir les options d’exécution PKI
   ...
   * Créez un objet `PKIOptions` en utilisant son constructeur.
   * Définissez l’heure de vérification en affectant au membre de données `verificationTime` de l’objet `PKIOptions` une valeur d’énumération `VerificationTime` spécifiant l’heure de vérification.
   * Définissez l’option de vérification de révocation en affectant au membre de données `revocationCheckStyle` de l’objet `PKIOptions` une valeur d’énumération `RevocationCheckStyle` qui spécifie s’il faut effectuer une vérification de révocation.
...
1. Déterminer l’identité du signataire
   ...
   * Récupérez le membre de données `status` de l’objet `IdentityInformation` pour déterminer l’identité du signataire. Ce membre de données renvoie une valeur d’énumération `IdentityStatus` indiquant l’identité. Par exemple, si le signataire est fiable, ce membre renvoie `IdentityStatus.TRUSTED`.
...
Effectuez une itération sur chaque signature. Autrement dit, pour chaque signature, vérifiez la signature numérique, l’identité du signataire ainsi que l’état de chaque signature. (Voir [Vérifier des signatures numériques](#unresolvedlink-lc-si).)
...
[Vérifier plusieurs signatures numériques à l’aide de l’API Java](#unresolvedlink-lc-si)

[Vérifier plusieurs signatures numériques à l’aide de l’API Web Service](#unresolvedlink-lc-si)
...
1. Enregistrer le document PDF en tant que fichier PDF
   ...
   * Ecrivez le contenu du tableau d’octets dans le fichier PDF en appelant la méthode `Write` de l’objet `System.IO.BinaryWriter` et en transmettant le tableau d’octets.
