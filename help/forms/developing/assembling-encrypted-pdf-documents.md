...
Le service Encryption ne doit pas nécessairement faire partie de votre installation AEM forms pour chiffrer un document PDF avec un mot de passe. Voir [Chiffrement et déchiffrement de documents](/help/forms/developing/encrypting-decrypting-pdf-documents.md)PDF.
...
Un document DDX doit être référencé pour assembler un document PDF. Prenons l’exemple du document DDX présenté dans cette section. Pour chiffrer un document PDF, le document DDX doit contenir l’ `PasswordEncryptionProfile` élément.
...
Après avoir créé le client de service Assembler, référencez le document DDX contenant des informations de chiffrement, référencez un document PDF non sécurisé et définissez des options d’exécution, vous pouvez appeler l’ `invokeOneDocument` opération. Dans la mesure où un seul document PDF d’entrée est transmis au service Assembler (et qu’un seul document est renvoyé), vous pouvez utiliser l’ `invokeOneDocument` opération au lieu de l’ `invokeDDX` opération.

**Enregistrer le document PDF chiffré**

Si un seul document PDF est transmis au service Assembler, le service Assembler renvoie un document unique au lieu d’un objet de collection. En d’autres termes, lors de l’appel de l’ `invokeOneDocument` opération, un seul document est renvoyé. Le document DDX référencé dans cette section contenant des informations de chiffrement, le service Assembler renvoie un document PDF chiffré avec un mot de passe.
...
1. Enregistrez le document PDF chiffré.
   ...
   * Créez un tableau d’octets qui stocke le contenu de l’ `BLOB` objet renvoyé par la `invokeOneDocument` méthode. Renseignez le tableau d’octets en obtenant la valeur du membre `BLOB` `MTOM` de données de l’objet.
