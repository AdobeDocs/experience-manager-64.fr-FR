...
Vous pouvez assembler un document PDF contenant des signets. Supposons, par exemple, que vous disposiez d’un document PDF qui ne contient aucun signet et que vous souhaitiez le modifier en fournissant des signets. Le service Assembler vous permet de lui transmettre un document PDF qui ne contient pas de signets et de récupérer un document PDF contenant des signets.

Les signets contiennent les propriétés suivantes :

* Titre qui s’affiche sous forme de texte à l’écran.
* Action qui spécifie ce qui se produit lorsqu’un utilisateur clique sur le signet. L’action type d’un signet consiste à se déplacer vers un autre emplacement du document actif ou à ouvrir un autre document PDF, bien que d’autres actions puissent être spécifiées.
...
Dans ce document DDX, notez que la valeur est affectée à l’attribut source `Loan.pdf`. Ce document DDX indique qu’un document PDF unique est transmis au service Assembler. Lors de l’assemblage d’un document PDF avec des signets, vous devez spécifier un document XML de signet qui décrit les signets dans le document cible. Pour spécifier un document XML en signet, assurez-vous que l’ `Bookmarks` élément est spécifié dans votre document DDX.

Dans cet exemple de document DDX, l’ `Bookmarks` élément spécifie `doc2` comme valeur. Cette valeur indique que la carte d’entrée transmise au service Assembler contient une clé nommée `doc2`. La valeur de la `doc2` clé est une `com.adobe.idp.Document` valeur qui représente le document XML du signet. (Voir &quot;Langage des signets&quot; dans le Guide de référence [du service](https://www.adobe.com/go/learn_aemforms_ddx_63)Assembler et du DDX.)
...
Compte tenu du document DDX spécifié dans cette section et du fichier XML de signets en entrée, le service Assembler assemble un document PDF qui contient les signets suivants.
...
1. Référence à un document PDF auquel des signets sont ajoutés.
...
Un document DDX doit être référencé pour assembler un document PDF. Ce document DDX doit contenir l’ `Bookmarks` élément, qui demande au service Assembler d’assembler un PDF contenant des signets. (Voir le document DDX illustré plus haut dans cette section pour un exemple.)

**Référence à un document PDF auquel des signets sont ajoutés**

Référence à un document PDF auquel des signets sont ajoutés. Peu importe que le document PDF référencé contienne déjà des signets. Si l’ `Bookmarks` élément est un enfant de l’élément source PDF, les signets remplaceront ceux qui existent déjà dans la source PDF. Toutefois, si vous souhaitez conserver les signets existants, assurez-vous qu’ `Bookmarks` il s’agit d’un frère de l’élément source PDF. Prenons l’exemple suivant :
...
Pour assembler un PDF contenant de nouveaux signets, vous devez référencer un document XML de signet. Le document XML du signet est transmis au service Assembler dans l’objet de collection Map. (Voir le document XML du signet illustré plus haut dans cette section pour un exemple.)
...
Vous devez ajouter le document PDF auquel les signets sont ajoutés et le document XML de signet à la collection Map. L’objet de collection Map contient donc deux éléments : un document PDF et le document XML du signet.
...
Pour assembler un document PDF contenant de nouveaux signets, utilisez l’ `invokeDDX` opération du service Assembler. La raison pour laquelle vous devez utiliser l’ `invokeDDX` opération par rapport à d’autres opérations du service Assembler, par exemple `invokeOneDocument` , est que le service Assembler requiert un document XML de signet transmis dans l’objet de collection Map. Cet objet est un paramètre de l’ `invokeDDX` opération.

**Enregistrer le document PDF contenant des signets**

Vous devez extraire les résultats de l’objet de mappage renvoyé et enregistrer le document PDF correspondant. (voir la section &quot;Extraction des résultats&quot; dans Assemblage [programmatique de documents](/help/forms/developing/programmatically-assembling-pdf-documents.md)PDF).
...
Assemblez un document PDF avec des signets à l’aide de l’API du service Assembler (Java) :
...
1. Référencez le document XML du signet.
   ...
   * Créez un `com.adobe.idp.Document` objet et transmettez l’ `java.io.FileInputStream` objet qui contient le document PDF.

1. Ajoutez le document PDF et le document XML du signet à une collection Map.

   * Créez un `java.util.Map` objet utilisé pour stocker le document PDF d’entrée et le document XML de signet.
...
1. Assemblez le document PDF.
   ...
   * Objet `AssemblerOptionSpec` spécifiant les options d’exécution
   La `invokeDDX` méthode renvoie un `AssemblerResult` objet qui contient les résultats de la tâche et les exceptions qui peuvent s’être produites.
...
1. Enregistrez le document PDF qui contient des signets.
   ...
   * Extrayez les données binaires qui représentent le document PDF en accédant au `BLOB` `MTOM` champ de son objet. Cette opération renvoie un tableau d’octets que vous pouvez écrire dans un fichier PDF.
