...
* les types de données XML tels que `org.w3c.Document` et `org.w3c.Element`
* Objets de collection tels que `java.util.List` et `java.util.Map`
   ...
   Si un service AEM Forms requiert plusieurs paramètres d’entrée, le corps de la requête HTTP doit être un message MIME en plusieurs parties, tel que défini par la norme RFC 1867. (RFC 1867 est une norme utilisée par les navigateurs Web pour télécharger des fichiers sur des sites Web.) Chaque paramètre d’entrée doit être envoyé en tant que partie distincte du message multipartie et codé au `multipart/form-data` format. Le nom de chaque pièce doit correspondre au nom du paramètre.
...
L’état de l’appel asynchrone peut être récupéré à l’aide d’une URL d’appel avec une valeur de `services` substitution `async_status`. L’URL doit contenir un `job_id` paramètre spécifiant la valeur d’identificateur de la tâche associée à cet appel. Par exemple :
...
Une fois la tâche terminée et le résultat récupéré, la tâche peut être éliminée en utilisant une URL d’appel avec `services` est remplacée par `async_dispose`. L’URL doit également contenir un `job_id` paramètre spécifiant la valeur d’identificateur de la tâche. Par exemple :
...
Si l’URL d’appel (ou l’ `async_result` URL en cas d’appel asynchrone) comporte un suffixe .xml, le fournisseur REST renvoie le code HTTP `200 OK`suivi d’un document XML décrivant l’exception au format suivant.
...
* Transfert de valeurs booléennes à un processus AEM Forms
...
* Transfert de valeurs de énumération à un processus AEM Forms
