...
Il existe deux types de clients de HTML. Le premier client de HTML est un client AJAX écrit en JavaScript. Le deuxième client est un formulaire de HTML contenant un bouton d’envoi. Une application cliente basée sur des HTMLS n’est pas le seul client REST possible. Toute application cliente prenant en charge les requêtes HTTP peut appeler un service à l’aide d’un appel REST. Par exemple, vous pouvez appeler un service à l’aide d’un appel REST à partir d’un formulaire de PDF. (Voir [Appel du processus MyApplication/EncryptDocument à partir d’Acrobat](#rest-invocation-examples).)
...
La variable `ServiceName` est le nom du service Forms à appeler. Le paramètre facultatif `OperationName` value est le nom de l’opération du service. Si cette valeur n’est pas spécifiée, ce nom est défini par défaut sur `invoke`, qui est le nom de l’opération qui lance le processus. Le paramètre facultatif `ServiceVersion` est la version codée au format X.Y. Si cette valeur n’est pas spécifiée, la version la plus récente est utilisée. Le `enctype` peut également être `application/x-www-form-urlencoded`.
...
* Les objets de collection, tels que `java.util.List` et `java.util.Map`
   ...
   >[!NOTE]
   >
   >Le `output-paramater1` représente le nom du paramètre de sortie.

   Si un service Forms requiert une `com.adobe.idp.Document` , le service ne peut être appelé qu’à l’aide de la méthode de POST HTTP. Si le service en nécessite un `com.adobe.idp.Document` , le corps de la requête HTTP devient le contenu de l’objet Document d’entrée.

   Si un service AEM Forms nécessite plusieurs paramètres d’entrée, le corps de la requête HTTP doit être un message MIME en plusieurs parties, tel que défini par la norme RFC 1867. (La norme RFC 1867 est une norme utilisée par les navigateurs Web pour charger des fichiers sur des sites Web.) Chaque paramètre d’entrée doit être envoyé en tant que partie distincte du message en plusieurs parties et codé dans le `multipart/form-data` format. Le nom de chaque partie doit correspondre au nom du paramètre.
...
Un service AEM Forms peut être appelé de manière asynchrone en substituant `services` avec `async_invoke` dans l’URL d’appel, comme illustré dans l’exemple suivant.
...
L’état de l’appel asynchrone peut être récupéré à l’aide d’une URL d’appel avec `services` remplacé par `async_status`. L’URL doit contenir un `job_id` paramètre spécifiant la valeur d’identifiant de la tâche associée à cet appel. Par exemple :
...
Une fois la tâche terminée et le résultat récupéré, la tâche peut être éliminée à l’aide d’une URL d’appel avec `services` est remplacé par `async_dispose`. L’URL doit également contenir un `job_id` paramètre spécifiant la valeur d’identifiant de la tâche. Par exemple :
...
Le `DSCError` est facultatif et présent uniquement si l’exception est une instance de `com.adobe.idp.dsc.DSCException`.
...
Pour créer une stratégie, vous devez définir des types de données complexes tels qu’une `PolicyEntry` . A `PolicyEntry` définit des attributs tels que les autorisations associées à la stratégie. (Voir [Création de stratégies](/help/forms/developing/protecting-documents-policies.md#creating-policies).)

Au lieu d’envoyer une requête REST pour créer une stratégie (qui inclurait la définition de types de données complexes tels qu’une `PolicyEntry` ), créez un processus qui crée une stratégie à l’aide de Workbench. Définissez le processus pour accepter des variables d’entrée primitives, telles qu’une valeur string qui définit le nom du processus ou un entier qui définit la période d’ouverture hors ligne.
...
L’exemple de HTML suivant transmet deux `Boolean` valeurs d’un processus AEM Forms nommé `RestTest2`. Le nom de la méthode d’appel est `invoke` et la version est 1.0. Notez que la méthode HTML Post est utilisée.
...
L’exemple de HTML suivant transmet une valeur de date à un processus AEM Forms nommé `SOAPEchoService`. Le nom de la méthode d’appel est `echoCalendar`. Notez que le HTML `Post` est utilisée.
...
L’exemple de HTML suivant appelle un processus AEM Forms nommé `MyApplication/EncryptDocument` qui nécessite un document de PDF. Pour plus d’informations sur ce processus, voir [Appel d’AEM Forms à l’aide de MTOM](/help/forms/developing/invoking-aem-forms-using-web.md#invoking-aem-forms-using-mtom).
...
L’exemple de HTML suivant appelle un processus AEM Forms nommé `RestTest3` qui nécessite un document et deux valeurs de texte. Notez que la méthode HTML Post est utilisée.
...
L’exemple de HTML suivant appelle un processus AEM Forms nommé `SOAPEchoService` qui nécessite une valeur d’énumération. Notez que la méthode HTML Post est utilisée.
...
>[!NOTE]
>
>Ce processus n’est pas basé sur un processus AEM Forms existant. Pour suivre l’exemple de code, créez un processus nommé `MyApplication/EncryptDocument` utilisation de workbench. (Voir [Utilisation de Workbench](https://www.adobe.com/go/learn_aemforms_workbench_63).)
...
1. Chiffrement du document PDF avec un mot de passe. Cette action est basée sur l’opération `PasswordEncryptPDF`. Le document PDF chiffré avec un mot de passe est retourné dans une variable de processus nommée `outDoc`.
   ...
   Lorsque ce processus est appelé à l’aide d’une requête REST, le document de PDF chiffré s’affiche dans le navigateur web. Avant d’afficher le document du PDF, vous devez spécifier le mot de passe (sauf si la sécurité est désactivée). Le code de HTML suivant représente une demande d’appel REST à la fonction `MyApplication/EncryptDocument` processus.
...
Vous pouvez appeler un processus Forms à partir d’Acrobat à l’aide d’une requête REST. Par exemple, vous pouvez appeler la fonction *MyApplication/EncryptDocument* processus. Pour appeler un processus Forms à partir d’Acrobat, placez un bouton d’envoi sur un fichier XDP dans Designer. (Voir l’[aide de Designer](https://www.adobe.com/go/learn_aemforms_designer_63).)
...
Si le processus nécessite un document de PDF comme valeur d’entrée, veillez à envoyer le formulaire comme PDF, comme illustré ci-dessus. En outre, pour appeler un processus, le processus doit renvoyer un document de PDF. Sinon, Acrobat ne peut pas gérer la valeur renvoyée et une erreur se produit. Il n’est pas nécessaire de spécifier le nom de la variable de processus d’entrée. Par exemple, le processus* MyApplication/EncryptDocument* comporte une variable d’entrée nommée `inDoc`. Il n’est pas nécessaire de spécifier inDoc tant que le formulaire est envoyé en tant que PDF.

Vous pouvez également envoyer des données de formulaire au format XML à un processus Forms. Pour envoyer des données XML, assurez-vous que la variable `Submit As` La liste déroulante indique le code XML. Comme la valeur renvoyée par le processus doit être un document PDF, le document du PDF s’affiche dans Acrobat.
