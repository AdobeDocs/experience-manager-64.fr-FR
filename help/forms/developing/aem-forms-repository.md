...
* Écrivez des ressources et leurs propriétés. Voir [Ecriture de ressources](aem-forms-repository.md#writing-resources).
...
* Lisez les ressources et leurs propriétés. Voir Ressources [](aem-forms-repository.md#reading-resources)de lecture.
...
1. Création du dossier
   ...
   Appelez la méthode `repositoryInfomodelFactoryBean` de l’ `newResourceCollection` objet et transmettez les paramètres suivants :
   ...
   * Un `java.lang.String` contenant le nom de la collecte de ressources. Par exemple, `FormsFolder`.
   ...
   * Description `String` de la collecte de ressources. Dans cet exemple, `"test Folder"` est utilisé `.`


1. Écrire le dossier dans le référentiel

   Appelez la méthode `ResourceRepositoryClient` de l’objet `writeResource` et transmettez l’URI du dossier et de l’ `ResourceCollection` objet. Par exemple, l’URI du dossier peut être la valeur suivante `/Applications/FormsApplication/1.0/`.

   La méthode renvoie une instance de l’ `com.adobe.repository.infomodel.bean.Resource` objet nouvellement créé. Vous pouvez, par exemple, récupérer la valeur d’identificateur de la nouvelle ressource en appelant la `com.adobe.repository.infomodel.bean.Resource` méthode de l’ `getId` objet.
...
1. Création du dossier
   ...
   Créez le dossier en utilisant le constructeur par défaut pour la `ResourceCollection` classe et transmettez les paramètres suivants :

   * Un `Id` objet, qui est créé en appelant le constructeur par défaut pour la `Id` classe et affecté au `Resource` champ de l&#39; `id` objet.
   * Un `Lid` objet, qui est créé en appelant le constructeur par défaut pour la `Lid` classe et affecté au `Resource` champ de l&#39; `lid` objet.
...
1. Écrire le dossier dans le référentiel
   ...
   Appelez la méthode `RepositoryServiceService` de l’ `writeResource` objet et transmettez les paramètres suivants :
...
Créez une chaîne contenant l&#39;URI de la ressource à lire. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*folder*&quot;.
...
1. Spécifiez l’URI du dossier de cible pour la ressource.
   ...
   Spécifiez l&#39;URI du dossier de cible pour la ressource. Dans ce cas, puisque la ressource nommée `testResource` sera stockée dans le dossier nommé `testFolder`, l’URI du dossier est `"/testFolder"`l’URI. L&#39;URI est stocké en tant qu&#39; `java.lang.String` objet.
...
1. Créer la ressource
   ...
   * Un `com.adobe.repository.infomodel.Id` objet, qui est créé en appelant le constructeur par défaut pour la `Id` classe.
   * Un `com.adobe.repository.infomodel.Lid` objet, qui est créé en appelant le constructeur par défaut pour la `Lid` classe.
...
1. Spécifier le contenu de la ressource
   ...
   Pour créer du contenu pour la ressource, appelez la `RepositoryInfomodelFactoryBean` méthode de l’objet `newResourceContent` qui renvoie un `com.adobe.repository.infomodel.bean.ResourceContent` objet. Add content to the `ResourceContent` object. Dans cet exemple, vous effectuez les tâches suivantes :

   * Appel de la `ResourceContent` méthode de l’ `setDataDocument` objet et transmission d’un `com.adobe.idp.Document` objet
   * Appeler la méthode `ResourceContent` de l’ `setSize` objet et transmettre la taille en octets de l’ `Document` objet
...
1. Créer la ressource
   ...
   * Un `com.adobe.repository.infomodel.Id` objet, qui est créé en appelant le constructeur par défaut pour la `Id` classe et affecté au `Resource` champ de l&#39; `id` objet.
   * Un `com.adobe.repository.infomodel.Lid` objet, qui est créé en appelant le constructeur par défaut pour la `Lid` classe et affecté au `Resource` champ de l&#39; `lid` objet.
   ...
   * Chaîne contenant la description de la ressource, qui est affectée au `Resource` `description` champ de l’objet. La description utilisée dans cet exemple est `"test resource"`.
...
1. Spécifier le contenu de la ressource
   ...
   * Affectation d’un `BLOB` objet contenant un document au `ResourceContent` champ de l’ `dataDocument` objet.
   * Attribuer la taille en octets de l’ `BLOB` objet au `ResourceContent` `size` champ de l’objet.

   Ajoutez le contenu à la ressource en affectant l’ `ResourceContent` objet au champ `Resource` `content` de l’objet.
...
Créez une chaîne contenant le chemin du dossier contenant les ressources. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*folder*&quot;.
...
1. Récupérer la liste des ressources
   ...
   Appelez la méthode `RepositoryServiceService` de l’objet `listMembers` et transmettez l’URI du dossier en tant que premier paramètre. Transmettez `null` les deux autres paramètres.

   La méthode renvoie un tableau d’objets pouvant être moulés vers `Resource` des objets. Vous pouvez effectuer une itération dans le tableau d&#39;objets pour récupérer chacune des ressources associées. Dans cet exemple, le nom et la description de chaque ressource s&#39;affichent.
...
Créez une chaîne contenant l&#39;URI de la ressource à lire. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*resource*&quot;.
...
1. Lire la ressource
   ...
   Appelez la méthode `ResourceRepositoryClient` de l’objet `readResource` et transmettez l’URI de la ressource en tant que paramètre. Cette méthode renvoie une `Resource` instance qui représente la ressource.
...
1. Spécifiez l&#39;URI de la ressource à lire.
   ...
   Spécifiez une chaîne contenant l&#39;URI de la ressource à récupérer. Dans ce cas, puisque la ressource nommée `testResource` se trouve dans le dossier nommé `testFolder`, son URI est `"/testFolder/testResource"`défini. Lorsque vous utilisez un langage compatible avec Microsoft .NET Framework (C#, par exemple), stockez l&#39;URI dans un `System.String` objet.

1. Lire la ressource

   Appelez la méthode `RepositoryServiceService` de l’objet `readResource` et transmettez l’URI de la ressource en tant que premier paramètre. Transmettez `null` les deux autres paramètres.
...
Lisez la ressource. Pour plus d’informations, voir Ressources [de](aem-forms-repository.md#reading-resources)lecture.
...
1. Mettre à jour la ressource
   ...
   Appelez ensuite la `ServiceClientFactory` méthode de l’ `updateResource` objet, puis transmettez les paramètres suivants :
   ...
   * Valeur `boolean` indiquant si la version majeure ou mineure doit être mise à jour. Dans cet exemple, une valeur de `true` est transmise pour indiquer que la version majeure doit être incrémentée.
...
1. Récupérer la ressource à mettre à jour
   ...
   Spécifiez l&#39;URI de la ressource à récupérer et lisez-la. Dans cet exemple, l&#39;URI de la ressource est `"/testFolder/testResource"`. Pour plus d’informations, voir Ressources [de](aem-forms-repository.md#reading-resources)lecture.
...
1. Appelez la méthode `RepositoryServiceService` de l’ `updateResource` objet et transmettez les paramètres suivants :
...
L’ordre de tri n’est pas respecté lors de l’appel de la `ResourceRepositoryClient` `searchProperties` méthode de l’objet et de la spécification d’un ordre de tri. Par exemple, supposons que vous créez une ressource avec trois propriétés personnalisées, où les noms d&#39;attribut sont `name`, `secondName`et `asecondName`. Ensuite, vous créez un élément d’ordre de tri sur le nom de l’attribut et définissez la `ascending` valeur sur `true`.

Ensuite, vous appelez la `ResourceRepositoryClient` méthode de l’ `searchProperties` objet et passez dans l’ordre de tri. La recherche renvoie la ressource appropriée, avec les trois propriétés. Cependant, les propriétés ne sont pas triées par nom d’attribut. Ils sont renvoyés dans l’ordre dans lequel ils ont été ajoutés : `name`, `secondName`et `asecondName`.
...
Créez une chaîne contenant le chemin de base à partir duquel effectuer la recherche. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*folder*&quot;.
...
1. Spécifier les attributs utilisés dans la recherche
   ...
   Spécifiez les valeurs des attributs sur lesquels effectuer la recherche. Les attributs existent dans un `com.adobe.repository.infomodel.bean.Resource` objet. Dans cet exemple, la recherche sera effectuée sur l&#39;attribut name ; par conséquent, un `java.lang.String` contenant le nom de l’ `Resource` objet est utilisé, ce qui est `testResource` le cas présent.
...
1. Créer la requête utilisée dans la recherche
   ...
   * Opérande de droite contenant la valeur d’attribut sur laquelle effectuer la recherche. Dans cet exemple, l’attribut name, un `String` contenant la valeur `"testResource"`, est utilisé.
   ...
   Ajoutez chaque instruction à la requête en appelant la `Query` méthode de l’ `addStatement` objet et en transmettant l’ `Query.Statement` objet.

1. Créer l&#39;ordre de tri des résultats de la recherche

   Pour spécifier l&#39;ordre de tri utilisé dans les résultats de la recherche, créez un `com.adobe.repository.query.sort.SortOrder` objet en appelant le constructeur par défaut de la `SortOrder` classe et ajoutez des éléments à l&#39;ordre de tri.

   Pour créer un élément pour l&#39;ordre de tri, appelez l&#39;un des constructeurs de la `com.adobe.repository.query.sort.SortOrder.Element` classe. Dans cet exemple, puisque le nom de la ressource est utilisé comme base de la recherche, la valeur statique `Resource.ATTRIBUTE_NAME` est utilisée comme premier paramètre et l’ordre croissant (une `boolean` valeur de `true`) est spécifié comme second paramètre.

   Ajoutez chaque élément à l’ordre de tri en appelant la `SortOrder` méthode de l’ `addSortElement` objet et en transmettant l’ `SortOrder.Element` objet.

1. Rechercher les ressources

   Pour rechercher `resources` en fonction des propriétés d’attribut, appelez la `ResourceRepositoryClient` méthode de l’ `searchProperties` objet et transmettez les paramètres suivants :

   * Un chemin `String` contenant le chemin de base à partir duquel exécuter la recherche. Dans ce cas, `"/testFolder"` est utilisé.
...
1. Récupérer les ressources des résultats de la recherche
   ...
   Pour récupérer les ressources contenues dans le résultat de la recherche, effectuez une itération dans le `List` fichier et définissez chaque objet sur un `Resource` afin d’extraire ses informations. Dans cet exemple, le nom de chaque ressource s&#39;affiche.
...
Créez des chaînes contenant les URI de la ressource à relier. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*resource*&quot;.
...
1. Spécifier les URI des ressources à relier
   ...
   Spécifiez les URI des ressources à relier. Dans ce cas, étant donné que les ressources sont nommées `testResource1` et `testResource2` et se trouvent dans le dossier nommé `testFolder`, leurs URI sont `"/testFolder/testResource1"` et `"/testFolder/testResource2"`. Les URI sont stockés en tant qu’ `java.lang.String` objets. Dans cet exemple, les ressources sont d’abord écrites dans le référentiel et leurs URI sont récupérés. Pour plus d&#39;informations sur l&#39;écriture d&#39;une ressource, consultez [Ecriture de ressources](aem-forms-repository.md#writing-resources).

1. Créer la relation

   Appelez la méthode `ResourceRepositoryClient` de l’ `createRelationship` objet et transmettez les paramètres suivants :
   ...
   * Valeur `boolean` indiquant si la ressource de cible est automatiquement mise à jour vers l’identifiant `com.adobe.repository.infomodel.Id`basé de la nouvelle ressource principale. Dans cet exemple, en raison de la relation de dépendance, la valeur `true` est spécifiée.

   Vous pouvez également récupérer une liste de ressources connexes pour une ressource donnée en invoquant la `ResourceRepositoryClient` `getRelated` méthode de l’objet et en transmettant les paramètres suivants :
   ...
   * Type de relation, qui est l&#39;une des constantes statiques de la `Relation` classe. Dans cet exemple, une relation de dépendance est spécifiée en utilisant la même valeur que celle utilisée précédemment : `Relation.TYPE_DEPENDANT_OF`.

   La `getRelated` méthode renvoie un `java.util.List` nombre d&#39; `Resource` objets à travers lesquels vous pouvez itérer pour récupérer chacune des ressources associées, en faisant passer les objets contenus dans le `List` à `Resource` comme vous le faites. Dans cet exemple, `testResource2` on s&#39;attend à ce que les ressources retournées soient dans la liste des ressources renvoyées.
...
1. Spécifier les URI des ressources à relier
   ...
   Spécifiez les URI des ressources à relier. Dans ce cas, étant donné que les ressources sont nommées `testResource1` et `testResource2` et se trouvent dans le dossier nommé `testFolder`, leurs URI sont `"/testFolder/testResource1"` et `"/testFolder/testResource2"`. Lorsque vous utilisez un langage compatible avec Microsoft .NET Framework (par exemple, C#), les URI sont stockés en tant qu&#39; `System.String` objets. Dans cet exemple, les ressources sont d’abord écrites dans le référentiel et leurs URI sont récupérés. Pour plus d&#39;informations sur l&#39;écriture d&#39;une ressource, consultez [Ecriture de ressources](aem-forms-repository.md#writing-resources).

1. Créer la relation

   Appelez la méthode `RepositoryServiceService` de l’ `createRelationship` objet et transmettez les paramètres suivants :
   ...
   * Valeur `boolean` indiquant si la ressource de cible est automatiquement mise à jour vers l’identifiant `Id`basé de la nouvelle ressource principale. Dans cet exemple, en raison de la relation de dépendance, la valeur `true` est spécifiée.
   ...
   * Transmettez `null` le dernier paramètre.
   ...
   Vous pouvez également récupérer une liste de ressources connexes pour une ressource donnée en invoquant la `RepositoryServiceService` `getRelated` méthode de l’objet et en transmettant les paramètres suivants :
   ...
   * Transmettez `null` les deux paramètres restants.
   ...
   La `getRelated` méthode renvoie un tableau d&#39;objets qui peuvent être convertis en `Resource` objets grâce auxquels vous pouvez effectuer une itération pour récupérer chacune des ressources associées. Dans cet exemple, `testResource2` on s&#39;attend à ce que les ressources retournées soient dans la liste des ressources renvoyées.
...
Créez une chaîne contenant l&#39;URI de la ressource à verrouiller. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*resource*&quot;.
...
1. Spécifiez l&#39;URI de la ressource à verrouiller.
   ...
   Spécifiez l&#39;URI de la ressource à verrouiller. Dans ce cas, puisque la ressource nommée `testResource` se trouve dans le dossier nommé `testFolder`, son URI est `"/testFolder/testResource"`défini. L&#39;URI est stocké en tant qu&#39; `java.lang.String` objet.

1. Verrouiller la ressource

   Appelez la méthode `ResourceRepositoryClient` de l’ `lockResource` objet et transmettez les paramètres suivants :
   ...
   * La profondeur de verrouillage. Dans cet exemple, étant donné que le verrouillage ne s&#39;applique qu&#39;à la ressource particulière et qu&#39;aucun de ses membres ou enfants ne l&#39;est, la profondeur de verrouillage est spécifiée comme `Lock.DEPTH_ZERO`.
   ...
   >[!NOTE]
   >
   >La version surchargée de la `lockResource` méthode qui requiert quatre paramètres renvoie une exception. Veillez à utiliser la `lockResource` méthode qui requiert trois paramètres, comme indiqué dans cette procédure pas à pas.

1. Récupérer les verrous de la ressource

   Appelez la méthode `ResourceRepositoryClient` de l’objet `getLocks` et transmettez l’URI de la ressource en tant que paramètre. La méthode renvoie une Liste d&#39;objets Lock par laquelle vous pouvez effectuer une itération. Dans cet exemple, le propriétaire du verrou, la profondeur et la portée sont imprimés pour chaque objet en appelant respectivement les méthodes `getOwnerUserId`, `getDepth`et `getType` méthodes de l’objet Lock.

1. Déverrouiller la ressource

   Appelez la méthode `ResourceRepositoryClient` de l’objet `unlockResource` et transmettez l’URI de la ressource en tant que paramètre. Pour plus d’informations, voir le Guide de référence [de l’API](https://www.adobe.com/go/learn_aemforms_javadocs_63_en)AEM Forms.
...
### Verrouillage des ressources à l’aide de l’API du service Web {#lock-resources-using-the-web-service-api}
...
1. Verrouiller la ressource
   ...
   Appelez la méthode `RepositoryServiceService` de l’ `lockResource` objet et transmettez les paramètres suivants :
   ...
   * Valeur indiquant le nombre de secondes avant l’expiration du verrou. `int` Dans cet exemple, la valeur de `1000` est utilisée.
   * Transmettez `null` le dernier paramètre.

1. Récupérer les verrous de la ressource

   Appelez la `RepositoryServiceService` méthode de l’objet et transmettez l’URI de la ressource en tant que premier paramètre et `getLocks` `null` pour le second paramètre. La méthode renvoie un `object` tableau contenant `Lock` des objets à travers lesquels vous pouvez effectuer une itération. Dans cet exemple, le propriétaire du verrou, la profondeur et la portée sont imprimés pour chaque objet en accédant aux champs `Lock` , `ownerUserId`et `depth``type` de chaque objet, respectivement.

1. Déverrouiller la ressource

   Appelez la `RepositoryServiceService` méthode de l’objet et transmettez l’URI de la ressource en tant que premier paramètre et `unlockResource` `null` pour le second paramètre.
...
Créez une chaîne contenant l&#39;URI de la ressource à supprimer. La syntaxe comprend des barres obliques verticales, comme dans cet exemple : &quot;/*path*/*resource*&quot;. Si la ressource à supprimer est un dossier, la suppression est récursive.
...
1. Spécifiez l&#39;URI de la ressource à supprimer.
   ...
   Spécifiez l&#39;URI de la ressource à récupérer. Dans ce cas, puisque la ressource nommée testResourceToBeDeleted se trouve dans le dossier appelé testFolder, son URI est `/testFolder/testResourceToBeDeleted`défini. L&#39;URI est stocké en tant qu&#39; `java.lang.String` objet. Dans cet exemple, la ressource est d&#39;abord écrite dans le référentiel et son URI est récupéré. Pour plus d&#39;informations sur l&#39;écriture d&#39;une ressource, consultez [Ecriture de ressources](aem-forms-repository.md#writing-resources).
...
1. Spécifiez l&#39;URI de la ressource à supprimer.
   ...
   Spécifiez l&#39;URI de la ressource à récupérer. Dans ce cas, puisque la ressource nommée `testResourceToBeDeleted` se trouve dans le dossier nommé `testFolder`, son URI est `"/testFolder/testResourceToBeDeleted"`défini. Dans cet exemple, la ressource est d&#39;abord écrite dans le référentiel et son URI est récupéré. Pour plus d&#39;informations sur l&#39;écriture d&#39;une ressource, consultez [Ecriture de ressources](aem-forms-repository.md#writing-resources).

1. Supprimer la ressource

   Appelez la `RepositoryServiceService` méthode de l’ `deleteResources` objet et transmettez un `System.String` tableau contenant l’URI de la ressource en tant que premier paramètre. Transmettez `null` le second paramètre.
