...
* Lorsque le document ne se trouve pas sur le serveur, utilisez la servlet de téléchargement à distance pour télécharger un document vers AEM Forms. La nouvelle fonctionnalité d’AEM Forms permet de télécharger des documents sécurisés. Lorsque vous téléchargez un document sécurisé, vous devez utiliser un utilisateur doté du * rôle Utilisateur de l’application de téléchargement de Document *. Sans ce rôle, l’utilisateur ne peut pas télécharger un document sécurisé. Il est recommandé d’utiliser la connexion unique pour télécharger un document sécurisé. (Voir [Transmission de documents sécurisés pour appeler des processus à l’aide de Remoting](invoking-aem-forms-using-remoting.md#passing-secure-documents-to-invoke-processes-using-remoting).)
   ...
   Vous utilisez les fonctionnalités de téléchargement Flash standard pour l’URL de téléchargement désignée : `https://SERVER:PORT/remoting/lcfileupload`. Vous pouvez ensuite utiliser l’ `DocumentReference` objet chaque fois qu’un paramètre d’entrée de type `Document` est attendu.
...
**Création d’un Canal à AEM Forms**
...
Dans cet exemple de code, `pdfDocument` est une `DocumentReference` instance qui contient un document PDF non sécurisé. Pour plus d’informations sur un `DocumentReference`module, voir [Gestion des documents avec AEM Forms Remoting](invoking-aem-forms-using-remoting.md#handling-documents-with-remoting)(obsolète pour AEM Forms).
...
Les paramètres de sortie de processus d’AEM Forms sont désérialisés en objets ActionScript à partir desquels l’application cliente extrait des paramètres spécifiques par nom, comme le montre l’exemple suivant. (La valeur de sortie du `MyApplication/EncryptDocument` processus est nommée `outDoc`.)
...
1. Configurez une `ChannelSet` instance pour communiquer avec AEM Forms et associez-la à l’ `mx:RemoteObject` instance. Voir Création d’un Canal à AEM Forms.
...
Lorsqu’un service AEM Forms est appelé à l’aide d’AEM Forms Remoting (obsolète pour AEM Forms), l’application cliente transmet le cookie d’authentification dans le cadre de la demande. L’utilisateur ayant déjà été authentifié, aucune connexion supplémentaire n’est requise pour établir une connexion à partir de l’application cliente au service AEM Forms.
...
Une application cliente accède à AEM Forms par le biais d’un point de terminaison distant à l’aide du `RemoteObject` composant, comme le montre l’exemple suivant.
...
A la demande suivante de l’application cliente, AEM Forms détecte que le cookie a changé et déconnecte l’utilisateur. Par conséquent, la première requête après un changement de cookie échoue. Toutes les requêtes suivantes sont effectuées dans le contexte du nouveau cookie et sont réussies.
...
>[!NOTE]
Pour configurer AEM Forms pour qu’il accepte les documents non sécurisés, sélectionnez l’option Autoriser le téléchargement de documents non sécurisés à partir des applications Flex. Redémarrez ensuite une application ou un service pour vous assurer que le paramètre est appliqué.
...
Dans la classe ActionScript, utilisez la balise `RemoteClass` de métadonnées pour mapper sur le type complexe AEM Forms. Par exemple, lors de l’appel de l’ `createCustomer` opération du service client, créez une classe ActionScript qui mappe au type de `com.adobe.livecycle.sample.customer.Customer` données.

La classe ActionScript suivante nommée Customer indique comment mapper sur le type de données AEM Forms `com.adobe.livecycle.sample.customer.Customer`.
