...
seo-description: Utilisez Remoting pour appeler un processus AEM Forms afin d’appeler les processus créés dans Workbench. Vous pouvez appeler un processus AEM Forms à partir d’une application cliente créée avec Flex.
...
>[!NOTE]
>
>Lors de l’utilisation de Remoting, il est recommandé d’appeler les processus créés dans Workbench par opposition aux services AEM Forms. Il est toutefois possible d’appeler les services AEM Forms directement. (Voir Chiffrement de documents PDF à l’aide d’Adobe Remoting dans AEM Forms Developer Center.)
...
1. Enregistre le document PDF chiffré par mot de passe en tant que fichier PDF dans le système de fichiers local. Cette action est basée sur l’opération `WriteDocument`. 

>[!NOTE]
>
>Le processus `MyApplication/EncryptDocument` n’est pas basé sur un processus AEM Forms existant. Pour suivre les exemples de code, créez un processus nommé `MyApplication/EncryptDocument` à l’aide de Workbench.

>[!NOTE]
>
>Pour plus d’informations sur l’utilisation de Remoting pour appeler un processus de longue durée, voir [Appel de processus pour des intervenants humains de longue durée](/help/forms/developing/invoking-human-centric-long-lived.md#invoking-human-centric-long-lived-processes).
...
>[!NOTE]
>
>*Placez le fichier crossdomain.xml au bon endroit. Par exemple, si vous avez déployé AEM Forms sur JBoss, placez ce fichier à l’emplacement suivant : &lt;répertoire_installation>\Adobe_Experience_Manager_forms\jboss\server\lc_turnkey\deploy\jboss-web.deployer\ROOT.war.*
...
* *&lt;install_directory>\Adobe_Experience_Manager_forms\sdk\misc\DataServices\Client-Libraries*
   ...
   où &quot;a0/>install_directory *est le répertoire dans lequel AEM Forms est installé.*
...
Le didacticiel de mise en route à distance utilise le servlet de téléchargement Remoting pour transmettre un fichier PDF au processus `MyApplication/EncryptDocument`. (Voir [Appel d’un processus de courte durée en transmettant un document non sécurisé à l’aide d’AEM Forms Remoting](invoking-aem-forms-using-remoting.md#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting) (obsolète pour les formulaires AEM).)

### Transmission d’un document à une application cliente {#passing-a-document-back-to-a-client-application}

Une application cliente reçoit un objet de type `mx.rpc.livecycle.DocumentReference` pour une opération de service qui renvoie une instance `com.adobe.idp.Document` en tant que paramètre de sortie. Comme une application cliente traite des objets ActionScript et non de Java, vous ne pouvez pas transmettre un objet Document basé sur Java à un client Flex. Au lieu de cela, le serveur génère une URL pour le document et la transmet au client. La propriété `referenceType` de l’objet `DocumentReference` spécifie si le contenu se trouve dans l’objet `DocumentReference` ou doit être récupéré à partir d’une URL dans la propriété `DocumentReference.url`. La propriété `DocumentReference.contentType` spécifie le type de document.
...
## Appel d’un processus de courte durée en transmettant un document non sécurisé à l’aide de Remoting {#invoking-a-short-lived-process-by-passing-an-unsecure-document-using-remoting}
...
* **id :** nom de l’ `mx:RemoteObject` instance représentant le processus à appeler.
...
Une application cliente peut appeler AEM Forms en spécifiant un canal dans MXML ou un ActionScript, comme le montre l’exemple d’ActionScript suivant. Le canal doit être `AMFChannel`, `SecureAMFChannel`, `HTTPChannel` ou `SecureHTTPChannel`.
...
Dans cet exemple de code, `pdfDocument` est une instance `DocumentReference` qui contient un document PDF non sécurisé. Pour plus d’informations sur une balise `DocumentReference`, voir [Gestion des documents avec AEM Forms Remoting](invoking-aem-forms-using-remoting.md#handling-documents-with-remoting) (Obsolète pour les formulaires AEM).
...
Le paramètre `version` doit être une chaîne contenant un seul point. Les valeurs à gauche, version majeure, à droite, version mineure de la période doivent être des entiers. Si ce paramètre n’est pas spécifié, la version principale de l’en-tête est appelée.

**Gestion des valeurs de retour**

Les paramètres de sortie de processus AEM Forms sont désérialisés en objets ActionScript à partir desquels l’application cliente extrait des paramètres spécifiques par nom, comme le montre l’exemple suivant. (La valeur de sortie du processus `MyApplication/EncryptDocument` est nommée `outDoc`.)
...
1. Configurez une instance `ChannelSet` pour communiquer avec AEM Forms et associez-la à l’instance `mx:RemoteObject`. Voir Création d’un canal vers AEM Forms.
1. Appelez la méthode `login` de ChannelSet ou la méthode `setCredentials` du service pour spécifier la valeur de l’identifiant utilisateur et le mot de passe. (Voir [Utilisation de l’authentification unique](invoking-aem-forms-using-remoting.md#using-single-sign-on).)
...
1. Chiffrez le document PDF en appelant la méthode `mx:RemoteObject` de l’instance `invoke`. Transmettez la balise `Object` contenant le paramètre d’entrée (qui est le document PDF non sécurisé). Voir Transmission des valeurs d’entrée.
1. Récupérez le document PDF chiffré par mot de passe renvoyé par le processus. Voir Gestion des valeurs de retour.
...
>[!NOTE]
Pour plus d’informations sur l’exécution de l’authentification à l’aide de jetons HTTP, voir [Création d’applications de Flash Builder qui effectuent l’authentification SSO à l’aide de jetons HTTP](/help/forms/developing/creating-flash-builder-applications-perform.md#creating-flash-builder-applications-that-perform-sso-authentication-using-http-tokens).
...
* Se déconnecte du serveur en appelant la fonction `ROLogout` en réponse à un événement de clic sur le bouton.
...
Une application cliente accède à AEM Forms par le biais d’un point de terminaison distant à l’aide du composant `RemoteObject`, comme le montre l’exemple suivant.
...
Par conséquent, l’appel de la méthode `RemoteObject.logout` dans une application cliente génère un message d’erreur sur le client indiquant que la session n’est pas déconnectée. À la place, l’utilisateur peut utiliser le service de connexion centralisé pour se déconnecter et supprimer le cookie d’authentification.
...
## Transmission de documents sécurisés pour appeler des processus à l’aide de Remoting {#passing-secure-documents-to-invoke-processes-using-remoting}
...
AEM Forms prend en charge une opération nommée `getFileUploadToken` qui renvoie un jeton transmis au servlet de chargement. La méthode `DocumentReference.constructRequestForUpload` nécessite une URL vers AEM Forms avec le jeton renvoyé par la méthode `LC.FileUploadAuthenticator.getFileUploadToken` . Cette méthode renvoie un objet `URLRequest` utilisé dans l’appel au servlet de chargement. Le code suivant illustre cette logique d’application.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td><p>txtCustId</p></td> 
   <td><p>Indique la valeur de l’identifiant du client à laquelle appartient le nouveau compte. Cette zone de texte est renseignée par la valeur renvoyée par l’opération <code>createCustomer</code> du service client. </p></td> 
  </tr> 
 </tbody> 
</table>
...
>[!NOTE]
Pour déterminer les noms de champ appartenant à un type complexe Forms, une méthode efficace consiste à afficher le WSDL d’un service dans un navigateur Web. Une WSDL spécifie les types complexes d’un service et les membres de données correspondants. Le WSDL suivant est utilisé pour le service client : *https://[votreServer]:[votrePort]/soap/services/CustomerService?wsdl.*
