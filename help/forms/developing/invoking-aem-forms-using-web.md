...
Les services AEM Forms sont exposés via le protocole SOAP et sont conformes au profil de base WSI 1.1. L&#39;interopérabilité des services Web (WSI) est une organisation de normes ouvertes qui favorise l&#39;interopérabilité des services Web entre les plateformes. Pour plus d’informations, voir [https://www.ws-i.org/](https://www.ws-i.org).

AEM Forms prend en charge les normes de service Web suivantes :

* **Encodage**: Prend uniquement en charge le codage de document et littéral (qui est le codage préféré selon le profil de base WSI). (Voir [Appel d’AEM Forms à l’aide du codage](#invoking-aem-forms-using-base64-encoding)Base64.)
...
Les fichiers WSDL du service AEM Forms définissent de nombreux types de données. Un des types de données les plus importants exposés dans un service Web est un `BLOB` type. Ce type de données est mappé à la `com.adobe.idp.Document` classe lors de l’utilisation des API Java d’AEM Forms. (See [Passing data to AEM Forms services using the Java API](/help/forms/developing/invoking-aem-forms-using-java.md#passing-data-to-aem-forms-services-using-the-java-api).)
...
Si une opération de service AEM Forms nécessite un `BLOB` type comme valeur d’entrée, créez une instance du `BLOB` type dans la logique de votre application. (De nombreux démarrages rapides de services Web situés dans *Programmation avec AEM Forms* montrent comment utiliser un type de données BLOB.)
...
1. **L’URL cible spécifie le protocole** de transmission. Si l’URL cible spécifiée lors de l’appel SOAP contient le paramètre `blob="`*BLOB_TYPE *,* BLOB_TYPE *détermine le protocole de transmission.* BLOB_TYPE *est un espace réservé pour base64, dime, mime, http, mtom ou swaref.
1. **Le point de fin SOAP du service est Smart**. Si les conditions suivantes sont vraies, les documents de sortie sont renvoyés à l’aide du même protocole de transmission que les documents d’entrée :
...
1. **Le point de fin SOAP du service n’est pas Smart**. Le protocole configuré détermine le protocole de transmission du document et les données sont renvoyées dans le `BLOB` champ correspondant. Si, par exemple, le point de fin SOAP est défini sur DIME, l’objet blob renvoyé se trouve dans le `blob.attachmentID` champ, quel que soit le protocole de transmission d’un document d’entrée.
1. **Sinon**. Si un service ne prend pas le type de document en entrée, les documents de sortie sont renvoyés dans le `BLOB.remoteURL` champ du protocole HTTP.

Comme décrit dans la première condition, vous pouvez garantir le type de transmission de tout document renvoyé en étendant l’URL du point de fin SOAP avec le suffixe suivant :
...
>[!NOTE]
>
>Il est recommandé de ne pas dépasser 30 Mo lorsque vous renseignez un `BLOB` objet en appelant sa `setBinaryData` méthode. Sinon, il est possible qu’une exception `OutOfMemory`* se produise.*
...
Le tableau suivant répertorie les types de données Java et le type de données correspondant du service Web.
...
1. Utilisez JAX-WS et Apache Ant pour générer des classes de proxy Java. Créez un script de génération Ant pour accomplir cette tâche. Le script suivant est un exemple de script de génération Ant nommé build.xml :
   ...
   Dans ce script de génération Ant, notez que la `url` propriété est définie pour faire référence au WSDL du service Encryption s’exécutant sur localhost. Les propriétés `username` et `password` doivent être définies sur un nom d’utilisateur et un mot de passe AEM forms valides. L’URL contient l’ `lc_version` attribut. Sans spécifier l’ `lc_version` option, vous ne pouvez pas appeler les nouvelles opérations du service AEM Forms.
...
1. compresser les fichiers JAVA dans un fichier JAR ; Si vous travaillez sur Eclipse, procédez comme suit :
   ...
   * Importez ce fichier JAR dans le chemin de classe d’un projet client. En outre, importez tous les fichiers JAR situés dans &lt;Répertoire d’installation>\Adobe\Adobe_Experience_Manager_forms\sdk\client-libs\thirdparty.
   >[!NOTE]
   >
   >Tous les démarrages rapides du service Web Java (à l’exception du service Forms) situés dans Programmation avec AEM forms créent des fichiers proxy Java à l’aide de JAX-WS. En outre, tous les démarrages rapides du service Web Java, utilisez SwaRef. (Voir [Appel d’AEM Forms à l’aide de SwaRef](#invoking-aem-forms-using-swaref).)
...
Vous pouvez utiliser l’outil Apache Axis WSDL2Java pour convertir un service Forms en classes de proxy Java. Ces classes vous permettent d’appeler les opérations du service Forms. Apache Ant vous permet de générer des fichiers de bibliothèque Axis à partir d’un WSDL de service. Vous pouvez télécharger Apache Axis à l’adresse URL [https://ws.apache.org/axis/](https://ws.apache.org/axis/).

>[!NOTE]
>
>Les démarrages rapides du service Web associés au service Forms utilisent des classes proxy Java créées à l’aide d’Apache Axis. Les démarrages rapides du service Web Forms utilisent également Base64 comme type de codage. (voir Démarrage [rapide de l’API du service](/help/forms/developing/forms-service-api-quick-starts.md#forms-service-api-quick-starts)Forms).
...
1. Utilisez l’outil Apache WSDL2Java dans Axis pour générer des classes de proxy Java. Créez un script de génération Ant pour accomplir cette tâche. Le script suivant est un exemple de script de génération Ant nommé build.xml :
   ...
   Dans ce script de génération Ant, notez que la `url` propriété est définie pour faire référence au WSDL du service Encryption s’exécutant sur localhost. Les propriétés `username` et `password` doivent être définies sur un nom d’utilisateur et un mot de passe AEM forms valides.
...
Vous pouvez appeler un service AEM Forms à l’aide du codage Base64. Le codage Base64 code les pièces jointes envoyées avec une demande d’appel de service Web. En d’autres termes, `BLOB` les données sont codées en base 64, et non le message SOAP entier.
...
>[!NOTE]
>
>Veillez à ajouter `?blob=base64` à l&#39;URL dans la classe proxy utilisée pour créer l&#39;assembly client .NET. Sinon, vous ne pouvez pas récupérer les données binaires de l’ `BLOB` objet.
...
1. Create a `System.IO.FileStream` object by invoking its constructor. Transmettez une valeur de chaîne représentant l’emplacement du fichier PDF et le mode d’ouverture du fichier.
...
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `MyApplication_EncryptDocumentService` méthode de l’objet et en transmettant l’ `invoke` `BLOB` objet qui contient le document PDF. Ce processus renvoie un document PDF chiffré dans un `BLOB` objet.
...
1. Renseignez l’ `BLOB` objet en appelant sa `setBinaryData` méthode et en transmettant le tableau d’octets. L’objet `BLOB` `setBinaryData` est la méthode à appeler lors de l’utilisation du codage Base64. Voir Fourniture d’objets BLOB dans les demandes de service.
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `MyApplicationEncryptDocument` `invoke` méthode de l’objet. Transmettez l’ `BLOB` objet contenant le document PDF. La méthode invoke renvoie un `BLOB` objet contenant le document PDF chiffré.
1. Créez un tableau d’octets contenant le document PDF chiffré en appelant la `BLOB` `getBinaryData` méthode de l’objet.
1. Enregistrez le document PDF chiffré au format PDF. Ecrivez le tableau d’octets dans un fichier.

**Voir également**

[Démarrage rapide : Appel d’un service à l’aide de fichiers proxy Java et encodage Base64](/help/forms/developing/invocation-api-quick-starts.md#quick-start-invoking-a-service-using-java-proxy-files-and-base64-encoding)

[Création d&#39;un assembly client .NET utilisant le codage Base64](#creating-a-net-client-assembly-that-uses-base64-encoding)

## Appel d’AEM Forms à l’aide de MTOM {#invoking-aem-forms-using-mtom}

Vous pouvez appeler les services AEM Forms à l’aide de la MTOM standard du service Web. Cette norme définit la manière dont les données binaires, telles qu’un document PDF, sont transmises sur Internet ou sur l’intranet. L’utilisation de l’ `XOP:Include` élément constitue une fonctionnalité de MTOM. Cet élément est défini dans la spécification XOP (XML Binary Optimized Packaging) pour référencer les pièces jointes binaires d’un message SOAP.
...
La discussion porte sur l’utilisation de MTOM dans un projet Microsoft .NET pour appeler les services AEM Forms. La structure .NET utilisée est la version 3.5 et l&#39;environnement de développement est Visual Studio 2008. Si vous avez installé Web Service Enhancements (WSE) sur votre ordinateur de développement, supprimez-le. La structure .NET 3.5 prend en charge une structure SOAP nommée Windows Communication Foundation (WCF). Lors de l’appel d’AEM Forms à l’aide de MTOM, seul WCF (et non WSE) est pris en charge.
...
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `MyApplication_EncryptDocumentClient` `invoke` méthode de l’objet. Transmettez l’ `BLOB` objet contenant le document PDF. Ce processus renvoie un document PDF chiffré dans un `BLOB` objet.
1. Créez un `System.IO.FileStream` objet en appelant son constructeur et en transmettant une valeur de chaîne représentant l’emplacement du fichier du document PDF sécurisé.
...
>[!NOTE]
>
>La plupart des opérations du service AEM Forms ont un démarrage rapide MTOM. Vous pouvez afficher ces démarrages rapides dans la section de démarrage rapide correspondante d’un service. Par exemple, pour consulter la section Démarrage rapide d’Output, voir Démarrage [rapide de l’API](/help/forms/developing/output-service-java-api-quick.md#output-service-java-api-quick-start-soap)Output Service.
...
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `MyApplicationEncryptDocument` méthode de l’objet et en transmettant l’ `invoke` `BLOB` objet qui contient le document PDF. La méthode invoke renvoie un `BLOB` objet contenant un document PDF chiffré.
...
1. Ecrivez l’ `java.io.InputSteam` instance dans un fichier PDF représentant le document PDF chiffré.
...
Vous pouvez appeler les services AEM Forms à l’aide de services Web et transmettre des données BLOB via HTTP. La transmission de données BLOB via HTTP est une autre technique que l’utilisation du codage base64, DIME ou MIME. Par exemple, vous pouvez transmettre des données via HTTP dans un projet Microsoft .NET qui utilise Web Service Enhancement 3.0, qui ne prend pas en charge DIME ou MIME. Lorsque vous utilisez des données BLOB sur HTTP, les données d’entrée sont téléchargées avant l’appel du service AEM Forms.
...
### Création d&#39;un assembly client .NET utilisant des données via HTTP {#creating-a-net-client-assembly-that-uses-data-over-http}
...
1. Attribuez une valeur de chaîne au membre de `BLOB` données de l’objet qui spécifie l’emplacement URI d’un document PDF à transmettre au `remoteURL` `MyApplication/EncryptDocument`service.
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `MyApplication_EncryptDocumentService` méthode de l’objet et en transmettant l’ `invoke` `BLOB` objet. Ce processus renvoie un document PDF chiffré dans un `BLOB` objet.
...
1. Convertissez l’ `System.UriBuilder` objet en `System.IO.Stream` objet. (La section Démarrage rapide C# qui suit cette liste illustre comment effectuer cette tâche.)
...
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `MyApplicationEncryptDocument` méthode de l’objet et en transmettant l’ `invoke` `BLOB` objet qui contient le document PDF. Ce processus renvoie un document PDF chiffré dans un `BLOB` objet.
1. Créez un tableau d’octets pour stocker le flux de données représentant le document PDF chiffré. Appelez la `BLOB` méthode de l’objet (utilisez l’ `getRemoteURL` objet renvoyé par la `BLOB` `invoke` méthode).
...
1. Appelez la méthode `java.io.FileOutputStream` `write` de l’objet. Transmettez le tableau d’octets qui contient le flux de données représentant le document PDF chiffré.
...
Sur cette page Web, recherchez Améliorations des services Web 2.0 et téléchargez-le sur votre ordinateur de développement. Ce téléchargement place un fichier nommé Microsoft WSE 2.0 SPI.msi sur votre ordinateur. Exécutez le programme d&#39;installation et suivez les instructions en ligne.
...
1. Créez un projet Microsoft .NET qui vous permette d’appeler un service Forms à l’aide de DIME. Veillez à inclure les améliorations des services Web 2.0 et à créer une référence Web au service AEM Forms.
...
1. Create a `Microsoft.Web.Services2.Dime.DimeAttachment` object by using its constructor and passing the following values:
   ...
   * Valeur de chaîne qui spécifie le type de contenu. Etant donné que ce processus nécessite un document PDF, spécifiez `application/pdf`.
   * Valeur `TypeFormat` d’énumération. Specify `TypeFormat.MediaType`.
   * Valeur de chaîne spécifiant l’emplacement du document PDF à transmettre au processus AEM Forms.

1. Créez un objet `BLOB` en utilisant son constructeur.
1. Ajoutez la pièce jointe DIME à l’ `BLOB` objet en affectant la valeur `Microsoft.Web.Services2.Dime.DimeAttachment` du membre de données de l’objet au membre de `Id` `BLOB` `attachmentID` données de l’objet.
...
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `EncryptDocumentServiceWse` méthode de l’objet et en transmettant l’ `invoke` `BLOB` objet contenant la pièce jointe DIME. Ce processus renvoie un document PDF chiffré dans un `BLOB` objet.
...
1. Parcourez les pièces jointes situées dans `EncryptDocumentServiceWse.ResponseSoapContext.Attachments` et utilisez la valeur d’identificateur de pièce jointe pour obtenir le document PDF chiffré.
1. Obtenez un `System.IO.Stream` objet en obtenant la valeur du membre `Attachment` de données de l’ `Stream` objet.
1. Créez un tableau d’octets et transmettez-le à la méthode `System.IO.Stream` `Read` de l’objet. Cette méthode remplit le tableau d’octets avec un flux de données qui représente le document PDF chiffré.
1. Créez un `System.IO.FileStream` objet en appelant son constructeur et en transmettant une valeur de chaîne représentant un emplacement de fichier PDF. Cet objet représente le document PDF chiffré.
1. Create a `System.IO.BinaryWriter` object by invoking its constructor and passing the `System.IO.FileStream` object.
...
L’outil Apache Axis WSDL2Java génère des fichiers JAVA qui contiennent des méthodes utilisées pour envoyer des requêtes SOAP à un service. Les requêtes SOAP reçues par un service sont décodées par les bibliothèques générées par Axis et redeviennent les méthodes et arguments.
...
1. Appelez le `MyApplication/EncryptDocument` processus en appelant la `EncryptDocumentSoapBindingStub` `invoke` méthode de l’objet. Transmettez l’ `BLOB` objet contenant la pièce jointe DIME. Ce processus renvoie un document PDF chiffré dans un `BLOB` objet.
...
1. Créez un tableau d’octets et transmettez-le à la méthode `java.io.FileStream` `read` de l’objet. Cette méthode remplit le tableau d’octets avec un flux de données qui représente le document PDF chiffré.
1. Créez un objet `java.io.File` en utilisant son constructeur. Cet objet représente le document PDF chiffré.
1. Créez un objet `java.io.FileOutputStream` en utilisant son constructeur et en transmettant l’objet `java.io.File`. 
...
>[!NOTE]
>
>Le service Web démarre rapidement dans Programmation avec AEM Forms et spécifie le nom d’utilisateur et le mot de passe pour effectuer l’autorisation.
...
L’architecture WSE utilise les types de données Stratégies, Affirmations et SecurityToken. En bref, pour un appel de service Web, spécifiez une stratégie. Une stratégie peut comporter plusieurs assertions. Chaque affirmation peut contenir des filtres. Un filtre est appelé à certaines étapes d’un appel de service Web et, à ce moment, il peut modifier la requête SOAP. Pour plus d’informations, reportez-vous à la documentation des Améliorations des services Web 3.0.
...
Créez une classe pour représenter l’assertion SAML. La tâche principale que cette classe effectue est de convertir les valeurs de données de chaîne en xml et de conserver l’espace blanc. Ce fichier d’assertion xml est ensuite importé dans la requête SOAP.
...
Utilisez ce fichier XML lorsque vous créez des fichiers proxy Java à l’aide de JAX-WS. (voir [Création de classes de proxy Java à l’aide de JAX-WS](#creating-java-proxy-classes-using-jax-ws)).
...
La liste suivante indique d’autres services qui nécessitent un fichier de liaison personnalisé lorsqu’ils sont appelés de manière asynchrone :
...
La liste suivante spécifie les types de données qui ne peuvent pas être partagés entre plusieurs WSDL de service :
...
Pour éviter ce problème, il est recommandé de qualifier complètement les types de données. Prenons l’exemple d’une application .NET qui référence le service Forms et le service Signature à l’aide d’une référence de service. Les deux références de service contiennent une `BLOB` classe. Pour utiliser une `BLOB` instance, qualifiez l’ `BLOB` objet au moment de sa déclaration. Cette approche est illustrée dans l’exemple de code suivant. Pour plus d’informations sur cet exemple de code, voir Signature [numérique de formulaires](/help/forms/developing/digitally-signing-certifying-documents.md#digitally-signing-interactive-forms)interactifs.

L’exemple de code C# suivant montre comment signer un formulaire interactif rendu par le service Forms. L’application cliente comporte deux références de service. L’ `BLOB` instance associée au service Forms appartient à l’ `SignInteractiveForm.ServiceReference2` espace de noms. De même, l’ `BLOB` instance associée au service Signature appartient à l’ `SignInteractiveForm.ServiceReference1` espace de noms. Le formulaire interactif signé est enregistré sous la forme d’un fichier PDF appelé *LoanXFASigned.pdf*.
