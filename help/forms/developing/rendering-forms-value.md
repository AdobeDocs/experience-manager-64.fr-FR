...
Lors du rendu d’un formulaire par valeur, vous devez créer un `com.adobe.idp.Document` objet contenant la conception de formulaire à générer. Vous pouvez référencer un fichier XDP existant ou créer de manière dynamique une conception de formulaire au moment de l’exécution et remplir une `com.adobe.idp.Document` demande avec ces données.
...
>[!NOTE]
>
>Si vous souhaitez afficher des données dans le formulaire, elles doivent être spécifiées dans l’ `xfa:datasets` élément. Pour plus d’informations sur l’architecture XFA, voir [https://partners.adobe.com/public/developer/xml/index_arch.html](https://partners.adobe.com/public/developer/xml/index_arch.html).
...
1. Ecrire le flux de données du formulaire dans le navigateur Web client
   ...
   * Créez un tableau d’octets et affectez la taille de l’ `InputStream` objet. Appelez la méthode `InputStream` de l’ `available` objet pour obtenir la taille de l’ `InputStream` objet.
...
1. Référence à la conception de formulaire
   ...
   * Renseignez le tableau d’octets avec les données de flux en appelant la méthode `java.io.FileInputStream` `read` de l’objet et en transmettant le tableau d’octets.
