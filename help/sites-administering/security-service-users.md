...
La méthode principale pour obtenir une session d’administration ou un résolveur de ressources dans AEM utilisait la variable `SlingRepository.loginAdministrative()` et `ResourceResolverFactory.getAdministrativeResourceResolver()` méthodes fournies par Sling.
...
* **Respectez les paramètres de confidentialité.**
   ...
   * Dans le cas des profils privés, par exemple, ne pas exposer la photo, l’email ou le nom complet du profil trouvés dans le fichier privé. `/profile` noeud .

## Contrôle d’accès strict {#strict-access-control}

Que vous ayez appliqué le contrôle d’accès lors de la restructuration du contenu ou lorsque vous le faites pour un nouvel utilisateur de service, vous devez appliquer les listes de contrôle d’accès les plus strictes possible. Utilisez tous les moyens de contrôle d’accès possibles :

* Par exemple, au lieu d’appliquer `jcr:read` on `/apps`, appliquez-le uniquement à `/apps/*/components/*/analytics`
...
1. Ajouter un `service-user` mappage pour votre service et pour `user/sub-users`
...
1. Remplacez la variable `admin-session` dans votre code avec la fonction `loginService` ou `getServiceResourceResolver` API.
...
L’approche recommandée consiste à créer un utilisateur de service pour utiliser l’explorateur de référentiel à l’adresse *https://&lt;server>:&lt;port>/crx/explorer/index.jsp*
...
1. Accédez à l’explorateur de référentiel à l’adresse *https://&lt;server>:&lt;port>/crx/explorer/index.jsp*
...
1. Créez un `SLING-INF/content` sous le dossier `src/main/resources` dossier de votre lot ;
...
1. Référencez le contenu initial Sling dans la configuration de la propriété `maven-bundle-plugin` dans le `pom.xml` de votre lot. Exemple :
...
Appels à `loginAdministrative()` s’affichent souvent avec des sessions partagées. Ces sessions sont acquises lors de l’activation du service et ne sont déconnectées qu’après l’arrêt du service. Bien que cette pratique soit courante, elle pose deux problèmes :
...
Les JSP ne peuvent pas utiliser `loginService()`, car aucun service n’est associé. Cependant, les sessions administratives dans les JSP sont généralement le signe d&#39;une violation du paradigme MVC.
...
1. Transmettez le `user-id` dans la charge utile de l’événement et utilisez l’emprunt d’identité.
   ...
   **Inconvénients :** Utilisations fixes `loginAdministrative()`. Il réauthentifie une requête qui a déjà été authentifiée.
...
1. Créez ou réutilisez un utilisateur de service ayant accès aux données.
   ...
   **Inconvénients :** Nécessite des utilisateurs de services très puissants pour être flexibles, ce qui peut facilement conduire à des réaffectations de privilèges. Évite le modèle de sécurité.

1. Transmettre une sérialisation de la variable `Subject` dans la payload de l’événement, puis créez une `ResourceResolver` en fonction de ce sujet. Un exemple serait d’utiliser le JAAS `doAsPrivileged` dans le `ResourceResolverFactory`.
   ...
   **Inconvénients :** Il faut refactoriser. Le fait que le code relevant de la sécurité soit transparent pour le consommateur de l’événement peut également entraîner des problèmes.
