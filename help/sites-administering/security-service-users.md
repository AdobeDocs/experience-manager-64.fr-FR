...
La principale façon d’obtenir un résolveur de session d’administration ou de ressource dans AEM était d’utiliser les méthodes `SlingRepository.loginAdministrative()` et `ResourceResolverFactory.getAdministrativeResourceResolver()` fournies par Sling.
...
* **Respectez les paramètres de confidentialité.**
   ...
   * Dans le cas des profils privés, par exemple, ne pas exposer l’image de profil, l’email ou le nom complet figurant sur le noeud privé `/profile`.
...
* Limitation des autorisations
   ...
   * par exemple, lorsque vous n’avez besoin que d’écrire des propriétés, ne donnez pas l’autorisation `jcr:write` ; utilisez plutôt `jcr:modifyProperties`
...
1. Ajoutez un mappage `service-user` pour votre service et pour `user/sub-users`
...
1. Dans ce dossier, créez un fichier nommé org.apache.sling.serviceusermapping.impl.ServiceUserMapperImpl.modified-&lt;nom unique de votre configuration d’usine>.xml avec le contenu de votre configuration d’usine (y compris tous les mappings d’utilisateurs de sous-services). Exemple :
...
Les appels à `loginAdministrative()` s’affichent souvent avec des sessions partagées. Ces sessions sont acquises lors de l’activation du service et ne sont déconnectées qu’après l’arrêt du service. Bien que cette pratique soit courante, elle pose deux problèmes :
...
1. Transmettez le `user-id` dans la charge utile de l’événement et utilisez l’emprunt d’identité.
   ...
   **Inconvénients :** Utilise toujours  `loginAdministrative()`. Il réauthentifie une requête qui a déjà été authentifiée.
...
1. Créez ou réutilisez un utilisateur de service ayant accès aux données.
   ...
   **Inconvénients :** nécessite que les utilisateurs de services très puissants soient flexibles, ce qui peut facilement conduire à des réaffectations de privilèges. Évite le modèle de sécurité.

1. Transmettez une sérialisation de `Subject` dans la payload de l’événement et créez une `ResourceResolver` basée sur cet objet. Un exemple serait d’utiliser le JAAS `doAsPrivileged` dans le `ResourceResolverFactory`.
   ...
   **Inconvénients :** nécessite une refactorisation. Le fait que le code relevant de la sécurité soit transparent pour le consommateur de l’événement peut également entraîner des problèmes.
