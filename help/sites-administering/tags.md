...
Lorsqu’un espace de noms ou une autre balise est sélectionné, sélectionnez la **`Unpublish`L’icône ** désactive la balise dans l’environnement de création et la supprime de l’environnement de publication. Semblable au `Delete`si la balise sélectionnée est une balise conteneur, toutes ses balises enfants sont désactivées dans l’environnement de création et supprimées de l’environnement de publication.
