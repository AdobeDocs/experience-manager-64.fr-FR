...
1. Accédez à** Apache Felix OSGi Management Console** et modifiez le **nom d’utilisateur** et **mot de passe**.
...
>[!NOTE]
>
>Les exemples d’applications We.Retail sont supprimés si cette instance est en cours d’exécution en [mode Prêt pour la production](/help/sites-administering/production-ready.md). Si, pour quelque raison que ce soit, ce n’est pas le cas, vous pouvez désinstaller l’exemple de contenu en sélectionnant Gestionnaire de modules, puis en recherchant et en désinstallant tous les modules We.Retail. Pour plus d’informations, voir [Comment utiliser les packages](package-manager.md).
...
1. Contrôlez la configuration des outils de rendu par défaut, ce qui peut poser un problème.
   ...
   * Notamment, l’outil de rendu JSON, qui peut traverser l’arborescence sur plusieurs des niveaux.
      ...
      **Résultats**  max JSON(  `json.maximumresults`)
      ...
      dans la configuration du [Apache Sling GET Servlet](/help/sites-deploying/osgi-configuration-settings.md). Lorsque cette limite est dépassée, le rendu est réduit. La valeur par défaut pour Sling dans AEM est `200`.
