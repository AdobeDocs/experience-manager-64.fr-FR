...
seo-description: Consultez cette page pour en savoir plus sur les modèles de page. Les composants de page que vous créez pour votre application sont basés sur le composant /libs/mobileapps/components/angular/ng-page .
...
Introduit pour la première fois dans la section body.jsp , template.jsp contient simplement le parsys de la page. En mode de publication, ce contenu est référencé directement (à l’adresse &lt;page-path>.template.html) et chargé dans le SPA via le modèleUrl configuré sur le \$routeProvider.
...
Comme décrit dans la section Modèles de page d’AEM, chaque composant peut générer un fragment JavaScript afin d’utiliser le contenu JSON exposé par la promesse `data`. Conformément aux conventions d’Angular, un contrôleur ne doit être utilisé que pour affecter des variables à la portée.
