...
Voici quelques-unes des directives pour le développement des gestionnaires de synchronisation de contenu :
...
* Le gestionnaire ne doit générer un rapport true que s’il met à jour le cache ContentSync. La génération de rapports erronés sur true permet à AEM de créer une mise à jour lorsqu’une mise à jour n’a pas eu lieu.
* Le gestionnaire ne doit mettre à jour le cache que si le contenu a réellement changé. N’écrivez pas dans le cache si un blanc n’est pas nécessaire. Une mise à jour inutile est alors créée.

>[!NOTE]
>
>Activez la journalisation *du débogage* ContentSync via les configurations de journalisation OSGI sur le package com.day.cq.contentsync **. Cela permet de suivre les gestionnaires qui ont exécuté et s’ils ont mis à jour le cache et signalé la mise à jour du cache.
...
Si le `cq:ContentSyncConfig` noeud n’a aucune `updateuser` propriété, l’ `anonymous` utilisateur par défaut met à jour le cache.
...
**contenu** Générer le contenu à l’aide du traitement [de requête](/help/sites-developing/the-basics.md#sling-request-processing)Sling standard.
